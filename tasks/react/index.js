...
const generateReactComponents = async (cb) => {
    const packs = fsx.readdirSync(ASSETS_DIR, {withFileTypes: true})
        .filter(dirent => dirent.isDirectory() && dirent.name != ".git" && dirent.name !== "rrze-icon-set (tango)"
            && dirent.name === 'CoreUI Flags'
            || dirent.name === 'Akar Icons'
            || dirent.name === 'Flat Color Icons'
        )
        .map(p => {
...
const exportPack = async (pack) => {
    ...
    for (const file of files) {
        ...
        exists.add(name)
        const comRes = template(svgStr, pack.multiColor, name, "module", exists.size == 1)
        ...
        const dtsRes = template(svgStr, pack.multiColor, name, "dts", exists.size == 1)
        await fs.appendFile(
        ...
        const modRes = template(svgStr, pack.multiColor, name, "esm", exists.size == 1)
        await fs.appendFile(
...
const transformJSX = () => {
    return src('../iconsea_react/**/*.js', {base: '../iconsea_react'}).pipe(babel({
