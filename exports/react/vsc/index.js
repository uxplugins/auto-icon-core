function _extends() { _extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; }; return _extends.apply(this, arguments); }

var React = require("react");

module.exports.VscAccount = function VscAccount(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M16 7.992C16 3.58 12.416 0 8 0S0 3.58 0 7.992c0 2.43 1.104 4.62 2.832 6.09.016.016.032.016.032.032.144.112.288.224.448.336.08.048.144.111.224.175A7.98 7.98 0 0 0 8.016 16a7.98 7.98 0 0 0 4.48-1.375c.08-.048.144-.111.224-.16.144-.111.304-.223.448-.335.016-.016.032-.016.032-.032 1.696-1.487 2.8-3.676 2.8-6.106zm-8 7.001c-1.504 0-2.88-.48-4.016-1.279.016-.128.048-.255.08-.383a4.17 4.17 0 0 1 .416-.991c.176-.304.384-.576.64-.816.24-.24.528-.463.816-.639.304-.176.624-.304.976-.4A4.15 4.15 0 0 1 8 10.342a4.185 4.185 0 0 1 2.928 1.166c.368.368.656.8.864 1.295.112.288.192.592.24.911A7.03 7.03 0 0 1 8 14.993zm-2.448-7.4a2.49 2.49 0 0 1-.208-1.024c0-.351.064-.703.208-1.023.144-.32.336-.607.576-.847.24-.24.528-.431.848-.575.32-.144.672-.208 1.024-.208.368 0 .704.064 1.024.208.32.144.608.336.848.575.24.24.432.528.576.847.144.32.208.672.208 1.023 0 .368-.064.704-.208 1.023a2.84 2.84 0 0 1-.576.848 2.84 2.84 0 0 1-.848.575 2.715 2.715 0 0 1-2.064 0 2.84 2.84 0 0 1-.848-.575 2.526 2.526 0 0 1-.56-.848zm7.424 5.306c0-.032-.016-.048-.016-.08a5.22 5.22 0 0 0-.688-1.406 4.883 4.883 0 0 0-1.088-1.135 5.207 5.207 0 0 0-1.04-.608 2.82 2.82 0 0 0 .464-.383 4.2 4.2 0 0 0 .624-.784 3.624 3.624 0 0 0 .528-1.934 3.71 3.71 0 0 0-.288-1.47 3.799 3.799 0 0 0-.816-1.199 3.845 3.845 0 0 0-1.2-.8 3.72 3.72 0 0 0-1.472-.287 3.72 3.72 0 0 0-1.472.288 3.631 3.631 0 0 0-1.2.815 3.84 3.84 0 0 0-.8 1.199 3.71 3.71 0 0 0-.288 1.47c0 .352.048.688.144 1.007.096.336.224.64.4.927.16.288.384.544.624.784.144.144.304.271.48.383a5.12 5.12 0 0 0-1.04.624c-.416.32-.784.703-1.088 1.119a4.999 4.999 0 0 0-.688 1.406c-.016.032-.016.064-.016.08C1.776 11.636.992 9.91.992 7.992.992 4.14 4.144.991 8 .991s7.008 3.149 7.008 7.001a6.96 6.96 0 0 1-2.032 4.907z"
  }));
};

module.exports.VscActivateBreakpoints = function VscActivateBreakpoints(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M15 5.5a4.394 4.394 0 0 1-4 4.5 2.955 2.955 0 0 0-.2-1A3.565 3.565 0 0 0 14 5.5a3.507 3.507 0 0 0-7-.3A3.552 3.552 0 0 0 6 5a4.622 4.622 0 0 1 4.5-4A4.481 4.481 0 0 1 15 5.5zM5.5 6a4.5 4.5 0 1 0 0 9.001 4.5 4.5 0 0 0 0-9z"
  }));
};

module.exports.VscAdd = function VscAdd(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14 7v1H8v6H7V8H1V7h6V1h1v6h6z"
  }));
};

module.exports.VscArchive = function VscArchive(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14.5 1h-13l-.5.5v3l.5.5H2v8.5l.5.5h11l.5-.5V5h.5l.5-.5v-3l-.5-.5zm-1 3H2V2h12v2h-.5zM3 13V5h10v8H3zm8-6H5v1h6V7z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscArrowBoth = function VscArrowBoth(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m3 9 2.146 2.146-.707.708-3-3v-.708l3-3 .707.708L3 8h10l-2.146-2.146.707-.708 3 3v.708l-3 3-.707-.707L13 9H3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscArrowDown = function VscArrowDown(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m3.147 9 5 5h.707l5-5-.707-.707L9 12.439V2H8v10.44L3.854 8.292 3.147 9z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscArrowLeft = function VscArrowLeft(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m7 3.093-5 5V8.8l5 5 .707-.707-4.146-4.147H14v-1H3.56L7.708 3.8 7 3.093z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscArrowRight = function VscArrowRight(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m9 13.887 5-5V8.18l-5-5-.707.707 4.146 4.147H2v1h10.44L8.292 13.18l.707.707z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscArrowSmallDown = function VscArrowSmallDown(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m10.7 8.64-2.5 2.5h-.7L5 8.64l.7-.71 1.65 1.64V4h1v5.57L10 7.92l.7.72z"
  }));
};

module.exports.VscArrowSmallLeft = function VscArrowSmallLeft(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M6.5 10.7 4 8.2v-.7L6.5 5l.71.7-1.64 1.65h5.57v1H5.57L7.22 10l-.72.7z"
  }));
};

module.exports.VscArrowSmallRight = function VscArrowSmallRight(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m8.64 5 2.5 2.5v.7l-2.5 2.5-.71-.7 1.64-1.65H4v-1h5.57L7.92 5.7l.72-.7z"
  }));
};

module.exports.VscArrowSmallUp = function VscArrowSmallUp(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M5 6.5 7.5 4h.7l2.5 2.5-.7.71-1.65-1.64v5.57h-1V5.57L5.7 7.22 5 6.5z"
  }));
};

module.exports.VscArrowSwap = function VscArrowSwap(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4.207 15.061 1 11.854v-.707L4.207 7.94l.707.707-2.353 2.354H15v1H2.56l2.354 2.353-.707.707zm7.586-7L15 4.854v-.707L11.793.94l-.707.707L13.439 4H1v1h12.44l-2.354 2.354.707.707z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscArrowUp = function VscArrowUp(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m13.854 7-5-5h-.707l-5 5 .707.707L8 3.561V14h1V3.56l4.146 4.147.708-.707z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscAzureDevops = function VscAzureDevops(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M15 3.622v8.512L11.5 15l-5.425-1.975v1.957l-3.071-4.01 8.951.698V4.006L15 3.622Zm-2.984.428L6.994 1v2.001L2.383 4.356 1 6.13v4.029l1.978.873V5.869l9.038-1.818Z"
  }));
};

module.exports.VscAzure = function VscAzure(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m15.37 13.68-4-12a1 1 0 0 0-1-.68H5.63a1 1 0 0 0-.95.68l-4.05 12a1 1 0 0 0 1 1.32h2.93a1 1 0 0 0 .94-.68l.61-1.78 3 2.27a1 1 0 0 0 .6.19h4.68a1 1 0 0 0 .98-1.32Zm-5.62.66a.32.32 0 0 1-.2-.07L3.9 10.08l-.09-.07h3l.08-.21 1-2.53 2.24 6.63a.34.34 0 0 1-.38.44Zm4.67 0H10.7a1 1 0 0 0 0-.66l-4.05-12h3.72a.34.34 0 0 1 .32.23l4.05 12a.34.34 0 0 1-.32.43Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscBeakerStop = function VscBeakerStop(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3 15.006h5.007a5.03 5.03 0 0 1-.998-.993L3 14.017l1.542-3.011H6V11c0-.34.034-.673.099-.994h-1.05l1.844-3.598L7 6.193V2.036l2-.024v4.237l.07.137c.31-.13.636-.23.974-.295L10 6.006v-4h1v-1H9.994V1l-.456.005H5V2h1v3.952l-3.894 7.609A1 1 0 0 0 3 15.006Zm5.778-7.332a4 4 0 1 1 4.444 6.652 4 4 0 0 1-4.444-6.652Zm.1 5.447A3 3 0 0 0 11 14a3 3 0 0 0 1.74-.55L8.55 9.26A3 3 0 0 0 8 11a3 3 0 0 0 .879 2.121Zm.382-4.57 4.19 4.189A3 3 0 0 0 14 11a3 3 0 0 0-3-3 3 3 0 0 0-1.74.55Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscBeaker = function VscBeaker(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13.893 13.558 10 6.006v-4h1v-1H9.994V1l-.456.005H5V2h1v3.952l-3.894 7.609A1 1 0 0 0 3 15.006h10a1 1 0 0 0 .893-1.448zm-7-7.15L7 6.193V2.036l2-.024v4.237l.11.215 1.827 3.542H5.049l1.844-3.598zM3 14.017l1.54-3.011h6.916l1.547 3L3 14.017z"
  }));
};

module.exports.VscBellDot = function VscBellDot(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13 8.9c.1.6.2 1.1.4 1.7l.6 1.7-.5.7H10c0 .5-.2 1-.6 1.4-.4.4-.9.6-1.4.6-.5 0-1.1-.2-1.4-.6-.4-.4-.6-.9-.6-1.4H2.5l-.5-.7.6-1.7c.2-.8.4-1.6.4-2.4V6c0-.7.1-1.4.4-2 .3-.7.7-1.2 1.2-1.7s1.1-.8 1.8-1C6.9 1.1 7.5 1 8 1c-.2.3-.4.7-.6 1.1-.2 0-.4 0-.7.2-.5.1-1 .4-1.4.8-.4.3-.8.8-1 1.3-.2.5-.3 1-.3 1.6v2.2c0 .9-.2 1.8-.4 2.7L3.2 12h9.6l-.4-1.1c-.175-.526-.274-1.13-.363-1.674L12 9c.4 0 .7 0 1-.1zM8 14c.2 0 .5-.1.7-.3.2-.2.3-.4.3-.7H7c0 .3.1.5.3.7.2.2.5.3.7.3zm7-10a3 3 0 1 1-6 0 3 3 0 0 1 6 0z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscBell = function VscBell(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13.377 10.573a7.63 7.63 0 0 1-.383-2.38V6.195a5.115 5.115 0 0 0-1.268-3.446 5.138 5.138 0 0 0-3.242-1.722c-.694-.072-1.4 0-2.07.227-.67.215-1.28.574-1.794 1.053a4.923 4.923 0 0 0-1.208 1.675 5.067 5.067 0 0 0-.431 2.022v2.2a7.61 7.61 0 0 1-.383 2.37L2 12.343l.479.658h3.505c0 .526.215 1.04.586 1.412.37.37.885.586 1.412.586.526 0 1.04-.215 1.411-.586s.587-.886.587-1.412h3.505l.478-.658-.586-1.77zm-4.69 3.147a.997.997 0 0 1-.705.299.997.997 0 0 1-.706-.3.997.997 0 0 1-.3-.705h1.999a.939.939 0 0 1-.287.706zm-5.515-1.71.371-1.114a8.633 8.633 0 0 0 .443-2.691V6.004c0-.563.12-1.113.347-1.616.227-.514.55-.969.969-1.34.419-.382.91-.67 1.436-.837.538-.18 1.1-.24 1.65-.18a4.147 4.147 0 0 1 2.597 1.4 4.133 4.133 0 0 1 1.004 2.776v2.01c0 .909.144 1.818.443 2.691l.371 1.113h-9.63v-.012z"
  }));
};

module.exports.VscBold = function VscBold(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M5 13V3h3.362c1.116 0 1.954.224 2.515.673.565.449.848 1.113.848 1.992 0 .467-.137.881-.41 1.243-.273.357-.645.634-1.116.831.556.151.993.44 1.314.865.325.422.487.925.487 1.511 0 .898-.299 1.603-.897 2.116-.598.513-1.443.769-2.536.769H5zm1.356-4.677v3.599h2.24c.63 0 1.127-.158 1.49-.474.367-.32.55-.76.55-1.319 0-1.204-.673-1.806-2.02-1.806h-2.26zm0-1.058h2.049c.593 0 1.066-.144 1.42-.433.357-.288.536-.68.536-1.174 0-.55-.165-.948-.494-1.195-.33-.252-.831-.378-1.505-.378H6.356v3.18z"
  }));
};

module.exports.VscBook = function VscBook(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14.5 2H9l-.35.15-.65.64-.65-.64L7 2H1.5l-.5.5v10l.5.5h5.29l.86.85h.7l.86-.85h5.29l.5-.5v-10l-.5-.5zm-7 10.32-.18-.17L7 12H2V3h4.79l.74.74-.03 8.58zM14 12H9l-.35.15-.14.13V3.7l.7-.7H14v9zM6 5H3v1h3V5zm0 4H3v1h3V9zM3 7h3v1H3V7zm10-2h-3v1h3V5zm-3 2h3v1h-3V7zm0 2h3v1h-3V9z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscBookmark = function VscBookmark(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M12.5 1h-9l-.5.5v13l.872.335L8 10.247l4.128 4.588L13 14.5v-13l-.5-.5zM12 13.2 8.372 9.165h-.744L4 13.2V2h8v11.2z"
  }));
};

module.exports.VscBracketDot = function VscBracketDot(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M6 2.972v.012h-.09c-.199 0-.386.04-.562.121a1.468 1.468 0 0 0-.457.32 1.47 1.47 0 0 0-.42 1.035c0 .227.004.453.012.677.008.224.008.446 0 .665a4.751 4.751 0 0 1-.055.64c-.033.21-.089.41-.168.603A2.284 2.284 0 0 1 3.522 8a2.284 2.284 0 0 1 .738.955c.08.193.135.395.168.608.033.206.051.42.055.64.008.216.008.438 0 .666-.008.22-.012.444-.012.672 0 .203.037.394.111.573.079.177.182.333.309.468.13.13.283.237.457.319.175.077.363.115.563.115H6V14h-.09c-.313 0-.616-.062-.909-.185a2.33 2.33 0 0 1-.775-.53 2.23 2.23 0 0 1-.493-.753v-.001a3.54 3.54 0 0 1-.198-.824v-.002a6.186 6.186 0 0 1-.024-.87c.012-.289.018-.578.018-.868 0-.198-.04-.387-.117-.566V9.4a1.414 1.414 0 0 0-.308-.465l-.002-.002a1.377 1.377 0 0 0-.455-.318h-.001a1.316 1.316 0 0 0-.557-.122H2v-.984h.09c.195 0 .38-.038.557-.115a1.504 1.504 0 0 0 .765-.786v-.002c.078-.18.117-.37.117-.572 0-.29-.006-.58-.018-.869a6.08 6.08 0 0 1 .024-.863v-.002c.033-.287.099-.564.197-.83v-.001a2.23 2.23 0 0 1 .494-.753A2.33 2.33 0 0 1 5 2.185c.293-.123.596-.185.91-.185H6v.972Zm7.923 5.52H14v-.984h-.09c-.195 0-.38-.04-.556-.121l-.001-.001a1.376 1.376 0 0 1-.455-.318l-.002-.002a1.414 1.414 0 0 1-.307-.465l-.001-.002a1.405 1.405 0 0 1-.117-.566c0-.29.006-.58.018-.869a6.19 6.19 0 0 0-.024-.87v-.001a3.542 3.542 0 0 0-.197-.824v-.001a2.23 2.23 0 0 0-.494-.753 2.33 2.33 0 0 0-.775-.53 2.325 2.325 0 0 0-.91-.185H10v.984h.09c.2 0 .386.038.562.115.174.082.326.188.457.32.127.134.23.29.309.467.074.18.11.37.11.573 0 .228-.003.452-.011.672-.008.228-.008.45 0 .665.004.222.022.435.055.64.033.214.089.416.168.609a2.282 2.282 0 0 0 .738.955l-.032.025c.53.058 1.03.221 1.477.467Z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    d: "M12 9a3 3 0 1 0 0 6 3 3 0 0 0 0-6Z"
  }));
};

module.exports.VscBracketError = function VscBracketError(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M6 2.972v.012h-.09c-.199 0-.386.04-.562.121a1.468 1.468 0 0 0-.457.32 1.47 1.47 0 0 0-.42 1.035c0 .227.004.453.012.677.008.224.008.446 0 .665a4.751 4.751 0 0 1-.055.64c-.033.21-.089.41-.168.603A2.284 2.284 0 0 1 3.522 8a2.284 2.284 0 0 1 .738.955c.08.193.135.395.168.608.033.206.051.42.055.64.008.216.008.438 0 .666-.008.22-.012.444-.012.672 0 .203.037.394.111.573.079.177.182.333.309.468.13.13.283.237.457.319.175.077.363.115.563.115H6V14h-.09c-.313 0-.616-.062-.909-.185a2.33 2.33 0 0 1-.775-.53 2.23 2.23 0 0 1-.493-.753v-.001a3.54 3.54 0 0 1-.198-.824v-.002a6.186 6.186 0 0 1-.024-.87c.012-.289.018-.578.018-.868 0-.198-.04-.387-.117-.566V9.4a1.414 1.414 0 0 0-.308-.465l-.002-.002a1.377 1.377 0 0 0-.455-.318h-.001a1.316 1.316 0 0 0-.557-.122H2v-.984h.09c.195 0 .38-.038.557-.115a1.504 1.504 0 0 0 .765-.786v-.002c.078-.18.117-.37.117-.572 0-.29-.006-.58-.018-.869a6.08 6.08 0 0 1 .024-.863v-.002c.033-.287.099-.564.197-.83v-.001a2.23 2.23 0 0 1 .494-.753A2.33 2.33 0 0 1 5 2.185c.293-.123.596-.185.91-.185H6v.972Zm7.923 5.52H14v-.984h-.09c-.195 0-.38-.04-.556-.121l-.001-.001a1.376 1.376 0 0 1-.455-.318l-.002-.002a1.414 1.414 0 0 1-.307-.465l-.001-.002a1.405 1.405 0 0 1-.117-.566c0-.29.006-.58.018-.869a6.19 6.19 0 0 0-.024-.87v-.001a3.542 3.542 0 0 0-.197-.824v-.001a2.23 2.23 0 0 0-.494-.753 2.33 2.33 0 0 0-.775-.53 2.325 2.325 0 0 0-.91-.185H10v.984h.09c.2 0 .386.038.562.115.174.082.326.188.457.32.127.134.23.29.309.467.074.18.11.37.11.573 0 .228-.003.452-.011.672-.008.228-.008.45 0 .665.004.222.022.435.055.64.033.214.089.416.168.609a2.282 2.282 0 0 0 .738.955l-.032.025c.53.058 1.03.221 1.477.467ZM10.333 9.506a3 3 0 1 1 3.334 4.988 3 3 0 0 1-3.334-4.988Zm2.813.64L12 11.293l-1.146-1.147-.707.707L11.293 12l-1.147 1.146.708.708L12 12.707l1.146 1.146.708-.707L12.707 12l1.147-1.146-.708-.708Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscBriefcase = function VscBriefcase(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14.5 4H11V2.5l-.5-.5h-5l-.5.5V4H1.5l-.5.5v8l.5.5h13l.5-.5v-8l-.5-.5zM6 3h4v1H6V3zm8 2v.76L10 8v-.5L9.51 7h-3L6 7.5V8L2 5.71V5h12zM9 8v1H7V8h2zm-7 4V6.86l4 2.29v.35l.5.5h3l.5-.5v-.31l4-2.28V12H2z"
  }));
};

module.exports.VscBroadcast = function VscBroadcast(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M4.667 2.011A6 6 0 0 1 8 1a6.007 6.007 0 0 1 6 6 6 6 0 0 1-3.996 5.655v-.044c.016-.014.031-.03.046-.045a1.48 1.48 0 0 0 .434-1.046v-.137A5.042 5.042 0 0 0 12.19 4.2a5.04 5.04 0 1 0-6.69 7.176v.144a1.48 1.48 0 0 0 .48 1.09v.04A5.999 5.999 0 0 1 4.667 2.01z"
  }), React.createElement("path", {
    d: "M9.343 11.86a.48.48 0 0 1-.34.14v2.52a.48.48 0 0 1-.48.48H7.46c.011 0-.004-.004-.034-.012-.075-.02-.241-.064-.305-.129a.48.48 0 0 1-.141-.34V12a.48.48 0 0 1-.48-.48V9.5a1 1 0 0 1 1-1h.984a1 1 0 0 1 1 1v2.02a.48.48 0 0 1-.137.335l-.004.004z"
  }), React.createElement("path", {
    d: "M10.64 7c0 .525-.157 1.034-.445 1.465.183.302.289.656.289 1.035v.106a3.596 3.596 0 0 0 .06-5.15A3.6 3.6 0 1 0 5.5 9.59V9.5c0-.384.108-.743.296-1.047A2.64 2.64 0 1 1 10.64 7z"
  }), React.createElement("path", {
    d: "M9 7a1 1 0 1 1-2 0 1 1 0 0 1 2 0z"
  }));
};

module.exports.VscBrowser = function VscBrowser(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 1h13l.5.5v12l-.5.5h-13l-.5-.5v-12l.5-.5zM2 5v8h12V5H2zm0-1h12V2H2v2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscBug = function VscBug(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M10.877 4.5v-.582a2.918 2.918 0 1 0-5.836 0V4.5h-.833L2.545 2.829l-.593.59 1.611 1.619-.019.049a8.03 8.03 0 0 0-.503 2.831c0 .196.007.39.02.58l.003.045H1v.836h2.169l.006.034c.172.941.504 1.802.954 2.531l.034.055L2.2 13.962l.592.592 1.871-1.872.058.066c.868.992 2.002 1.589 3.238 1.589 1.218 0 2.336-.579 3.199-1.544l.057-.064 1.91 1.92.593-.591-1.996-2.006.035-.056c.467-.74.81-1.619.986-2.583l.006-.034h2.171v-.836h-2.065l.003-.044a8.43 8.43 0 0 0 .02-.58 8.02 8.02 0 0 0-.517-2.866l-.019-.05 1.57-1.57-.592-.59L11.662 4.5h-.785zm-5 0v-.582a2.082 2.082 0 1 1 4.164 0V4.5H5.878zm5.697.837.02.053c.283.753.447 1.61.447 2.528 0 1.61-.503 3.034-1.274 4.037-.77 1.001-1.771 1.545-2.808 1.545-1.036 0-2.037-.544-2.807-1.545-.772-1.003-1.275-2.427-1.275-4.037 0-.918.164-1.775.448-2.528l.02-.053h7.229z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCalendar = function VscCalendar(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14.5 2H13V1h-1v1H4V1H3v1H1.5l-.5.5v12l.5.5h13l.5-.5v-12l-.5-.5zM14 14H2V5h12v9zm0-10H2V3h12v1zM4 8H3v1h1V8zm-1 2h1v1H3v-1zm1 2H3v1h1v-1zm2-4h1v1H6V8zm1 2H6v1h1v-1zm-1 2h1v1H6v-1zm1-6H6v1h1V6zm2 2h1v1H9V8zm1 2H9v1h1v-1zm-1 2h1v1H9v-1zm1-6H9v1h1V6zm2 2h1v1h-1V8zm1 2h-1v1h1v-1zm-1-4h1v1h-1V6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCallIncoming = function VscCallIncoming(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M12.547 9.328a1.567 1.567 0 0 0-.594-.117 1.202 1.202 0 0 0-.555.101 2.762 2.762 0 0 0-.43.258 2.166 2.166 0 0 0-.359.328c-.104.12-.205.23-.304.329a2.409 2.409 0 0 1-.29.25.534.534 0 0 1-.695-.063 32.17 32.17 0 0 1-.328-.312c-.14-.136-.312-.3-.515-.493A61.776 61.776 0 0 1 7.844 9l-.68-.664a25.847 25.847 0 0 1-1.21-1.266 5.312 5.312 0 0 1-.391-.484c-.094-.135-.141-.234-.141-.297a.46.46 0 0 1 .101-.312c.073-.094.16-.19.258-.29.1-.098.209-.203.328-.312.12-.11.23-.227.329-.352.098-.125.182-.268.25-.43.067-.16.101-.343.101-.546a1.567 1.567 0 0 0-.453-1.102 7.604 7.604 0 0 1-.531-.578 6.487 6.487 0 0 0-.617-.64 4.207 4.207 0 0 0-.696-.516A1.46 1.46 0 0 0 3.742 1a1.567 1.567 0 0 0-1.101.453c-.271.271-.508.513-.711.727a4.006 4.006 0 0 0-.516.664 2.63 2.63 0 0 0-.312.765A4.39 4.39 0 0 0 1 4.625c0 .552.089 1.125.266 1.719.177.593.416 1.185.718 1.773.302.589.67 1.167 1.102 1.735.432.567.901 1.106 1.406 1.617.505.51 1.042.982 1.61 1.414.567.432 1.148.805 1.742 1.117.593.313 1.19.557 1.789.734a6.157 6.157 0 0 0 1.75.266 4.696 4.696 0 0 0 1.008-.11 2.59 2.59 0 0 0 .773-.312c.23-.14.45-.312.664-.515.214-.204.453-.438.719-.704A1.568 1.568 0 0 0 15 12.257a2.009 2.009 0 0 0-.102-.515 1.674 1.674 0 0 0-.257-.484 7.24 7.24 0 0 0-.368-.445 5.381 5.381 0 0 0-.421-.422 91.549 91.549 0 0 0-.43-.383 8.277 8.277 0 0 1-.367-.344 1.516 1.516 0 0 0-.508-.336zm-.367 4.586a3.13 3.13 0 0 1-.797.086 5.526 5.526 0 0 1-1.516-.242 8.362 8.362 0 0 1-1.586-.664 13.205 13.205 0 0 1-3.047-2.297 17.15 17.15 0 0 1-1.289-1.461 10.502 10.502 0 0 1-1.03-1.578 9.12 9.12 0 0 1-.673-1.61A5.308 5.308 0 0 1 2 4.602a3.34 3.34 0 0 1 .094-.79c.057-.218.143-.414.258-.585.114-.172.255-.339.421-.5.167-.162.357-.35.57-.563a.542.542 0 0 1 .4-.164c.062-.005.158.036.288.125.13.089.271.195.422.32a7.058 7.058 0 0 1 .899.899c.125.15.229.289.312.414.083.125.125.221.125.289a.429.429 0 0 1-.101.312c-.073.084-.16.18-.258.29-.1.109-.209.213-.328.312-.12.099-.23.216-.329.351a2.266 2.266 0 0 0-.25.438 1.345 1.345 0 0 0-.101.54c.005.213.047.413.125.6.078.188.19.355.336.5l3.726 3.727a1.527 1.527 0 0 0 1.102.46 1.2 1.2 0 0 0 .547-.1 2.414 2.414 0 0 0 .789-.586c.11-.12.21-.23.305-.329.093-.098.19-.182.289-.25a.545.545 0 0 1 .312-.101c.073 0 .172.042.297.125.125.083.263.19.414.32.151.13.307.274.469.43.161.156.305.312.43.469.124.156.229.297.312.422.083.125.125.22.125.289a.533.533 0 0 1-.164.39c-.224.219-.414.41-.57.57a3.159 3.159 0 0 1-.5.422 1.93 1.93 0 0 1-.586.266zM15 1.704l-4.64 4.648h3.288v1h-5v-5h1V5.64L14.297 1l.703.703z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCallOutgoing = function VscCallOutgoing(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8.648 6.648 13.29 2H10V1h5v5h-1V2.71L9.352 7.353l-.704-.704zm3.305 2.563a1.567 1.567 0 0 1 1.102.453c.11.11.232.224.367.344l.43.383c.15.135.291.276.421.421.13.146.253.295.368.446.114.15.2.312.257.484.058.172.092.344.102.516a1.568 1.568 0 0 1-.453 1.101c-.266.266-.505.5-.719.704a4.006 4.006 0 0 1-.664.515c-.23.14-.487.245-.773.313a4.696 4.696 0 0 1-1.008.109 6.157 6.157 0 0 1-1.75-.266A9.819 9.819 0 0 1 7.843 14a12.445 12.445 0 0 1-1.741-1.117 15.329 15.329 0 0 1-1.61-1.414c-.505-.51-.974-1.05-1.406-1.617a11.64 11.64 0 0 1-1.102-1.735 10.38 10.38 0 0 1-.718-1.773A6.005 6.005 0 0 1 1 4.625c0-.396.034-.734.102-1.016a2.63 2.63 0 0 1 .312-.765c.14-.23.313-.45.516-.664.203-.214.44-.456.71-.727A1.567 1.567 0 0 1 3.743 1c.26 0 .51.07.75.21.24.142.472.313.696.517.223.203.43.416.617.64.187.224.364.417.53.578a1.567 1.567 0 0 1 .453 1.102 1.4 1.4 0 0 1-.1.547 1.824 1.824 0 0 1-.25.43 2.983 2.983 0 0 1-.329.351c-.12.11-.229.214-.328.313a3.128 3.128 0 0 0-.258.289.46.46 0 0 0-.101.312c0 .063.047.162.14.297a5.3 5.3 0 0 0 .391.484 24.386 24.386 0 0 0 1.211 1.266c.234.23.461.45.68.664.218.214.43.417.633.61.203.192.375.356.515.492.14.135.25.24.328.312a.534.534 0 0 0 .696.063c.093-.068.19-.152.289-.25.099-.1.2-.209.304-.329.104-.12.224-.229.36-.328.135-.099.278-.185.43-.258a1.21 1.21 0 0 1 .554-.101zM11.383 14c.318 0 .583-.029.797-.086a1.93 1.93 0 0 0 .586-.266c.177-.12.343-.26.5-.421.156-.162.346-.352.57-.57.11-.11.164-.24.164-.391 0-.068-.042-.164-.125-.29a6.122 6.122 0 0 0-.313-.421 5.01 5.01 0 0 0-.43-.47c-.16-.155-.317-.299-.468-.429a4.322 4.322 0 0 0-.414-.32c-.125-.083-.224-.125-.297-.125a.545.545 0 0 0-.312.101 1.801 1.801 0 0 0-.29.25c-.093.1-.195.209-.304.329-.11.12-.23.229-.36.328-.13.099-.273.185-.43.258a1.208 1.208 0 0 1-.546.101 1.527 1.527 0 0 1-1.102-.46L4.883 7.39a1.537 1.537 0 0 1-.336-.5 1.655 1.655 0 0 1-.125-.602c0-.203.034-.383.101-.539.068-.156.151-.302.25-.438.1-.135.209-.252.329-.351.12-.099.229-.203.328-.313.099-.109.185-.205.258-.289a.429.429 0 0 0 .101-.312c0-.068-.042-.164-.125-.29a5.085 5.085 0 0 0-.312-.413 6.791 6.791 0 0 0-.43-.469 6.787 6.787 0 0 0-.469-.43 5.674 5.674 0 0 0-.422-.32c-.13-.089-.226-.13-.289-.125a.542.542 0 0 0-.398.164 65.24 65.24 0 0 1-.57.563 3.073 3.073 0 0 0-.422.5 1.9 1.9 0 0 0-.258.586A3.377 3.377 0 0 0 2 4.601c0 .5.08 1.015.242 1.546a9.12 9.12 0 0 0 .672 1.61c.287.541.63 1.068 1.031 1.578.401.51.831.997 1.29 1.46a13.205 13.205 0 0 0 3.046 2.298 8.37 8.37 0 0 0 1.586.664 5.526 5.526 0 0 0 1.516.242z"
  }));
};

module.exports.VscCaseSensitive = function VscCaseSensitive(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8.854 11.702h-1l-.816-2.159H3.772l-.768 2.16H2L4.954 4h.935l2.965 7.702Zm-2.111-2.97L5.534 5.45a3.142 3.142 0 0 1-.118-.515h-.021c-.036.218-.077.39-.124.515L4.073 8.732h2.67ZM13.756 11.702h-.88v-.86h-.022c-.383.66-.947.99-1.692.99-.548 0-.978-.146-1.29-.436-.307-.29-.461-.675-.461-1.155 0-1.027.605-1.625 1.815-1.794l1.65-.23c0-.935-.379-1.403-1.134-1.403-.663 0-1.26.226-1.794.677V6.59c.54-.344 1.164-.516 1.87-.516 1.292 0 1.938.684 1.938 2.052v3.577Zm-.88-2.782-1.327.183c-.409.057-.717.159-.924.306-.208.143-.312.399-.312.768 0 .268.095.489.285.66.193.169.45.253.768.253a1.41 1.41 0 0 0 1.08-.457c.286-.308.43-.696.43-1.165V8.92Z"
  }));
};

module.exports.VscCheckAll = function VscCheckAll(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M15.62 3.596 7.815 12.81l-.728-.033L4 8.382l.754-.53 2.744 3.907L14.917 3l.703.596z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m7.234 8.774 4.386-5.178L10.917 3l-4.23 4.994.547.78zm-1.55.403.548.78-.547-.78zm-1.617 1.91.547.78-.799.943-.728-.033L0 8.382l.754-.53 2.744 3.907.57-.672z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCheck = function VscCheck(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m14.431 3.323-8.47 10-.79-.036-3.35-4.77.818-.574 2.978 4.24 8.051-9.506.764.646z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscChecklist = function VscChecklist(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3.75 4.48h-.71L2 3.43l.71-.7.69.68L4.81 2l.71.71-1.77 1.77zM6.99 3h8v1h-8V3zm0 3h8v1h-8V6zm8 3h-8v1h8V9zm-8 3h8v1h-8v-1zM3.04 7.48h.71l1.77-1.77-.71-.7L3.4 6.42l-.69-.69-.71.71 1.04 1.04zm.71 3.01h-.71L2 9.45l.71-.71.69.69 1.41-1.42.71.71-1.77 1.77zm-.71 3.01h.71l1.77-1.77-.71-.71-1.41 1.42-.69-.69-.71.7 1.04 1.05z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscChevronDown = function VscChevronDown(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m7.976 10.072 4.357-4.357.62.618L8.284 11h-.618L3 6.333l.619-.618 4.357 4.357z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscChevronLeft = function VscChevronLeft(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m5.928 7.976 4.357 4.357-.618.62L5 8.284v-.618L9.667 3l.618.619-4.357 4.357z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscChevronRight = function VscChevronRight(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M10.072 8.024 5.715 3.667l.618-.62L11 7.716v.618L6.333 13l-.618-.619 4.357-4.357z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscChevronUp = function VscChevronUp(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m8.024 5.928-4.357 4.357-.62-.618L7.716 5h.618L13 9.667l-.619.618-4.357-4.357z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscChromeClose = function VscChromeClose(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m7.116 8-4.558 4.558.884.884L8 8.884l4.558 4.558.884-.884L8.884 8l4.558-4.558-.884-.884L8 7.116 3.442 2.558l-.884.884L7.116 8z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscChromeMaximize = function VscChromeMaximize(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M3 3v10h10V3H3zm9 9H4V4h8v8z"
  }));
};

module.exports.VscChromeMinimize = function VscChromeMinimize(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14 8v1H3V8h11z"
  }));
};

module.exports.VscChromeRestore = function VscChromeRestore(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M3 5v9h9V5H3zm8 8H4V6h7v7z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5 5h1V4h7v7h-1v1h2V3H5v2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCircleFilled = function VscCircleFilled(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8 4c.367 0 .721.048 1.063.145a3.943 3.943 0 0 1 1.762 1.031 3.944 3.944 0 0 1 1.03 1.762c.097.34.145.695.145 1.062 0 .367-.048.721-.145 1.063a3.94 3.94 0 0 1-1.03 1.765 4.017 4.017 0 0 1-1.762 1.031C8.72 11.953 8.367 12 8 12s-.721-.047-1.063-.14a4.056 4.056 0 0 1-1.765-1.032A4.055 4.055 0 0 1 4.14 9.062 3.992 3.992 0 0 1 4 8c0-.367.047-.721.14-1.063a4.02 4.02 0 0 1 .407-.953A4.089 4.089 0 0 1 5.98 4.546a3.94 3.94 0 0 1 .957-.401A3.89 3.89 0 0 1 8 4z"
  }));
};

module.exports.VscCircleLargeFilled = function VscCircleLargeFilled(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8 1a6.8 6.8 0 0 1 1.86.253 6.899 6.899 0 0 1 3.083 1.805 6.903 6.903 0 0 1 1.804 3.083C14.916 6.738 15 7.357 15 8s-.084 1.262-.253 1.86a6.9 6.9 0 0 1-.704 1.674 7.157 7.157 0 0 1-2.516 2.509 6.966 6.966 0 0 1-1.668.71A6.984 6.984 0 0 1 8 15a6.984 6.984 0 0 1-1.86-.246 7.098 7.098 0 0 1-1.674-.711 7.3 7.3 0 0 1-1.415-1.094 7.295 7.295 0 0 1-1.094-1.415 7.098 7.098 0 0 1-.71-1.675A6.985 6.985 0 0 1 1 8c0-.643.082-1.262.246-1.86a6.968 6.968 0 0 1 .711-1.667 7.156 7.156 0 0 1 2.509-2.516 6.895 6.895 0 0 1 1.675-.704A6.808 6.808 0 0 1 8 1z"
  }));
};

module.exports.VscCircleLargeOutline = function VscCircleLargeOutline(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M9.588 2.215A5.808 5.808 0 0 0 8 2c-.554 0-1.082.073-1.588.215l-.006.002c-.514.141-.99.342-1.432.601A6.156 6.156 0 0 0 2.82 4.98l-.002.004A5.967 5.967 0 0 0 2.21 6.41 5.986 5.986 0 0 0 2 8c0 .555.07 1.085.21 1.591a6.05 6.05 0 0 0 1.548 2.651c.37.365.774.677 1.216.94a6.1 6.1 0 0 0 1.435.609A6.02 6.02 0 0 0 8 14c.555 0 1.085-.07 1.591-.21.515-.145.99-.348 1.426-.607l.004-.002a6.16 6.16 0 0 0 2.161-2.155 5.85 5.85 0 0 0 .6-1.432l.003-.006A5.807 5.807 0 0 0 14 8c0-.554-.072-1.082-.215-1.588l-.002-.006a5.772 5.772 0 0 0-.6-1.423l-.002-.004a5.9 5.9 0 0 0-.942-1.21l-.008-.008a5.902 5.902 0 0 0-1.21-.942l-.004-.002a5.772 5.772 0 0 0-1.423-.6l-.006-.002zm4.455 9.32a7.157 7.157 0 0 1-2.516 2.508 6.966 6.966 0 0 1-1.668.71A6.984 6.984 0 0 1 8 15a6.984 6.984 0 0 1-1.86-.246 7.098 7.098 0 0 1-1.674-.711 7.3 7.3 0 0 1-1.415-1.094 7.295 7.295 0 0 1-1.094-1.415 7.098 7.098 0 0 1-.71-1.675A6.985 6.985 0 0 1 1 8c0-.643.082-1.262.246-1.86a6.968 6.968 0 0 1 .711-1.667 7.156 7.156 0 0 1 2.509-2.516 6.895 6.895 0 0 1 1.675-.704A6.808 6.808 0 0 1 8 1a6.8 6.8 0 0 1 1.86.253 6.899 6.899 0 0 1 3.083 1.805 6.903 6.903 0 0 1 1.804 3.083C14.916 6.738 15 7.357 15 8s-.084 1.262-.253 1.86a6.9 6.9 0 0 1-.704 1.674z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCircleOutline = function VscCircleOutline(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8zm2.61-4a2.61 2.61 0 1 1-5.22 0 2.61 2.61 0 0 1 5.22 0zM8 5.246z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCircleSlash = function VscCircleSlash(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8 1a7 7 0 1 1-7 7 7.008 7.008 0 0 1 7-7zM2 8c0 1.418.504 2.79 1.423 3.87l8.447-8.447A5.993 5.993 0 0 0 2 8zm12 0c0-1.418-.504-2.79-1.423-3.87L4.13 12.577A5.993 5.993 0 0 0 14 8z"
  }));
};

module.exports.VscCircuitBoard = function VscCircuitBoard(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14.5 1h-13l-.5.5v13l.5.5h13l.5-.5v-13l-.5-.5zM14 14H5v-2h2.3c.3.6 1 1 1.7 1 1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2H4v3H2V2h2v2.3c-.6.3-1 1-1 1.7 0 1.1.9 2 2 2s2-.9 2-2h2c0 1.1.9 2 2 2s2-.9 2-2-.9-2-2-2c-.7 0-1.4.4-1.7 1H6.7c-.3-.6-1-1-1.7-1V2h9v12zm-6-3c0-.6.4-1 1-1s1 .4 1 1-.4 1-1 1-1-.4-1-1zM5 5c.6 0 1 .4 1 1s-.4 1-1 1-1-.4-1-1 .4-1 1-1zm6 0c.6 0 1 .4 1 1s-.4 1-1 1-1-.4-1-1 .4-1 1-1z"
  }));
};

module.exports.VscClearAll = function VscClearAll(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m10 12.6.7.7 1.6-1.6 1.6 1.6.8-.7L13 11l1.7-1.6-.8-.8-1.6 1.7-1.6-1.7-.7.8 1.6 1.6-1.6 1.6zM1 4h14V3H1v1zm0 3h14V6H1v1zm8 2.5V9H1v1h8v-.5zM9 13v-1H1v1h8z"
  }));
};

module.exports.VscClippy = function VscClippy(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7 13.992H4v-9h8v2h1v-2.5l-.5-.5H11v-1h-1a2 2 0 0 0-4 0H4.94v1H3.5l-.5.5v10l.5.5H7v-1zm0-11.2a1 1 0 0 1 .8-.8 1 1 0 0 1 .58.06.94.94 0 0 1 .45.36 1 1 0 1 1-1.75.94 1 1 0 0 1-.08-.56zm7.08 9.46L13 13.342v-5.35h-1v5.34l-1.08-1.08-.71.71 1.94 1.93h.71l1.93-1.93-.71-.71zm-5.92-4.16h.71l1.93 1.93-.71.71-1.08-1.08v5.34h-1v-5.35l-1.08 1.09-.71-.71 1.94-1.93z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCloseAll = function VscCloseAll(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m8.621 8.086-.707-.707L6.5 8.793 5.086 7.379l-.707.707L5.793 9.5l-1.414 1.414.707.707L6.5 10.207l1.414 1.414.707-.707L7.207 9.5l1.414-1.414z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m5 3 1-1h7l1 1v7l-1 1h-2v2l-1 1H3l-1-1V6l1-1h2V3zm1 2h4l1 1v4h2V3H6v2zm4 1H3v7h7V6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscClose = function VscClose(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m8 8.707 3.646 3.647.708-.707L8.707 8l3.647-3.646-.707-.708L8 7.293 4.354 3.646l-.707.708L7.293 8l-3.646 3.646.707.708L8 8.707z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCloudDownload = function VscCloudDownload(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.957 6h.05a2.99 2.99 0 0 1 2.116.879 3.003 3.003 0 0 1 0 4.242 2.99 2.99 0 0 1-2.117.879v-1a2.002 2.002 0 0 0 0-4h-.914l-.123-.857a2.49 2.49 0 0 0-2.126-2.122A2.478 2.478 0 0 0 6.231 5.5l-.333.762-.809-.189A2.49 2.49 0 0 0 4.523 6c-.662 0-1.297.263-1.764.732A2.503 2.503 0 0 0 4.523 11h.498v1h-.498a3.486 3.486 0 0 1-2.628-1.16 3.502 3.502 0 0 1 1.958-5.78 3.462 3.462 0 0 1 1.468.04 3.486 3.486 0 0 1 3.657-2.06A3.479 3.479 0 0 1 11.957 6zm-5.25 5.121 1.314 1.314V7h.994v5.4l1.278-1.279.707.707-2.146 2.147h-.708L6 11.829l.707-.708z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCloudUpload = function VscCloudUpload(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.956 6h.05a2.99 2.99 0 0 1 2.117.879 3.003 3.003 0 0 1 0 4.242 2.99 2.99 0 0 1-2.117.879h-1.995v-1h1.995a2.002 2.002 0 0 0 0-4h-.914l-.123-.857a2.49 2.49 0 0 0-2.126-2.122A2.478 2.478 0 0 0 6.23 5.5l-.333.762-.809-.189A2.49 2.49 0 0 0 4.523 6c-.662 0-1.297.263-1.764.732A2.503 2.503 0 0 0 4.523 11h2.494v1H4.523a3.486 3.486 0 0 1-2.628-1.16 3.502 3.502 0 0 1-.4-4.137A3.497 3.497 0 0 1 3.853 5.06c.486-.09.987-.077 1.468.041a3.486 3.486 0 0 1 3.657-2.06A3.479 3.479 0 0 1 11.956 6zm-1.663 3.853L8.979 8.54v5.436h-.994v-5.4L6.707 9.854 6 9.146 8.146 7h.708L11 9.146l-.707.707z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCloud = function VscCloud(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.957 6h.05a2.99 2.99 0 0 1 2.116.879 3.003 3.003 0 0 1 0 4.242 2.99 2.99 0 0 1-2.117.879v-.013L12 12H4.523a3.486 3.486 0 0 1-2.628-1.16 3.502 3.502 0 0 1 1.958-5.78 3.462 3.462 0 0 1 1.468.04 3.486 3.486 0 0 1 3.657-2.06A3.479 3.479 0 0 1 11.957 6zM5 11h7.01a1.994 1.994 0 0 0 1.992-2 2.002 2.002 0 0 0-1.996-2h-.914l-.123-.857a2.49 2.49 0 0 0-2.126-2.122A2.478 2.478 0 0 0 6.231 5.5l-.333.762-.809-.189A2.49 2.49 0 0 0 4.523 6c-.662 0-1.297.263-1.764.732A2.503 2.503 0 0 0 4.523 11H5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCode = function VscCode(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M4.708 5.578 2.061 8.224l2.647 2.646-.708.708-3-3V7.87l3-3 .708.708zm7-.708L11 5.578l2.647 2.646L11 10.87l.708.708 3-3V7.87l-3-3zM4.908 13l.894.448 5-10L9.908 3l-5 10z"
  }));
};

module.exports.VscCollapseAll = function VscCollapseAll(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M9 9H4v1h5V9z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m5 3 1-1h7l1 1v7l-1 1h-2v2l-1 1H3l-1-1V6l1-1h2V3zm1 2h4l1 1v4h2V3H6v2zm4 1H3v7h7V6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscColorMode = function VscColorMode(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zm0 13V2a6 6 0 1 1 0 12z"
  }));
};

module.exports.VscCombine = function VscCombine(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m1.5 1-.5.5v3l.5.5h3l.5-.5v-3L4.5 1h-3zM2 4V2h2v2H2zm-.5 2-.5.5v3l.5.5h3l.5-.5v-3L4.5 6h-3zM2 9V7h2v2H2zm-1 2.5.5-.5h3l.5.5v3l-.5.5h-3l-.5-.5v-3zm1 .5v2h2v-2H2zm10.5-7-.5.5v6l.5.5h3l.5-.5v-6l-.5-.5h-3zM15 8h-2V6h2v2zm0 3h-2V9h2v2zM9.1 8H6v1h3.1l-1 1 .7.6 1.8-1.8v-.7L8.8 6.3l-.7.7 1 1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCommentDiscussion = function VscCommentDiscussion(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m4 11.29 1-1v1.42l-1.15 1.14L3 12.5V10H1.5L1 9.5v-8l.5-.5h12l.5.5V6h-1V2H2v7h1.5l.5.5v1.79zM10.29 13l1.86 1.85.85-.35V13h1.5l.5-.5v-5l-.5-.5h-8l-.5.5v5l.5.5h3.79zm.21-1H7V8h7v4h-1.5l-.5.5v.79l-1.15-1.14-.35-.15z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscComment = function VscComment(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14.5 2h-13l-.5.5v9l.5.5H4v2.5l.854.354L7.707 12H14.5l.5-.5v-9l-.5-.5zm-.5 9H7.5l-.354.146L5 13.293V11.5l-.5-.5H2V3h12v8z"
  }));
};

module.exports.VscCompassActive = function VscCompassActive(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M9.101 13.9a6.004 6.004 0 0 1-.601.08V13h-1v.98a6.001 6.001 0 0 1-5.482-5.518H3v-1h-.976A6.001 6.001 0 0 1 7.5 2.02V3h1v-.98a6.001 6.001 0 0 1 5.48 5.48H13v1h.98a6.004 6.004 0 0 1-.08.601c.334.077.652.196.95.351a7 7 0 1 0-5.397 5.397 3.973 3.973 0 0 1-.352-.95Zm.803-3.433L6.99 9.01 4.967 4.967 9.009 6.99l1.459 2.913a4.02 4.02 0 0 0-.564.563Zm-.469-1.032L8.481 7.52l-1.916-.955.954 1.917 1.916.954Z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.333 10.506a3 3 0 1 1 3.333 4.987 3 3 0 0 1-3.333-4.987Zm1.698 3.817 1.79-2.387-.8-.6-1.48 1.974-.876-.7-.624.78 1.278 1.023.712-.09Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCompassDot = function VscCompassDot(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M9.101 13.9a6.004 6.004 0 0 1-.601.08V13h-1v.98a6.001 6.001 0 0 1-5.482-5.518H3v-1h-.976A6.001 6.001 0 0 1 7.5 2.02V3h1v-.98a6.001 6.001 0 0 1 5.48 5.48H13v1h.98a6.004 6.004 0 0 1-.08.601c.334.077.652.196.95.351a7 7 0 1 0-5.397 5.397 3.973 3.973 0 0 1-.352-.95Zm.803-3.433L6.99 9.01 4.967 4.967 9.009 6.99l1.459 2.913a4.02 4.02 0 0 0-.564.563Zm-.469-1.032L8.481 7.52l-1.916-.955.954 1.917 1.916.954Z",
    "clip-rule": "evenodd"
  }), React.createElement("circle", {
    cx: "13",
    cy: "13",
    r: "3"
  }));
};

module.exports.VscCompass = function VscCompass(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m9.009 6.991 2.024 4.042L6.991 9.01 4.967 4.967 9.009 6.99Zm.426 2.444L8.481 7.52l-1.916-.955.954 1.917 1.916.954Z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13.98 8.5a6.002 6.002 0 0 1-5.48 5.48V13h-1v.98a6.001 6.001 0 0 1-5.482-5.518H3v-1h-.976A6.001 6.001 0 0 1 7.5 2.02V3h1v-.98a6.001 6.001 0 0 1 5.48 5.48H13v1h.98ZM8 15A7 7 0 1 0 8 1a7 7 0 0 0 0 14Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCopy = function VscCopy(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m4 4 1-1h5.414L14 6.586V14l-1 1H5l-1-1V4zm9 3-3-3H5v10h8V7z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3 1 2 2v10l1 1V2h6.414l-1-1H3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscCreditCard = function VscCreditCard(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14 5v1H2V5h12zM2 7h12v5H2V7zm12-3H2a1 1 0 0 0-1 1v7a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V5a1 1 0 0 0-1-1zm-3 6h2v1h-2v-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDash = function VscDash(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M5 8h6v1H5z"
  }));
};

module.exports.VscDashboard = function VscDashboard(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3.889 2.095a6.5 6.5 0 1 1 7.222 10.81A6.5 6.5 0 0 1 3.89 2.094zm.555 9.978A5.5 5.5 0 0 0 7.5 13 5.506 5.506 0 0 0 13 7.5a5.5 5.5 0 1 0-8.556 4.573zM10.294 4l.706.707-2.15 2.15a1.514 1.514 0 1 1-.707-.707L10.293 4zM7.221 7.916a.5.5 0 1 0 .556-.832.5.5 0 0 0-.556.832zm4.286-2.449-.763.763c.166.403.253.834.255 1.27a3.463 3.463 0 0 1-.5 1.777l.735.735a4.477 4.477 0 0 0 .274-4.545h-.001zM8.733 4.242A3.373 3.373 0 0 0 7.5 4 3.5 3.5 0 0 0 4 7.5a3.46 3.46 0 0 0 .5 1.777l-.734.735A4.5 4.5 0 0 1 9.5 3.473l-.767.769z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDatabase = function VscDatabase(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13 3.5C13 2.119 10.761 1 8 1S3 2.119 3 3.5c0 .04.02.077.024.117H3v8.872l.056.357C3.336 14.056 5.429 15 8 15c2.571 0 4.664-.944 4.944-2.154l.056-.357V3.617h-.024c.004-.04.024-.077.024-.117zM8 2.032c2.442 0 4 .964 4 1.468s-1.558 1.468-4 1.468S4 4 4 3.5s1.558-1.468 4-1.468zm4 10.458-.03.131C11.855 13.116 10.431 14 8 14s-3.855-.884-3.97-1.379L4 12.49v-7.5A7.414 7.414 0 0 0 8 6a7.414 7.414 0 0 0 4-1.014v7.504z"
  }));
};

module.exports.VscDebugAll = function VscDebugAll(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m7.293 9.006-.88.88A2.484 2.484 0 0 0 4 8a2.488 2.488 0 0 0-2.413 1.886l-.88-.88L0 9.712l1.147 1.146-.147.146v1H0v.999h1v.053c.051.326.143.643.273.946L0 15.294.707 16l1.1-1.099A2.873 2.873 0 0 0 4 16a2.875 2.875 0 0 0 2.193-1.099L7.293 16 8 15.294l-1.273-1.292A3.92 3.92 0 0 0 7 13.036v-.067h1v-.965H7v-1l-.147-.146L8 9.712l-.707-.706ZM4 9.006a1.5 1.5 0 0 1 1.5 1.499h-3A1.498 1.498 0 0 1 4 9.006Zm2 3.997A2.217 2.217 0 0 1 4 15a2.22 2.22 0 0 1-2-1.998v-1.499h4v1.499Z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3.78 2 3 2.41V7h1V3.35l7.6 5.07L9 10.15v1.2l3.78-2.52V8l-9-6ZM9 13.35v-1.202l5.6-3.728L7 3.35V2.147L15.78 8v.83L9 13.35Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugAltSmall = function VscDebugAltSmall(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m7.293 9.006-.88.88A2.484 2.484 0 0 0 4 8a2.488 2.488 0 0 0-2.413 1.886l-.88-.88L0 9.712l1.147 1.146-.147.146v1H0v.999h1v.053c.051.326.143.643.273.946L0 15.294.707 16l1.1-1.099A2.873 2.873 0 0 0 4 16a2.875 2.875 0 0 0 2.193-1.099L7.293 16 8 15.294l-1.273-1.292A3.92 3.92 0 0 0 7 13.036v-.067h1v-.965H7v-1l-.147-.146L8 9.712l-.707-.706zM4 9.006a1.5 1.5 0 0 1 1.5 1.499h-3A1.498 1.498 0 0 1 4 9.006zm2 3.997A2.217 2.217 0 0 1 4 15a2.22 2.22 0 0 1-2-1.998v-1.499h4v1.499z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5 2.41 5.78 2l9 6v.83L9 12.683v-1.2l4.6-3.063L6 3.35V7H5V2.41z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugAlt = function VscDebugAlt(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    d: "m10.94 13.5-1.32 1.32a3.73 3.73 0 0 0-7.24 0L1.06 13.5 0 14.56l1.72 1.72-.22.22V18H0v1.5h1.5v.08c.077.489.214.966.41 1.42L0 22.94 1.06 24l1.65-1.65A4.308 4.308 0 0 0 6 24a4.31 4.31 0 0 0 3.29-1.65L10.94 24 12 22.94 10.09 21c.198-.464.336-.951.41-1.45v-.1H12V18h-1.5v-1.5l-.22-.22L12 14.56l-1.06-1.06zM6 13.5a2.25 2.25 0 0 1 2.25 2.25h-4.5A2.25 2.25 0 0 1 6 13.5zm3 6a3.33 3.33 0 0 1-3 3 3.33 3.33 0 0 1-3-3v-2.25h6v2.25zm14.76-9.9v1.26L13.5 17.37V15.6l8.5-5.37L9 2v9.46a5.07 5.07 0 0 0-1.5-.72V.63L8.64 0l15.12 9.6z"
  }));
};

module.exports.VscDebugBreakpointConditionalUnverified = function VscDebugBreakpointConditionalUnverified(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5.778 4.674a4 4 0 1 1 4.444 6.652 4 4 0 0 1-4.444-6.652zm.694 5.612a2.75 2.75 0 1 0 3.056-4.572 2.75 2.75 0 0 0-3.056 4.572zM9.5 6.5h-3v1h3v-1zm0 2h-3v1h3v-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugBreakpointConditional = function VscDebugBreakpointConditional(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 4a4 4 0 1 0 0 8 4 4 0 0 0 0-8zm2 5v1H6V9h4zm0-3v1H6V6h4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugBreakpointDataUnverified = function VscDebugBreakpointDataUnverified(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M9.931 4h-4.62l-2.31 4 2.31 4h4.62l2.31-4-2.31-4zm-.75 6.7h-3.12L4.501 8l1.56-2.7h3.12l1.56 2.7-1.56 2.7z"
  }));
};

module.exports.VscDebugBreakpointData = function VscDebugBreakpointData(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m12.238 8-2.31 4H5.31L3 8l2.31-4h4.618l2.31 4z"
  }));
};

module.exports.VscDebugBreakpointFunctionUnverified = function VscDebugBreakpointFunctionUnverified(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4 11h8L8 4l-4 7zm2.154-1.25h3.692L8 6.52 6.154 9.75z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugBreakpointFunction = function VscDebugBreakpointFunction(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m8 4 4 6.905H4L8 4z"
  }));
};

module.exports.VscDebugBreakpointLogUnverified = function VscDebugBreakpointLogUnverified(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3.02 7.98 8 3l4.98 4.98L8 12.96 3.02 7.98zM8 10.77l2.79-2.79L8 5.19 5.21 7.98 8 10.77z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugBreakpointLog = function VscDebugBreakpointLog(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m8 3 5 5-5 5-5-5 5-5z"
  }));
};

module.exports.VscDebugBreakpointUnsupported = function VscDebugBreakpointUnsupported(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.326 10.222a4 4 0 1 0-6.653-4.444 4 4 0 0 0 6.653 4.444zM8.65 10H7.4v1h1.25v-1zM7.4 9V5h1.25v4H7.4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugConsole = function VscDebugConsole(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m7.04 1.361.139-.057H21.32l.14.057 1.178 1.179.057.139V16.82l-.057.14-1.179 1.178-.139.057H14V18a1.99 1.99 0 0 0-.548-1.375h7.673V2.875H7.375v7.282a5.73 5.73 0 0 0-1.571-.164V2.679l.057-.14L7.04 1.362zm9.531 9.452-2.809 2.8a2 2 0 0 0-.348-.467l-.419-.42 2.236-2.235-3.606-3.694.813-.833 4.133 4.133v.716zM9.62 14.82l1.32-1.32L12 14.56l-1.72 1.72.22.22V18H12v1.45h-1.5v.1a5.888 5.888 0 0 1-.41 1.45L12 22.94 10.94 24l-1.65-1.65A4.308 4.308 0 0 1 6 24a4.31 4.31 0 0 1-3.29-1.65L1.06 24 0 22.94 1.91 21a5.889 5.889 0 0 1-.41-1.42v-.08H0V18h1.5v-1.5l.22-.22L0 14.56l1.06-1.06 1.32 1.32a3.73 3.73 0 0 1 7.24 0zm-2.029-.661A2.25 2.25 0 0 0 3.75 15.75h4.5a2.25 2.25 0 0 0-.659-1.591zm.449 7.38A3.33 3.33 0 0 0 9 19.5v-2.25H3v2.25a3.33 3.33 0 0 0 3 3 3.33 3.33 0 0 0 2.04-.96z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugContinueSmall = function VscDebugContinueSmall(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4 2H3v12h1V2Zm3.29.593L6.5 3v10l.79.407 7-5v-.814l-7-5ZM13.14 8 7.5 12.028V3.972L13.14 8Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugContinue = function VscDebugContinue(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2.5 2H4v12H2.5V2zm4.936.39L6.25 3v10l1.186.61 7-5V7.39l-7-5zM12.71 8l-4.96 3.543V4.457L12.71 8z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugCoverage = function VscDebugCoverage(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5 2.41 5.78 2l9 6v.83L9 12.683v-1.2l4.6-3.063L6 3.35V7H5V2.41Z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M6.13 12.124c-.182.18-.322.379-.42.6-.097.219-.145.457-.145.715v.862a.685.685 0 0 1-.205.495.703.703 0 0 1-.496.204h-.865a.691.691 0 0 1-.497-.204.701.701 0 0 1-.205-.495v-.862c0-.258-.049-.496-.147-.716a1.913 1.913 0 0 0-.418-.6 2.525 2.525 0 0 1-.542-.773 2.255 2.255 0 0 1-.19-.927A2.386 2.386 0 0 1 2.332 9.2a2.404 2.404 0 0 1 .87-.87A2.473 2.473 0 0 1 4.432 8a2.41 2.41 0 0 1 1.225.33 2.41 2.41 0 0 1 1.205 2.093c0 .332-.063.641-.19.927a2.525 2.525 0 0 1-.542.774Zm-1.103.991H3.835v1.186c0 .043.016.08.049.114.033.033.07.048.115.048h.865a.156.156 0 0 0 .114-.048.154.154 0 0 0 .049-.114v-1.186Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugDisconnect = function VscDebugDisconnect(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13.617 3.844a2.87 2.87 0 0 0-.451-.868l1.354-1.36L13.904 1l-1.36 1.354a2.877 2.877 0 0 0-.868-.452 3.073 3.073 0 0 0-2.14.075 3.03 3.03 0 0 0-.991.664L7 4.192l4.327 4.328 1.552-1.545c.287-.287.508-.618.663-.992a3.074 3.074 0 0 0 .075-2.14zm-.889 1.804a2.15 2.15 0 0 1-.471.705l-.93.93-3.09-3.09.93-.93a2.15 2.15 0 0 1 .704-.472 2.134 2.134 0 0 1 1.689.007c.264.114.494.271.69.472.2.195.358.426.472.69a2.134 2.134 0 0 1 .007 1.688zm-4.824 4.994 1.484-1.545-.616-.622-1.49 1.551-1.86-1.859 1.491-1.552L6.291 6 4.808 7.545l-.616-.615-1.551 1.545a3 3 0 0 0-.663.998 3.023 3.023 0 0 0-.233 1.169c0 .332.05.656.15.97.105.31.258.597.459.862L1 13.834l.615.615 1.36-1.353c.265.2.552.353.862.458.314.1.638.15.97.15.406 0 .796-.077 1.17-.232.378-.155.71-.376.998-.663l1.545-1.552-.616-.615zm-2.262 2.023a2.16 2.16 0 0 1-.834.164c-.301 0-.586-.057-.855-.17a2.278 2.278 0 0 1-.697-.466 2.28 2.28 0 0 1-.465-.697 2.167 2.167 0 0 1-.17-.854 2.16 2.16 0 0 1 .642-1.545l.93-.93 3.09 3.09-.93.93a2.22 2.22 0 0 1-.711.478z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugLineByLine = function VscDebugLineByLine(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M6 10V9h9v1H6zm4-4h5v1h-5V6zm5-3v1H6V3h9zm-9 9v1h9v-1H6z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m1 2.795.783-.419 5.371 3.581v.838l-5.371 3.581L1 9.957V2.795zm1.007.94v5.281l3.96-2.64-3.96-2.64z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugPause = function VscDebugPause(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M4.5 3H6v10H4.5V3zm7 0v10H10V3h1.5z"
  }));
};

module.exports.VscDebugRerun = function VscDebugRerun(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.167 12a3 3 0 0 1-5.74 1.223l-.928.376A4.001 4.001 0 1 0 1 9.556V8.333H0V11l.5.5h2.333v-1H1.568A3 3 0 0 1 7.167 12z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5 2.41 5.78 2l9 6v.83L10 12.017v-1.2l3.6-2.397L6 3.35V7H5V2.41z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugRestartFrame = function VscDebugRestartFrame(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1 10V9h5.207a5.48 5.48 0 0 0-.185 1H1zm6.257-3a5.54 5.54 0 0 1 1.08-1H1v1h6.257zM6.6 13a5.465 5.465 0 0 1-.393-1H1v1h5.6zM15 3v1H1V3h14zm-3.36 10.031a2.531 2.531 0 1 0-2.192-3.797h1.068v.844h-1.97l-.421-.422v-2.25h.844v1.032a3.375 3.375 0 1 1-.423 3.412l.782-.318a2.532 2.532 0 0 0 2.313 1.5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugRestart = function VscDebugRestart(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M12.75 8a4.5 4.5 0 0 1-8.61 1.834l-1.391.565A6.001 6.001 0 0 0 14.25 8 6 6 0 0 0 3.5 4.334V2.5H2v4l.75.75h3.5v-1.5H4.352A4.5 4.5 0 0 1 12.75 8z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugReverseContinue = function VscDebugReverseContinue(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13.5 2H12v12h1.5V2zm-4.936.39L9.75 3v10l-1.186.61-7-5V7.39l7-5zM3.29 8l4.96 3.543V4.457L3.29 8z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugStackframeActive = function VscDebugStackframeActive(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M10 8a2 2 0 1 1-4 0 2 2 0 0 1 4 0z"
  }), React.createElement("path", {
    d: "m14.5 7.15-4.26-4.74L9.31 2H4.25L3 3.25v9.48l1.25 1.25h5.06l.93-.42 4.26-4.74V7.15zm-5.19 5.58H4.25V3.25h5.06l4.26 4.73-4.26 4.75z"
  }));
};

module.exports.VscDebugStackframeDot = function VscDebugStackframeDot(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M10 8a2 2 0 1 1-4 0 2 2 0 0 1 4 0z"
  }));
};

module.exports.VscDebugStackframe = function VscDebugStackframe(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m14.5 7.15-4.26-4.74L9.31 2H4.25L3 3.25v9.48l1.25 1.25h5.06l.93-.42 4.26-4.74V7.15zm-5.19 5.58H4.25V3.25h5.06l4.26 4.73-4.26 4.75z"
  }));
};

module.exports.VscDebugStart = function VscDebugStart(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m4.25 3 1.166-.624 8 5.333v1.248l-8 5.334-1.166-.624V3zm1.5 1.401v7.864l5.898-3.932L5.75 4.401z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugStepBack = function VscDebugStepBack(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.75 5.75v-4h1.5v2.542c1.145-1.359 2.911-2.209 4.84-2.209 3.177 0 5.92 2.307 6.16 5.398l.02.269h-1.5l-.022-.226c-.212-2.195-2.202-3.94-4.656-3.94-1.736 0-3.244.875-4.05 2.166h2.83v1.5H2.707l-.961-.975V5.75h.003zM8 14a2 2 0 1 1 0-4 2 2 0 0 1 0 4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugStepInto = function VscDebugStepInto(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 9.532h.542l3.905-3.905-1.061-1.06-2.637 2.61V1H7.251v6.177l-2.637-2.61-1.061 1.06 3.905 3.905H8zm1.956 3.481a2 2 0 1 1-4 0 2 2 0 0 1 4 0z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugStepOut = function VscDebugStepOut(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 1h-.542L3.553 4.905l1.061 1.06 2.637-2.61v6.177h1.498V3.355l2.637 2.61 1.061-1.06L8.542 1H8zm1.956 12.013a2 2 0 1 1-4 0 2 2 0 0 1 4 0z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugStepOver = function VscDebugStepOver(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14.25 5.75v-4h-1.5v2.542c-1.145-1.359-2.911-2.209-4.84-2.209-3.177 0-5.92 2.307-6.16 5.398l-.02.269h1.501l.022-.226c.212-2.195 2.202-3.94 4.656-3.94 1.736 0 3.244.875 4.05 2.166h-2.83v1.5h4.163l.962-.975V5.75h-.004zM8 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDebugStop = function VscDebugStop(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M2 2v12h12V2H2zm10.75 10.75h-9.5v-9.5h9.5v9.5z"
  }));
};

module.exports.VscDebug = function VscDebug(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    d: "m3.463 12.86-.005-.07.005.07zm7.264.69-3.034-3.049 1.014-1.014 3.209 3.225 3.163-3.163 1.014 1.014-3.034 3.034 3.034 3.05-1.014 1.014-3.209-3.225L8.707 17.6l-1.014-1.014 3.034-3.034z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M16.933 5.003V6h1.345l2.843-2.842 1.014 1.014-2.692 2.691.033.085a13.75 13.75 0 0 1 .885 4.912c0 .335-.011.667-.034.995l-.005.075h3.54v1.434h-3.72l-.01.058c-.303 1.653-.891 3.16-1.692 4.429l-.06.094 3.423 3.44-1.017 1.012-3.274-3.29-.099.11c-1.479 1.654-3.395 2.646-5.483 2.646-2.12 0-4.063-1.023-5.552-2.723l-.098-.113-3.209 3.208-1.014-1.014 3.366-3.365-.059-.095c-.772-1.25-1.34-2.725-1.636-4.34l-.01-.057H0V12.93h3.538l-.005-.075a14.23 14.23 0 0 1-.034-.995c0-1.743.31-3.39.863-4.854l.032-.084-2.762-2.776L2.65 3.135 5.5 6h1.427v-.997a5.003 5.003 0 0 1 10.006 0zm-8.572 0V6H15.5v-.997a3.569 3.569 0 0 0-7.138 0zm9.8 2.522-.034-.09H5.733l-.034.09a12.328 12.328 0 0 0-.766 4.335c0 2.76.862 5.201 2.184 6.92 1.32 1.716 3.036 2.649 4.813 2.649 1.777 0 3.492-.933 4.813-2.65 1.322-1.718 2.184-4.16 2.184-6.919 0-1.574-.28-3.044-.766-4.335z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDesktopDownload = function VscDesktopDownload(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4 15v-1c2 0 2-.6 2-1H1.5l-.5-.5v-10l.5-.5h13l.5.5v9.24l-1-1V3H2v9h5.73l-.5.5 2.5 2.5H4zm7.86 0 2.5-2.5-.71-.7L12 13.45V7h-1v6.44l-1.64-1.65-.71.71 2.5 2.5h.71z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDeviceCameraVideo = function VscDeviceCameraVideo(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14.25 4.74 11 6.62V4.5l-.5-.5h-9l-.5.5v7l.5.5h9l.5-.5v-2l3.25 1.87.75-.47V5.18l-.75-.44zM10 11H2V5h8v6zm4-1-3-1.7v-.52L14 6v4z"
  }));
};

module.exports.VscDeviceCamera = function VscDeviceCamera(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M10.707 3H14.5l.5.5v9l-.5.5h-13l-.5-.5v-9l.5-.5h3.793l.853-.854L6.5 2h3l.354.146.853.854zM2 12h12V4h-3.5l-.354-.146L9.293 3H6.707l-.853.854L5.5 4H2v8zm1.5-7a.5.5 0 1 0 0 1 .5.5 0 0 0 0-1zM8 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0-1a3 3 0 1 0 0 6 3 3 0 0 0 0-6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDeviceMobile = function VscDeviceMobile(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4.5 1h8l.5.5v13l-.5.5h-8l-.5-.5v-13l.5-.5zM5 14h7V2H5v12zm2.5-2h2v1h-2v-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDiffAdded = function VscDiffAdded(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 1h12l.5.5v12l-.5.5h-12l-.5-.5v-12l.5-.5zM2 13h11V2H2v11z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 4H7v3H4v1h3v3h1V8h3V7H8V4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDiffIgnored = function VscDiffIgnored(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 1h13l.5.5v13l-.5.5h-13l-.5-.5v-13l.5-.5zM2 14h12V2H2v12zm8-10h2v2l-6 6H4v-2l6-6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDiffModified = function VscDiffModified(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 1h13l.5.5v13l-.5.5h-13l-.5-.5v-13l.5-.5zM2 2v12h12V2H2zm6 9a3 3 0 1 0 0-6 3 3 0 0 0 0 6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDiffRemoved = function VscDiffRemoved(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M10 7v1H5V7h5z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 1h12l.5.5v12l-.5.5h-12l-.5-.5v-12l.5-.5zM2 13h11V2H2v11z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDiffRenamed = function VscDiffRenamed(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 1h13l.5.5v13l-.5.5h-13l-.5-.5v-13l.5-.5zM2 14h12V2H2v12zm2-5h3v3l5-4-5-4v3H4v2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDiff = function VscDiff(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m2 3.5.5-.5h5l.5.5v9l-.5.5h-5l-.5-.5v-9zM3 12h4V6H3v6zm0-7h4V4H3v1zm6.5-2h5l.5.5v9l-.5.5h-5l-.5-.5v-9l.5-.5zm.5 9h4v-2h-4v2zm0-4h4V4h-4v4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscDiscard = function VscDiscard(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3.5 2v3.5L4 6h3.5V5H4.979l.941-.941a3.552 3.552 0 1 1 5.023 5.023L5.746 14.28l.72.72 5.198-5.198A4.57 4.57 0 0 0 5.2 3.339l-.7.7V2h-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscEdit = function VscEdit(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13.23 1h-1.46L3.52 9.25l-.16.22L1 13.59 2.41 15l4.12-2.36.22-.16L15 4.23V2.77L13.23 1zM2.41 13.59l1.51-3 1.45 1.45-2.96 1.55zm3.83-2.06L4.47 9.76l8-8 1.77 1.77-8 8z"
  }));
};

module.exports.VscEditorLayout = function VscEditorLayout(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m15 6.5-.47-.5H7V1.47L6.53 1H1.47L1 1.47v8.06l.47.47H4v4.53l.47.47h10.06l.47-.47V6.5zM2 9V3h4v6H2zm12 5H5v-4h1.53L7 9.53V8.013h7V14z"
  }));
};

module.exports.VscEllipsis = function VscEllipsis(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M4 8a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm5 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm5 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0z"
  }));
};

module.exports.VscEmptyWindow = function VscEmptyWindow(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4 4h3v1H4v3H3V5H0V4h3V1h1v3zM1 14.5V9h1v5h12V7H8V6h6V4H8V3h6.5l.5.5v11l-.5.5h-13l-.5-.5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscError = function VscError(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8.6 1c1.6.1 3.1.9 4.2 2 1.3 1.4 2 3.1 2 5.1 0 1.6-.6 3.1-1.6 4.4-1 1.2-2.4 2.1-4 2.4-1.6.3-3.2.1-4.6-.7-1.4-.8-2.5-2-3.1-3.5C.9 9.2.8 7.5 1.3 6c.5-1.6 1.4-2.9 2.8-3.8C5.4 1.3 7 .9 8.6 1zm.5 12.9c1.3-.3 2.5-1 3.4-2.1.8-1.1 1.3-2.4 1.2-3.8 0-1.6-.6-3.2-1.7-4.3-1-1-2.2-1.6-3.6-1.7-1.3-.1-2.7.2-3.8 1-1.1.8-1.9 1.9-2.3 3.3-.4 1.3-.4 2.7.2 4 .6 1.3 1.5 2.3 2.7 3 1.2.7 2.6.9 3.9.6zM7.9 7.5 10.3 5l.7.7-2.4 2.5 2.4 2.5-.7.7-2.4-2.5-2.4 2.5-.7-.7 2.4-2.5-2.4-2.5.7-.7 2.4 2.5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscExclude = function VscExclude(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M9.412 1H6.588l-.484 2.423-2.056-1.37-1.996 1.995 1.37 2.056L1 6.588v2.824l2.423.484-1.37 2.056 1.995 1.996 2.056-1.37L6.588 15h2.083a4.526 4.526 0 0 1-.917-1.005h-.342l-.288-1.441a4.473 4.473 0 0 1-.067-.334l-.116-.583-.764-.316-2 1.334-.832-.831L4.68 9.823l-.316-.764-2.358-.471V7.412l2.358-.471.316-.764-1.334-2 .831-.832 2 1.335.764-.316.471-2.358h1.176l.471 2.358.764.316 2-1.334.832.831-1.334 2.001.316.764.582.116c.113.018.225.04.335.067l1.441.288v.342c.38.254.719.563 1.005.917V6.588l-2.422-.484 1.37-2.056-1.996-1.996-2.056 1.37L9.412 1zM8 6a2 2 0 0 1 1.875 1.302 4.46 4.46 0 0 0-.9.473 1 1 0 1 0-1.2 1.2 4.46 4.46 0 0 0-.473.9A2 2 0 0 1 8 6zm1.28 2.795a3.5 3.5 0 1 1 4.44 5.41 3.5 3.5 0 0 1-4.44-5.41zM9 11v1h5v-1H9z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscExpandAll = function VscExpandAll(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M9 9H4v1h5V9z"
  }), React.createElement("path", {
    d: "M7 12V7H6v5h1z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m5 3 1-1h7l1 1v7l-1 1h-2v2l-1 1H3l-1-1V6l1-1h2V3zm1 2h4l1 1v4h2V3H6v2zm4 1H3v7h7V6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscExport = function VscExport(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m13.086 7-2.39-2.398.702-.704L15 7.5l-3.602 3.602-.703-.704 2.383-2.382V8H3V7h10.086zM1 4h1v7H1V4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscExtensions = function VscExtensions(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13.5 1.5 15 0h7.5L24 1.5V9l-1.5 1.5H15L13.5 9V1.5zm1.5 0V9h7.5V1.5H15zM0 15V6l1.5-1.5H9L10.5 6v7.5H18l1.5 1.5v7.5L18 24H1.5L0 22.5V15zm9-1.5V6H1.5v7.5H9zM9 15H1.5v7.5H9V15zm1.5 7.5H18V15h-7.5v7.5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscEyeClosed = function VscEyeClosed(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 2c-1.5 0-2.8.4-3.9 1.2l.8.7C5.8 3.3 6.8 3 8 3c3.3 0 6 2.7 6 6h1c0-3.9-3.1-7-7-7zM1 3l1.6 1.5C1.6 5.7 1 7.3 1 9h1c0-1.5.5-2.8 1.4-3.8l2.2 2C5.2 7.7 5 8.3 5 9c0 1.7 1.3 3 3 3 .8 0 1.5-.3 2-.8l3 2.8.7-.7-12-11L1 3zm5.3 4.9 2.9 2.7c-.3.2-.7.4-1.2.4-1.1 0-2-.9-2-2 0-.4.1-.8.3-1.1zM11 9.5l-1-.9c-.2-.8-.9-1.5-1.8-1.6l-1-.9c.3-.1.5-.1.8-.1 1.7 0 3 1.3 3 3v.5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscEye = function VscEye(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1 10c0-3.9 3.1-7 7-7s7 3.1 7 7h-1c0-3.3-2.7-6-6-6s-6 2.7-6 6H1zm4 0c0-1.7 1.3-3 3-3s3 1.3 3 3-1.3 3-3 3-3-1.3-3-3zm1 0c0 1.1.9 2 2 2s2-.9 2-2-.9-2-2-2-2 .9-2 2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFeedback = function VscFeedback(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M7.549 10.078c.46.182.88.424 1.258.725.378.3.701.65.97 1.046a4.829 4.829 0 0 1 .848 2.714V15H9.75v-.438a3.894 3.894 0 0 0-1.155-2.782 4.054 4.054 0 0 0-1.251-.84 3.898 3.898 0 0 0-1.532-.315A3.894 3.894 0 0 0 3.03 11.78a4.06 4.06 0 0 0-.84 1.251c-.206.474-.31.985-.315 1.531V15H1v-.438a4.724 4.724 0 0 1 .848-2.713 4.918 4.918 0 0 1 2.229-1.77 2.994 2.994 0 0 1-.555-.493 3.156 3.156 0 0 1-.417-.602 2.942 2.942 0 0 1-.26-.683 3.345 3.345 0 0 1-.095-.739c0-.423.08-.82.24-1.189a3.095 3.095 0 0 1 1.626-1.627 3.067 3.067 0 0 1 2.386-.007 3.095 3.095 0 0 1 1.627 1.627 3.067 3.067 0 0 1 .157 1.928c-.06.237-.148.465-.266.684a3.506 3.506 0 0 1-.417.608c-.16.187-.345.35-.554.492zM5.812 9.75c.301 0 .584-.057.848-.17a2.194 2.194 0 0 0 1.162-1.163c.119-.269.178-.554.178-.854a2.138 2.138 0 0 0-.643-1.538 2.383 2.383 0 0 0-.697-.472 2.048 2.048 0 0 0-.848-.178c-.3 0-.583.057-.847.17a2.218 2.218 0 0 0-1.17 1.17c-.113.264-.17.547-.17.848 0 .3.057.583.17.847.115.264.27.497.466.697a2.168 2.168 0 0 0 1.552.643zM15 1v7h-1.75l-2.625 2.625V8H9.75v-.875h1.75v1.388l1.388-1.388h1.237v-5.25h-8.75v1.572a7.255 7.255 0 0 0-.438.069 2.62 2.62 0 0 0-.437.123V1H15z"
  }));
};

module.exports.VscFileBinary = function VscFileBinary(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m10.57 1.14 3.28 3.3.15.36v9.7l-.5.5h-11l-.5-.5v-13l.5-.5h7.72l.35.14zM3 2v12h10V5l-3-3H3zm1.46 4.052c0 1.287.458 1.93 1.374 1.93.457 0 .807-.173 1.05-.52.246-.348.368-.847.368-1.499C7.252 4.654 6.805 4 5.91 4c-.471 0-.831.175-1.08.526-.247.35-.37.858-.37 1.526zm.862-.022c0-.922.183-1.383.55-1.383.344 0 .516.448.516 1.343s-.176 1.343-.527 1.343c-.36 0-.54-.434-.54-1.303zm3.187 1.886h2.435v-.672h-.792V4l-1.665.336v.687l.82-.177v2.398h-.798v.672zm-1.337 5H4.736v-.672h.798V9.846l-.82.177v-.687L6.38 9v3.244h.792v.671zm1.035-1.931c0 1.287.458 1.93 1.375 1.93.457 0 .807-.173 1.05-.52.245-.348.368-.847.368-1.499 0-1.309-.448-1.963-1.343-1.963-.47 0-.83.175-1.08.526-.246.35-.37.858-.37 1.526zm.862-.022c0-.922.184-1.383.55-1.383.344 0 .516.448.516 1.343s-.175 1.343-.526 1.343c-.36 0-.54-.434-.54-1.303z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFileCode = function VscFileCode(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m10.57 1.14 3.28 3.3.15.36v9.7l-.5.5h-11l-.5-.5v-13l.5-.5h7.72l.35.14zM10 5h3l-3-3v3zM3 2v12h10V6H9.5L9 5.5V2H3zm2.062 7.533 1.817-1.828L6.17 7 4 9.179v.707l2.171 2.174.707-.707-1.816-1.82zM8.8 7.714l.7-.709 2.189 2.175v.709L9.5 12.062l-.705-.709 1.831-1.82L8.8 7.714z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFileMedia = function VscFileMedia(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2 2h6v3.5l.5.5H12v1h1V4.8l-.15-.36-3.28-3.3L9.22 1H1.5l-.5.5v13l.5.5H5v-1H2V2zm7 0 3 3H9V2zm5.5 6h-8l-.5.5v6l.5.5h8l.5-.5v-6l-.5-.5zM14 9v4l-1.63-1.6h-.71l-1.16 1.17-2.13-2.13h-.71L7 11.1V9h7zm-2.8 4.27.81-.81L13.55 14h-1.62l-.73-.73zM7 14v-1.49l1-1L10.52 14H7zm5.5-3.5a.5.5 0 1 0 0-1 .5.5 0 0 0 0 1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFilePdf = function VscFilePdf(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m13.85 4.44-3.28-3.3-.35-.14H2.5l-.5.5V7h1V2h6v3.5l.5.5H13v1h1V4.8l-.15-.36zM10 5V2l3 3h-3zM2.5 8l-.5.5v6l.5.5h11l.5-.5v-6l-.5-.5h-11zM13 13v1H3V9h10v4zm-8-1h-.32v1H4v-3h1.06c.75 0 1.13.36 1.13 1a.94.94 0 0 1-.32.72A1.33 1.33 0 0 1 5 12zm-.06-1.45h-.26v.93h.26c.36 0 .54-.16.54-.47 0-.31-.18-.46-.54-.46zM9 12.58a1.48 1.48 0 0 0 .44-1.12c0-1-.53-1.46-1.6-1.46H6.78v3h1.06A1.6 1.6 0 0 0 9 12.58zm-1.55-.13v-1.9h.33a.94.94 0 0 1 .7.25.91.91 0 0 1 .25.67 1 1 0 0 1-.25.72.94.94 0 0 1-.69.26h-.34zm4.45-.61h-.97V13h-.68v-3h1.74v.55h-1.06v.74h.97v.55z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFileSubmodule = function VscFileSubmodule(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2 11h1V6.99H2V11zm1-5.01V5.5l.5-.5h4.43l.43.25.43.75h5.71l.5.5v8l-.5.5h-11l-.5-.5V12H1.5l-.5-.5v-9l.5-.5h4.42l.44.25.43.75h5.71l.5.5V6l-1-.03V4H6.5l-.43-.25L5.64 3H2v2.99h1zm5.07.76L7.64 6H4v3h3.15l.41-.74L8 8h6V7H8.5l-.43-.25zM7.45 10H4v4h10V9H8.3l-.41.74-.44.26z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFileSymlinkDirectory = function VscFileSymlinkDirectory(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.71 3h6.79l.51.5v10l-.5.5h-13l-.5-.5v-11l.5-.5h5l.35.15.85.85zm6.28 10v-1.51l.01-4v-1.5H7.7l-.86.86-.35.15H2v6h11.99zm-6.5-8h6.5l.01-.99H7.5l-.36-.15-.85-.85H2v3h4.28l.86-.86.35-.15zm2.29 4.07L8.42 7.7l.74-.69 2.22 2.22v.71l-2.29 2.21-.7-.72 1.4-1.35H8.42a2 2 0 0 0-1.35.61A1.8 1.8 0 0 0 6.54 12h-1a2.76 2.76 0 0 1 .81-2 3 3 0 0 1 2-.93h1.43z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFileSymlinkFile = function VscFileSymlinkFile(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m10.57 1.14 3.28 3.3.15.36v9.7l-.5.5H10v-1h3V6H9.5L9 5.5V2H3v4H2V1.5l.5-.5h7.72l.35.14zM10 5h3l-3-3v3zM8.5 7h-7l-.5.5v7l.5.5h7l.5-.5v-7L8.5 7zM8 14H2V8h6v6zM7 9.5v3H6v-1.793l-2.646 2.647-.708-.708L5.293 10H3.53V9H6.5l.5.5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFileZip = function VscFileZip(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2.5 1h11l.5.5v5l-.15.35-.85.86v6.79l-.5.5h-10l-.5-.5v-13l.5-.5zM6 2H5v2h1V2zm0 12h4V7.68l-.85-.85L9 6.47V2H7v2.5l-.5.5H6v1H5V5h-.5L4 4.5V2H3v12h2v-1h1v1zm0-2v1h1v-1H6zm0-1v1H5v-1h1zm0-1h1v1H6v-1zm0-1v1H5V9h1zm0-1h1v1H6V8zm0-1v1H5V7h1zm0 0h1V6H6v1zm6.15.15.85-.86V2h-3v4.27l.85.85.15.35V14h1V7.5l.15-.35z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFile = function VscFile(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m13.71 4.29-3-3L10 1H4L3 2v12l1 1h9l1-1V5l-.29-.71zM13 14H4V2h5v4h4v8zm-3-9V2l3 3h-3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFiles = function VscFiles(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    d: "M17.5 0h-9L7 1.5V6H2.5L1 7.5v15.07L2.5 24h12.07L16 22.57V18h4.7l1.3-1.43V4.5L17.5 0zm0 2.12 2.38 2.38H17.5V2.12zm-3 20.38h-12v-15H7v9.07L8.5 18h6v4.5zm6-6h-12v-15H16V6h4.5v10.5z"
  }));
};

module.exports.VscFilterFilled = function VscFilterFilled(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M15 2v1.67l-5 4.759V14H6V8.429l-5-4.76V2h14z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFilter = function VscFilter(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M15 2v1.67l-5 4.759V14H6V8.429l-5-4.76V2h14zM7 8v5h2V8l5-4.76V3H2v.24L7 8z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFlame = function VscFlame(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m9.13 15-.53-.77a1.85 1.85 0 0 0-.28-2.54 3.51 3.51 0 0 1-1.19-2c-1.56 2.23-.75 3.46 0 4.55l-.55.76A4.4 4.4 0 0 1 3 10.46S2.79 8.3 5.28 6.19c0 0 2.82-2.61 1.84-4.54L7.83 1a6.57 6.57 0 0 1 2.61 6.94 2.57 2.57 0 0 0 .56-.81l.87-.07c.07.12 1.84 2.93.89 5.3A4.72 4.72 0 0 1 9.13 15zm-2-6.95.87.39a3 3 0 0 0 .92 2.48 2.64 2.64 0 0 1 1 2.8A3.241 3.241 0 0 0 11.8 12a4.87 4.87 0 0 0-.41-3.63 1.85 1.85 0 0 1-1.84.86l-.35-.68a5.31 5.31 0 0 0-.89-5.8C8.17 4.87 6 6.83 5.93 6.94 3.86 8.7 4 10.33 4 10.4a3.47 3.47 0 0 0 1.59 3.14C5 12.14 5 10.46 7.16 8.05h-.03z"
  }));
};

module.exports.VscFoldDown = function VscFoldDown(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14.207 1.707 13.5 1l-6 6-6-6-.707.707 6.353 6.354h.708l6.353-6.354zm0 6L13.5 7l-6 6-6-6-.707.707 6.353 6.354h.708l6.353-6.354z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFoldUp = function VscFoldUp(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m1 7.4.7.7 6-6 6 6 .7-.7L8.1 1h-.7L1 7.4zm0 6 .7.7 6-6 6 6 .7-.7L8.1 7h-.7L1 13.4z"
  }));
};

module.exports.VscFold = function VscFold(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.952 2.381 7.976 6.357 4 2.381 3.38 3l4.286 4.285h.619L12.57 3l-.618-.619zM3.904 14l4.072-4.072L12.047 14l.62-.619L8.284 9h-.619l-4.381 4.381.619.619z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFolderActive = function VscFolderActive(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.71 3h6.79l.51.5v4.507A4.997 4.997 0 0 0 14 7.416V5.99H7.69l-.86.86-.35.15H1.99v6H7.1c.07.348.177.682.316 1H1.51l-.5-.5v-11l.5-.5h5l.35.15.85.85zm-.22 2h6.5l.01-.99H7.5l-.36-.15-.85-.85H2v3h4.28l.86-.86.35-.15z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M9.778 8.674a4 4 0 1 1 4.444 6.652 4 4 0 0 1-4.444-6.652zm2.13 4.99 2.387-3.182-.8-.6-2.077 2.769-1.301-1.041-.625.78 1.704 1.364.713-.09z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscFolderLibrary = function VscFolderLibrary(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.71 3h6.79l.51.5V7H14V5.99H7.69l-.86.86-.35.15H1.99v6H7v1H1.51l-.5-.5v-11l.5-.5h5l.35.15.85.85Zm-.22 2h6.5l.01-.99H7.5l-.36-.15-.85-.85H2v3h4.28l.86-.86.35-.15Z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    d: "M8 8H9V14H8z"
  }), React.createElement("path", {
    d: "M10 8H11V14H10z"
  }), React.createElement("path", {
    d: "M12.004 8.352H13.004V14.352H12.004z",
    transform: "rotate(-20 12.004 8.352)"
  }));
};

module.exports.VscFolderOpened = function VscFolderOpened(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M1.5 14h11l.48-.37 2.63-7-.48-.63H14V3.5l-.5-.5H7.71l-.86-.85L6.5 2h-5l-.5.5v11l.5.5zM2 3h4.29l.86.85.35.15H13v2H8.5l-.35.15-.86.85H3.5l-.47.34-1 3.08L2 3zm10.13 10H2.19l1.67-5H7.5l.35-.15.86-.85h5.79l-2.37 6z"
  }));
};

module.exports.VscFolder = function VscFolder(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14.5 3H7.71l-.85-.85L6.51 2h-5l-.5.5v11l.5.5h13l.5-.5v-10L14.5 3zm-.51 8.49V13h-12V7h4.49l.35-.15.86-.86H14v1.5l-.01 4zm0-6.49h-6.5l-.35.15-.86.86H2v-3h4.29l.85.85.36.15H14l-.01.99z"
  }));
};

module.exports.VscGear = function VscGear(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M9.1 4.4 8.6 2H7.4l-.5 2.4-.7.3-2-1.3-.9.8 1.3 2-.2.7-2.4.5v1.2l2.4.5.3.8-1.3 2 .8.8 2-1.3.8.3.4 2.3h1.2l.5-2.4.8-.3 2 1.3.8-.8-1.3-2 .3-.8 2.3-.4V7.4l-2.4-.5-.3-.8 1.3-2-.8-.8-2 1.3-.7-.2zM9.4 1l.5 2.4L12 2.1l2 2-1.4 2.1 2.4.4v2.8l-2.4.5L14 12l-2 2-2.1-1.4-.5 2.4H6.6l-.5-2.4L4 13.9l-2-2 1.4-2.1L1 9.4V6.6l2.4-.5L2.1 4l2-2 2.1 1.4.4-2.4h2.8zm.6 7c0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2 2 .9 2 2zM8 9c.6 0 1-.4 1-1s-.4-1-1-1-1 .4-1 1 .4 1 1 1z"
  }));
};

module.exports.VscGift = function VscGift(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13.5 4h-1.6c.1-.4.1-.8.1-1.2-.1-.3-.2-.6-.4-.9-.2-.3-.4-.5-.7-.6-.3-.1-.6-.3-.9-.3-.3 0-.6 0-.9.2-.7.2-1.2.7-1.6 1.3-.4-.6-.9-1.1-1.6-1.3-.3-.1-.6-.2-.9-.2-.3 0-.6.1-.9.3-.3.1-.5.3-.7.6-.2.2-.3.6-.4.9 0 .4 0 .8.1 1.2H1.5l-.5.5v9l.5.5h12l.5-.5v-9l-.5-.5zM7 13H2V5h5v8zm0-9H4v-.2c-.1-.3-.1-.5-.1-.8.1-.2.1-.4.3-.5.1-.2.3-.3.5-.4.1-.1.3-.1.5-.1s.4 0 .6.1c.3.1.6.3.8.6.2.3.4.6.4 1V4zm1-.3c0-.4.2-.7.4-1 .2-.3.5-.5.8-.6.2-.1.4-.1.6-.1.2 0 .4 0 .6.1.2.1.3.2.5.4.1.1.1.3.2.5 0 .3 0 .5-.1.8 0 .1 0 .1-.1.2H8v-.3zm5 9.3H8V5h5v8z"
  }));
};

module.exports.VscGistSecret = function VscGistSecret(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3 14h4v.91l.09.09H2.5l-.5-.5v-13l.5-.5h7.72l.35.14 3.28 3.3.15.36v2.54a3.1 3.1 0 0 0-1-.94V6H9.5L9 5.5V2H3v12zm10-9-3-3v3h3zm.5 4v1h1l.5.5v4l-.5.5h-6l-.5-.5v-4l.5-.5h1V9a2 2 0 0 1 4 0zm-2.707-.707A1 1 0 0 0 10.5 9v1h2V9a1 1 0 0 0-1.707-.707zM9 11v3h5v-3H9z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGist = function VscGist(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m10.57 1.14 3.28 3.3.15.36v9.7l-.5.5h-11l-.5-.5v-13l.5-.5h7.72l.35.14zM10 5h3l-3-3v3zM3 2v12h10V6H9.5L9 5.5V2H3zm2.062 7.533 1.817-1.828L6.17 7 4 9.179v.707l2.171 2.174.707-.707-1.816-1.82zM8.8 7.714l.7-.709 2.189 2.175v.709L9.5 12.062l-.705-.709 1.831-1.82L8.8 7.714z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGitCommit = function VscGitCommit(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M10.979 5a3.42 3.42 0 0 0-2-1V0h-1v4a3.5 3.5 0 0 0-.18 6.89h.18v4h1v-4a3.08 3.08 0 0 0 .84-.23 3.42 3.42 0 0 0 1.57-1.29 3.46 3.46 0 0 0-.41-4.37zm-.7 4.24a2.51 2.51 0 0 1-3.159.31 2.5 2.5 0 0 1-.92-1.12 2.49 2.49 0 0 1 1.82-3.41 2.72 2.72 0 0 1 .49 0 2.5 2.5 0 0 1 2.54 2.5 2.51 2.51 0 0 1-.8 1.75l.03-.03z"
  }));
};

module.exports.VscGitCompare = function VscGitCompare(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m7.389 12.99-1.27-1.27.67-.7 2.13 2.13v.7l-2.13 2.13-.71-.71L7.349 14h-1.85a2.49 2.49 0 0 1-2.5-2.5V5.95a2.59 2.59 0 0 1-1.27-.68 2.52 2.52 0 0 1-.54-2.73A2.5 2.5 0 0 1 3.499 1a2.45 2.45 0 0 1 1 .19 2.48 2.48 0 0 1 1.35 1.35c.133.317.197.658.19 1a2.5 2.5 0 0 1-2 2.45v5.5a1.5 1.5 0 0 0 1.5 1.5h1.85zm-4.68-8.25a1.5 1.5 0 0 0 2.08-2.08 1.55 1.55 0 0 0-.68-.56 1.49 1.49 0 0 0-.86-.08 1.49 1.49 0 0 0-1.18 1.18 1.49 1.49 0 0 0 .08.86c.117.277.311.513.56.68zm10.33 6.3c.48.098.922.335 1.27.68a2.51 2.51 0 0 1 .31 3.159 2.5 2.5 0 1 1-3.47-3.468c.269-.182.571-.308.89-.37V5.49a1.5 1.5 0 0 0-1.5-1.5h-1.85l1.27 1.27-.71.71-2.13-2.13v-.7l2.13-2.13.71.71-1.27 1.27h1.85a2.49 2.49 0 0 1 2.5 2.5v5.55zm-.351 3.943a1.5 1.5 0 0 0 1.1-2.322 1.55 1.55 0 0 0-.68-.56 1.49 1.49 0 0 0-.859-.08 1.49 1.49 0 0 0-1.18 1.18 1.49 1.49 0 0 0 .08.86 1.5 1.5 0 0 0 1.539.922z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGitMerge = function VscGitMerge(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13.273 7.73a2.51 2.51 0 0 0-3.159-.31 2.5 2.5 0 0 0-.921 1.12 2.23 2.23 0 0 0-.13.44 4.52 4.52 0 0 1-4-4 2.23 2.23 0 0 0 .44-.13 2.5 2.5 0 0 0 1.54-2.31 2.45 2.45 0 0 0-.19-1A2.48 2.48 0 0 0 5.503.19a2.45 2.45 0 0 0-1-.19 2.5 2.5 0 0 0-2.31 1.54 2.52 2.52 0 0 0 .54 2.73c.35.343.79.579 1.27.68v5.1a2.411 2.411 0 0 0-.89.37 2.5 2.5 0 1 0 3.47 3.468 2.5 2.5 0 0 0 .42-1.387 2.45 2.45 0 0 0-.19-1 2.48 2.48 0 0 0-1.81-1.49v-2.4a5.52 5.52 0 0 0 2 1.73 5.65 5.65 0 0 0 2.09.6 2.5 2.5 0 0 0 4.95-.49 2.51 2.51 0 0 0-.77-1.72zm-8.2 3.38c.276.117.512.312.68.56a1.5 1.5 0 0 1-2.08 2.08 1.55 1.55 0 0 1-.56-.68 1.49 1.49 0 0 1-.08-.86 1.49 1.49 0 0 1 1.18-1.18 1.49 1.49 0 0 1 .86.08zM4.503 4a1.5 1.5 0 0 1-1.39-.93 1.49 1.49 0 0 1-.08-.86 1.49 1.49 0 0 1 1.18-1.18 1.49 1.49 0 0 1 .86.08A1.5 1.5 0 0 1 4.503 4zm8.06 6.56a1.5 1.5 0 0 1-2.45-.49 1.49 1.49 0 0 1-.08-.86 1.49 1.49 0 0 1 1.18-1.18 1.49 1.49 0 0 1 .86.08 1.499 1.499 0 0 1 .49 2.45z"
  }));
};

module.exports.VscGitPullRequestClosed = function VscGitPullRequestClosed(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5.616 4.928a2.487 2.487 0 0 1-1.119.922c-.148.06-.458.138-.458.138v5.008a2.51 2.51 0 0 1 1.579 1.062c.273.412.419.895.419 1.388.008.343-.057.684-.19 1A2.485 2.485 0 0 1 3.5 15.984a2.48 2.48 0 0 1-1.388-.42A2.486 2.486 0 0 1 1.05 13c.095-.486.331-.932.68-1.283.349-.343.79-.579 1.269-.68V5.949a2.6 2.6 0 0 1-1.269-.68 2.503 2.503 0 0 1-.68-1.283A2.487 2.487 0 0 1 2.11 1.421 2.49 2.49 0 0 1 3.5 1a2.504 2.504 0 0 1 1.807.729 2.493 2.493 0 0 1 .729 1.81c.002.494-.144.977-.42 1.389Zm-.756 7.861a1.5 1.5 0 0 0-.552-.579 1.45 1.45 0 0 0-.77-.21 1.495 1.495 0 0 0-1.47 1.79 1.493 1.493 0 0 0 1.18 1.179c.288.058.586.03.86-.08.276-.117.512-.312.68-.56.15-.226.235-.49.249-.76a1.51 1.51 0 0 0-.177-.78ZM2.708 4.741c.247.161.536.25.83.25.271 0 .538-.075.77-.211a1.514 1.514 0 0 0 .729-1.36 1.513 1.513 0 0 0-.25-.76 1.551 1.551 0 0 0-.68-.559 1.49 1.49 0 0 0-.86-.08 1.494 1.494 0 0 0-1.179 1.18c-.058.288-.03.586.08.86.117.276.312.512.56.68Zm10.329 6.296c.48.097.922.335 1.269.68.466.47.729 1.107.725 1.766.002.493-.144.977-.42 1.388a2.5 2.5 0 0 1-3.848.384 2.5 2.5 0 0 1 .382-3.848c.268-.183.572-.308.89-.37V7.489h1.002v3.548Zm.557 3.508a1.493 1.493 0 0 0 .191-1.888 1.551 1.551 0 0 0-.68-.56 1.49 1.49 0 0 0-.86-.08 1.493 1.493 0 0 0-1.179 1.18 1.49 1.49 0 0 0 .08.86 1.497 1.497 0 0 0 2.448.49ZM11.688 3.4 10 5.088l.707.707 1.688-1.688 1.687 1.688.707-.707L13.103 3.4l1.688-1.687-.708-.707-1.687 1.687-1.688-1.687-.707.707L11.688 3.4Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGitPullRequestCreate = function VscGitPullRequestCreate(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5.616 4.928a2.487 2.487 0 0 1-1.119.922c-.148.06-.458.138-.458.138v5.008a2.51 2.51 0 0 1 1.579 1.062c.273.412.419.895.419 1.388.008.343-.057.684-.19 1A2.485 2.485 0 0 1 3.5 15.984a2.482 2.482 0 0 1-1.388-.419A2.487 2.487 0 0 1 1.05 13c.095-.486.331-.932.68-1.283.349-.343.79-.579 1.269-.68V5.949a2.6 2.6 0 0 1-1.269-.68 2.503 2.503 0 0 1-.68-1.283 2.487 2.487 0 0 1 1.06-2.565A2.49 2.49 0 0 1 3.5 1a2.504 2.504 0 0 1 1.807.729 2.493 2.493 0 0 1 .729 1.81c.002.494-.144.978-.42 1.389zm-.756 7.861a1.5 1.5 0 0 0-.552-.579 1.45 1.45 0 0 0-.77-.21 1.495 1.495 0 0 0-1.47 1.79 1.493 1.493 0 0 0 1.18 1.179c.288.058.586.03.86-.08.276-.117.512-.312.68-.56.15-.226.235-.49.249-.76a1.51 1.51 0 0 0-.177-.78zM2.708 4.741c.247.161.536.25.83.25.271 0 .538-.075.77-.211a1.514 1.514 0 0 0 .729-1.359 1.513 1.513 0 0 0-.25-.76 1.551 1.551 0 0 0-.68-.56 1.49 1.49 0 0 0-.86-.08 1.494 1.494 0 0 0-1.179 1.18c-.058.288-.03.586.08.86.117.276.312.512.56.68zM13.037 7h-1.002V5.49a1.5 1.5 0 0 0-1.5-1.5H8.687l1.269 1.27-.71.709L7.117 3.84v-.7l2.13-2.13.71.711-1.269 1.27h1.85a2.484 2.484 0 0 1 2.312 1.541c.125.302.189.628.187.957V7zM13 16h-1v-3H9v-1h3V9h1v3h3v1h-3v3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGitPullRequestDraft = function VscGitPullRequestDraft(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M4.497 5.85c.456-.19.846-.511 1.119-.922.275-.412.421-.895.419-1.388a2.493 2.493 0 0 0-1.538-2.35 2.504 2.504 0 0 0-.998-.19 2.49 2.49 0 0 0-1.388.42A2.487 2.487 0 0 0 1.05 3.987c.095.486.331.932.68 1.283a2.6 2.6 0 0 0 1.269.68v5.088c-.48.101-.92.337-1.269.68-.349.35-.585.797-.68 1.283a2.486 2.486 0 0 0 1.062 2.565 2.48 2.48 0 0 0 1.388.419 2.44 2.44 0 0 0 1-.19 2.485 2.485 0 0 0 1.538-2.349 2.51 2.51 0 0 0-1.998-2.45V5.989s.31-.078.458-.138Zm-.189 6.36a1.5 1.5 0 0 1 .48 2.12 1.551 1.551 0 0 1-.68.559 1.492 1.492 0 0 1-.86.08 1.487 1.487 0 0 1-1.18-1.18 1.49 1.49 0 0 1 .08-.86c.117-.276.312-.512.56-.68.245-.164.534-.25.83-.25.271-.003.538.07.77.211Zm-.77-7.22a1.52 1.52 0 0 1-.83-.25 1.551 1.551 0 0 1-.56-.68 1.491 1.491 0 0 1-.08-.86 1.486 1.486 0 0 1 1.18-1.179 1.49 1.49 0 0 1 .86.08c.276.117.512.312.68.56A1.49 1.49 0 0 1 4.86 4.2c-.129.24-.32.438-.552.579-.232.136-.499.21-.77.21Z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M15.054 13.5a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0Zm-2.5 1.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z",
    "clip-rule": "evenodd"
  }), React.createElement("circle", {
    cx: "12.554",
    cy: "7.751",
    r: "1"
  }), React.createElement("circle", {
    cx: "12.554",
    cy: "3.501",
    r: "1"
  }));
};

module.exports.VscGitPullRequest = function VscGitPullRequest(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5.616 4.928a2.487 2.487 0 0 1-1.119.922c-.148.06-.458.138-.458.138v5.008a2.51 2.51 0 0 1 1.579 1.062c.273.412.419.895.419 1.388.008.343-.057.684-.19 1A2.485 2.485 0 0 1 3.5 15.984a2.482 2.482 0 0 1-1.388-.419A2.487 2.487 0 0 1 1.05 13c.095-.486.331-.932.68-1.283.349-.343.79-.579 1.269-.68V5.949a2.6 2.6 0 0 1-1.269-.68 2.503 2.503 0 0 1-.68-1.283 2.487 2.487 0 0 1 1.06-2.565A2.49 2.49 0 0 1 3.5 1a2.504 2.504 0 0 1 1.807.729 2.493 2.493 0 0 1 .729 1.81c.002.494-.144.978-.42 1.389zm-.756 7.861a1.5 1.5 0 0 0-.552-.579 1.45 1.45 0 0 0-.77-.21 1.495 1.495 0 0 0-1.47 1.79 1.493 1.493 0 0 0 1.18 1.179c.288.058.586.03.86-.08.276-.117.512-.312.68-.56.15-.226.235-.49.249-.76a1.51 1.51 0 0 0-.177-.78zM2.708 4.741c.247.161.536.25.83.25.271 0 .538-.075.77-.211a1.514 1.514 0 0 0 .729-1.359 1.513 1.513 0 0 0-.25-.76 1.551 1.551 0 0 0-.68-.56 1.49 1.49 0 0 0-.86-.08 1.494 1.494 0 0 0-1.179 1.18c-.058.288-.03.586.08.86.117.276.312.512.56.68zm10.329 6.296c.48.097.922.335 1.269.68.466.47.729 1.107.725 1.766.002.493-.144.977-.42 1.388a2.499 2.499 0 0 1-4.532-.899 2.5 2.5 0 0 1 1.067-2.565c.267-.183.571-.308.889-.37V5.489a1.5 1.5 0 0 0-1.5-1.499H8.687l1.269 1.27-.71.709L7.117 3.84v-.7l2.13-2.13.71.711-1.269 1.27h1.85a2.484 2.484 0 0 1 2.312 1.541c.125.302.189.628.187.957v5.548zm.557 3.509a1.493 1.493 0 0 0 .191-1.89 1.552 1.552 0 0 0-.68-.559 1.49 1.49 0 0 0-.86-.08 1.493 1.493 0 0 0-1.179 1.18 1.49 1.49 0 0 0 .08.86 1.496 1.496 0 0 0 2.448.49z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGithubAction = function VscGithubAction(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3.04 10h2.58l.65 1H2.54l-.5-.5v-9l.5-.5h12l.5.5v4.77l-1-1.75V2h-11v8zm5.54 1-1.41 3.47h2.2L15 8.7 14.27 7h-1.63l.82-1.46L12.63 4H9.76l-.92.59-2.28 5L7.47 11h1.11zm1.18-6h2.87l-1.87 3h3.51l-5.76 5.84L10.2 10H7.47l2.29-5zM6.95 7H4.04V6H7.4l-.45 1zm-.9 2H4.04V8H6.5l-.45 1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGithubAlt = function VscGithubAlt(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    d: "M21.035 5.257c.91 1.092 1.364 2.366 1.364 3.822 0 5.277-3.002 6.824-5.823 7.279.364.637.455 1.365.455 2.093v3.73c0 .455-.273.728-.637.728a.718.718 0 0 1-.728-.728v-3.73a2.497 2.497 0 0 0-.728-2.093l.455-1.183c2.821-.364 5.733-1.274 5.733-6.187 0-1.183-.455-2.275-1.274-3.185l-.182-.727a4.04 4.04 0 0 0 .09-2.73c-.454.09-1.364.273-2.91 1.365l-.547.09a13.307 13.307 0 0 0-6.55 0l-.547-.09C7.57 2.71 6.66 2.437 6.204 2.437c-.273.91-.273 1.91.09 2.73l-.181.727c-.91.91-1.365 2.093-1.365 3.185 0 4.822 2.73 5.823 5.732 6.187l.364 1.183c-.546.546-.819 1.274-.728 2.002v3.821a.718.718 0 0 1-.728.728.718.718 0 0 1-.728-.728V20.18c-3.002.637-4.185-.91-5.095-2.092-.455-.546-.819-1.001-1.274-1.092-.09-.091-.364-.455-.273-.819.091-.364.455-.637.82-.455.91.182 1.455.91 2 1.547.82 1.092 1.639 2.092 4.095 1.547v-.364c-.09-.728.091-1.456.455-2.093-2.73-.546-5.914-2.093-5.914-7.279 0-1.456.455-2.73 1.365-3.822-.273-1.273-.182-2.638.273-3.73l.455-.364C5.749 1.073 7.023.8 9.66 2.437a13.673 13.673 0 0 1 6.642 0C18.851.708 20.216.98 20.398 1.072l.455.364c.455 1.274.546 2.548.182 3.821z"
  }));
};

module.exports.VscGithubInverted = function VscGithubInverted(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.976 0A7.977 7.977 0 0 0 0 7.976c0 3.522 2.3 6.507 5.431 7.584.392.049.538-.196.538-.392v-1.37c-2.201.49-2.69-1.076-2.69-1.076-.343-.93-.881-1.175-.881-1.175-.734-.489.048-.489.048-.489.783.049 1.224.832 1.224.832.734 1.223 1.859.88 2.3.685.048-.538.293-.88.489-1.076-1.762-.196-3.621-.881-3.621-3.964 0-.88.293-1.566.832-2.153-.05-.147-.343-.978.098-2.055 0 0 .685-.196 2.201.832.636-.196 1.322-.245 2.007-.245s1.37.098 2.006.245c1.517-1.027 2.202-.832 2.202-.832.44 1.077.146 1.908.097 2.104a3.16 3.16 0 0 1 .832 2.153c0 3.083-1.86 3.719-3.62 3.915.293.244.538.733.538 1.467v2.202c0 .196.146.44.538.392A7.984 7.984 0 0 0 16 7.976C15.951 3.572 12.38 0 7.976 0z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGithub = function VscGithub(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    d: "M12 0a12 12 0 1 0 0 24 12 12 0 0 0 0-24zm3.163 21.783h-.093a.513.513 0 0 1-.382-.14.513.513 0 0 1-.14-.372v-1.406c.006-.467.01-.94.01-1.416a3.693 3.693 0 0 0-.151-1.028 1.832 1.832 0 0 0-.542-.875 8.014 8.014 0 0 0 2.038-.471 4.051 4.051 0 0 0 1.466-.964c.407-.427.71-.943.885-1.506a6.77 6.77 0 0 0 .3-2.13 4.138 4.138 0 0 0-.26-1.476 3.892 3.892 0 0 0-.795-1.284 2.81 2.81 0 0 0 .162-.582c.033-.2.05-.402.05-.604 0-.26-.03-.52-.09-.773a5.309 5.309 0 0 0-.221-.763.293.293 0 0 0-.111-.02h-.11c-.23.002-.456.04-.674.111a5.34 5.34 0 0 0-.703.26 6.503 6.503 0 0 0-.661.343c-.215.127-.405.249-.573.362a9.578 9.578 0 0 0-5.143 0 13.507 13.507 0 0 0-.572-.362 6.022 6.022 0 0 0-.672-.342 4.516 4.516 0 0 0-.705-.261 2.203 2.203 0 0 0-.662-.111h-.11a.29.29 0 0 0-.11.02 5.844 5.844 0 0 0-.23.763c-.054.254-.08.513-.081.773 0 .202.017.404.051.604.033.199.086.394.16.582A3.888 3.888 0 0 0 5.702 10a4.142 4.142 0 0 0-.263 1.476 6.871 6.871 0 0 0 .292 2.12c.181.563.483 1.08.884 1.516.415.422.915.75 1.466.964.653.25 1.337.41 2.033.476a1.828 1.828 0 0 0-.452.633 2.99 2.99 0 0 0-.2.744 2.754 2.754 0 0 1-1.175.27 1.788 1.788 0 0 1-1.065-.3 2.904 2.904 0 0 1-.752-.824 3.1 3.1 0 0 0-.292-.382 2.693 2.693 0 0 0-.372-.343 1.841 1.841 0 0 0-.432-.24 1.2 1.2 0 0 0-.481-.101c-.04.001-.08.005-.12.01a.649.649 0 0 0-.162.02.408.408 0 0 0-.13.06.116.116 0 0 0-.06.1.33.33 0 0 0 .14.242c.093.074.17.131.232.171l.03.021c.133.103.261.214.382.333.112.098.213.209.3.33.09.119.168.246.231.381.073.134.15.288.231.463.188.474.522.875.954 1.145.453.243.961.364 1.476.351.174 0 .349-.01.522-.03.172-.028.343-.057.515-.091v1.743a.5.5 0 0 1-.533.521h-.062a10.286 10.286 0 1 1 6.324 0v.005z"
  }));
};

module.exports.VscGlobe = function VscGlobe(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8.5 1a6.5 6.5 0 1 1 0 13 6.5 6.5 0 0 1 0-13zm4.894 4a5.527 5.527 0 0 0-3.053-2.676c.444.84.765 1.74.953 2.676h2.1zm.582 2.995A5.11 5.11 0 0 0 14 7.5a5.464 5.464 0 0 0-.213-1.5h-2.342c.032.331.055.664.055 1a10.114 10.114 0 0 1-.206 2h2.493c.095-.329.158-.665.19-1.005zm-3.535 0 .006-.051A9.04 9.04 0 0 0 10.5 7a8.994 8.994 0 0 0-.076-1H6.576A8.82 8.82 0 0 0 6.5 7a8.98 8.98 0 0 0 .233 2h3.534c.077-.332.135-.667.174-1.005zM10.249 5a8.974 8.974 0 0 0-1.255-2.97C8.83 2.016 8.666 2 8.5 2a3.62 3.62 0 0 0-.312.015l-.182.015L8 2.04A8.97 8.97 0 0 0 6.751 5h3.498zM5.706 5a9.959 9.959 0 0 1 .966-2.681A5.527 5.527 0 0 0 3.606 5h2.1zM3.213 6A5.48 5.48 0 0 0 3 7.5 5.48 5.48 0 0 0 3.213 9h2.493A10.016 10.016 0 0 1 5.5 7c0-.336.023-.669.055-1H3.213zm2.754 4h-2.36a5.515 5.515 0 0 0 3.819 2.893A10.023 10.023 0 0 1 5.967 10zM8.5 12.644A8.942 8.942 0 0 0 9.978 10H7.022A8.943 8.943 0 0 0 8.5 12.644zM11.033 10a10.024 10.024 0 0 1-1.459 2.893A5.517 5.517 0 0 0 13.393 10h-2.36z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGoToFile = function VscGoToFile(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m6 5.914 2.06-2.06v-.708L5.915 1l-.707.707.043.043.25.25 1 1h-3a2.5 2.5 0 0 0 0 5H4V7h-.5a1.5 1.5 0 1 1 0-3h3L5.207 5.293 5.914 6 6 5.914zM11 2H8.328l-1-1H12l.71.29 3 3L16 5v9l-1 1H6l-1-1V6.5l1 .847V14h9V6h-4V2zm1 0v3h3l-3-3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGrabber = function VscGrabber(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M15 6H1v1h14V6zm0 3H1v1h14V9z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGraphLeft = function VscGraphLeft(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2.119 4 3 4.881l-.619.619L.715 3.833v-.618L2.38 1.548l.62.619L2.167 3H15v1H2.119zM4 14.546V5.455L4.5 5h2l.5.455v9.09L6.5 15h-2l-.5-.454zm2-.455V5.909H5v8.182h1zm2-1.535V5.444L8.5 5h2l.5.444v7.112l-.5.444h-2l-.5-.444zm2-.445V5.89H9v6.222h1zm2-6.682v5.143l.5.428h2l.5-.428V5.429L14.5 5h-2l-.5.429zm2 .428v4.286h-1V5.857h1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGraphLine = function VscGraphLine(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M15 13v1H1.5l-.5-.5V0h1v13h13Z"
  }), React.createElement("path", {
    d: "M13 3.207 7.854 8.354h-.708L5.5 6.707l-3.646 3.647-.708-.708 4-4h.708L7.5 7.293l5.146-5.147h.707l2 2-.707.708L13 3.207Z"
  }));
};

module.exports.VscGraphScatter = function VscGraphScatter(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M15 13v1H1.5l-.5-.5V0h1v13h13Z"
  }), React.createElement("path", {
    d: "M5 2H7V4H5z"
  }), React.createElement("path", {
    d: "M12 1H14V3H12z"
  }), React.createElement("path", {
    d: "M8 5H10V7H8z"
  }), React.createElement("path", {
    d: "M5 9H7V11H5z"
  }), React.createElement("path", {
    d: "M12 8H14V10H12z"
  }));
};

module.exports.VscGraph = function VscGraph(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 14H15v-1H2V0H1v13.5l.5.5zM3 11.5v-8l.5-.5h2l.5.5v8l-.5.5h-2l-.5-.5zm2-.5V4H4v7h1zm6-9.5v10l.5.5h2l.5-.5v-10l-.5-.5h-2l-.5.5zm2 .5v9h-1V2h1zm-6 9.5v-6l.5-.5h2l.5.5v6l-.5.5h-2l-.5-.5zm2-.5V6H8v5h1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscGripper = function VscGripper(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M5 3h2v2H5zm0 4h2v2H5zm0 4h2v2H5zm4-8h2v2H9zm0 4h2v2H9zm0 4h2v2H9z"
  }));
};

module.exports.VscGroupByRefType = function VscGroupByRefType(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 1h2v1H2v12h1.5v1h-2l-.5-.5v-13l.5-.5zm6 6h-2L5 6.5v-2l.5-.5h2l.5.5v2l-.5.5zM6 6h1V5H6v1zm7.5 1h-3l-.5-.5v-3l.5-.5h3l.5.5v3l-.5.5zM11 6h2V4h-2v2zm-3.5 6h-2l-.5-.5v-2l.5-.5h2l.5.5v2l-.5.5zM6 11h1v-1H6v1zm7.5 2h-3l-.5-.5v-3l.5-.5h3l.5.5v3l-.5.5zM11 12h2v-2h-2v2zm-1-2H8v1h2v-1zm0-5H8v1h2V5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscHeart = function VscHeart(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14.88 4.78a3.489 3.489 0 0 0-.37-.9 3.24 3.24 0 0 0-.6-.79 3.78 3.78 0 0 0-1.21-.81 3.74 3.74 0 0 0-2.84 0 4 4 0 0 0-1.16.75l-.05.06-.65.65-.65-.65-.05-.06a4 4 0 0 0-1.16-.75 3.74 3.74 0 0 0-2.84 0 3.78 3.78 0 0 0-1.21.81 3.55 3.55 0 0 0-.97 1.69 3.75 3.75 0 0 0-.12 1c0 .317.04.633.12.94a4 4 0 0 0 .36.89 3.8 3.8 0 0 0 .61.79L8 14.31l5.91-5.91c.237-.233.44-.5.6-.79A3.578 3.578 0 0 0 15 5.78a3.747 3.747 0 0 0-.12-1zm-1 1.63a2.69 2.69 0 0 1-.69 1.21l-5.21 5.2-5.21-5.2a2.9 2.9 0 0 1-.44-.57 3 3 0 0 1-.27-.65 3.25 3.25 0 0 1-.08-.69A3.36 3.36 0 0 1 2.06 5a2.8 2.8 0 0 1 .27-.65c.12-.21.268-.4.44-.57a2.91 2.91 0 0 1 .89-.6 2.8 2.8 0 0 1 2.08 0c.33.137.628.338.88.59l1.36 1.37 1.36-1.37a2.72 2.72 0 0 1 .88-.59 2.8 2.8 0 0 1 2.08 0c.331.143.633.347.89.6.174.165.32.357.43.57a2.69 2.69 0 0 1 .35 1.34 2.6 2.6 0 0 1-.06.72h-.03z"
  }));
};

module.exports.VscHistory = function VscHistory(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13.507 12.324a7 7 0 0 0 .065-8.56A7 7 0 0 0 2 4.393V2H1v3.5l.5.5H5V5H2.811a6.008 6.008 0 1 1-.135 5.77l-.887.462a7 7 0 0 0 11.718 1.092zm-3.361-.97.708-.707L8 7.792V4H7v4l.146.354 3 3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscHome = function VscHome(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m8.36 1.37 6.36 5.8-.71.71L13 6.964v6.526l-.5.5h-3l-.5-.5v-3.5H7v3.5l-.5.5h-3l-.5-.5V6.972L2 7.88l-.71-.71 6.35-5.8h.72zM4 6.063v6.927h2v-3.5l.5-.5h3l.5.5v3.5h2V6.057L8 2.43 4 6.063z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscHorizontalRule = function VscHorizontalRule(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M6.432 10h.823V4h-.823v2.61h-2.61V4H3v6h.823V7.394h2.61V10zm5.668 0h.9l-1.28-2.63c.131-.058.26-.134.389-.23a1.666 1.666 0 0 0 .585-.797c.064-.171.096-.364.096-.58a1.77 1.77 0 0 0-.082-.557 1.644 1.644 0 0 0-.22-.446 1.504 1.504 0 0 0-.31-.341 1.864 1.864 0 0 0-.737-.373A1.446 1.446 0 0 0 11.1 4H8.64v6h.824V7.518h1.467L12.1 10zm-.681-3.32a.874.874 0 0 1-.293.055H9.463V4.787h1.663a.87.87 0 0 1 .576.24.956.956 0 0 1 .306.737c0 .168-.029.314-.087.437a.91.91 0 0 1-.503.479zM13 12H3v1h10v-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscHubot = function VscHubot(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8.48 4h4l.5.5v2.03h.52l.5.5V8l-.5.5h-.52v3l-.5.5H9.36l-2.5 2.76L6 14.4V12H3.5l-.5-.64V8.5h-.5L2 8v-.97l.5-.5H3V4.36L3.53 4h4V2.86A1 1 0 0 1 7 2a1 1 0 0 1 2 0 1 1 0 0 1-.52.83V4zM12 8V5H4v5.86l2.5.14H7v2.19l1.8-2.04.35-.15H12V8zm-2.12.51a2.71 2.71 0 0 1-1.37.74v-.01a2.71 2.71 0 0 1-2.42-.74l-.7.71c.34.34.745.608 1.19.79.45.188.932.286 1.42.29a3.7 3.7 0 0 0 2.58-1.07l-.7-.71zM6.49 6.5h-1v1h1v-1zm3 0h1v1h-1v-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscInbox = function VscInbox(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 14h13l.5-.5V9l-2.77-7.66-.47-.34H4.27l-.47.33L1 8.74v4.76l.5.5zM14 13H2v-2.98h2.55l.74 1.25.43.24h4.57l.44-.26.69-1.23H14V13zm-.022-3.98H11.12l-.43.26-.69 1.23H6.01l-.75-1.25-.43-.24H2V9l2.62-7h6.78l2.578 7.02z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscInfo = function VscInfo(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8.568 1.031A6.8 6.8 0 0 1 12.76 3.05a7.06 7.06 0 0 1 .46 9.39 6.85 6.85 0 0 1-8.58 1.74 7 7 0 0 1-3.12-3.5 7.12 7.12 0 0 1-.23-4.71 7 7 0 0 1 2.77-3.79 6.8 6.8 0 0 1 4.508-1.149zM9.04 13.88a5.89 5.89 0 0 0 3.41-2.07 6.07 6.07 0 0 0-.4-8.06 5.82 5.82 0 0 0-7.43-.74 6.06 6.06 0 0 0 .5 10.29 5.81 5.81 0 0 0 3.92.58zM7.375 6h1.25V5h-1.25v1zm1.25 1v4h-1.25V7h1.25z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscInspect = function VscInspect(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m1 3 1-1h12l1 1v6h-1V3H2v8h5v1H2l-1-1V3zm14.707 9.707L9 6v9.414l2.707-2.707h4zM10 13V8.414l3.293 3.293h-2L10 13z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscIssueDraft = function VscIssueDraft(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m12.732 9.2.952.31A6.494 6.494 0 0 0 14 7.5c0-.701-.111-1.377-.316-2.01l-.952.31c.174.534.268 1.105.268 1.7s-.094 1.166-.268 1.7Zm-.33-4.197.89-.455a6.529 6.529 0 0 0-2.84-2.84l-.455.89a5.528 5.528 0 0 1 2.405 2.405ZM9.2 2.268l.31-.951A6.495 6.495 0 0 0 7.5 1c-.701 0-1.377.111-2.01.317l.31.95A5.495 5.495 0 0 1 7.5 2c.595 0 1.166.094 1.7.268Zm-4.197.33-.455-.89a6.528 6.528 0 0 0-2.84 2.84l.89.455a5.528 5.528 0 0 1 2.405-2.405ZM1 7.5c0-.701.111-1.377.317-2.01l.95.31A5.495 5.495 0 0 0 2 7.5c0 .595.094 1.166.268 1.7l-.951.31A6.495 6.495 0 0 1 1 7.5Zm1.598 2.497-.89.455a6.529 6.529 0 0 0 2.84 2.84l.455-.89a5.528 5.528 0 0 1-2.405-2.405ZM5.8 12.732l-.31.952A6.494 6.494 0 0 0 7.5 14c.701 0 1.377-.111 2.01-.316l-.31-.952A5.497 5.497 0 0 1 7.5 13a5.497 5.497 0 0 1-1.7-.268Zm4.197-.33.455.89a6.53 6.53 0 0 0 2.84-2.84l-.89-.455a5.528 5.528 0 0 1-2.405 2.405ZM7.5 8.5a1 1 0 1 0 0-2 1 1 0 0 0 0 2Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscIssueReopened = function VscIssueReopened(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5.28 5.656 2 7.006l-.66-.26L0 3.506l.92-.38.81 1.95a6.48 6.48 0 0 1 12.48 1.93h-1a5.48 5.48 0 0 0-10.64-1.28l2.32-1 .39.93Zm8.86 2.68 1.34 3.23-.92.44-.82-2a6.49 6.49 0 0 1-12.5-2h1v-.5a5.49 5.49 0 0 0 10.64 1.89l-2.25.93-.39-.92 3.25-1.35.65.28Z",
    "clip-rule": "evenodd"
  }), React.createElement("circle", {
    cx: "7.74",
    cy: "7.54",
    r: "1"
  }));
};

module.exports.VscIssues = function VscIssues(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M7.5 1a6.5 6.5 0 1 0 0 13 6.5 6.5 0 0 0 0-13Zm0 12a5.5 5.5 0 1 1 0-11 5.5 5.5 0 0 1 0 11Z"
  }), React.createElement("circle", {
    cx: "7.5",
    cy: "7.5",
    r: "1"
  }));
};

module.exports.VscItalic = function VscItalic(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m9.001 13.593-.097.325H4l.123-.325c.492-.012.817-.053.976-.123.257-.1.448-.238.57-.413.194-.276.394-.768.599-1.477l2.074-7.19c.176-.597.263-1.048.263-1.353a.643.643 0 0 0-.114-.387.683.683 0 0 0-.351-.237c-.153-.059-.454-.088-.906-.088L7.34 2h4.605l-.096.325c-.375-.006-.654.035-.835.123a1.358 1.358 0 0 0-.607.501c-.134.217-.31.697-.527 1.442l-2.066 7.19c-.187.661-.28 1.083-.28 1.265 0 .146.034.272.105.378.076.1.193.178.351.237.164.053.501.097 1.011.132z"
  }));
};

module.exports.VscJersey = function VscJersey(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M11.91 14.22H4.06l-.5-.5V7.06H2.15l-.48-.38L1 4l.33-.6L5.59 2l.64.32a2.7 2.7 0 0 0 .21.44c.071.103.152.2.24.29.168.169.369.302.59.39a1.82 1.82 0 0 0 1.43 0 1.74 1.74 0 0 0 .59-.39c.09-.095.173-.195.25-.3l.15-.29a1.21 1.21 0 0 0 .05-.14l.64-.32 4.26 1.42L15 4l-.66 2.66-.49.38h-1.44v6.66l-.5.52zm-7.35-1h6.85V6.56l.5-.5h1.52l.46-1.83-3.4-1.14a1.132 1.132 0 0 1-.12.21c-.11.161-.233.312-.37.45a2.75 2.75 0 0 1-.91.61 2.85 2.85 0 0 1-2.22 0A2.92 2.92 0 0 1 6 3.75a2.17 2.17 0 0 1-.36-.44l-.13-.22-3.43 1.14.46 1.83h1.52l.5.5v6.66z"
  }));
};

module.exports.VscJson = function VscJson(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M6 2.984V2h-.09c-.313 0-.616.062-.909.185a2.33 2.33 0 0 0-.775.53 2.23 2.23 0 0 0-.493.753v.001a3.542 3.542 0 0 0-.198.83v.002a6.08 6.08 0 0 0-.024.863c.012.29.018.58.018.869 0 .203-.04.393-.117.572v.001a1.504 1.504 0 0 1-.765.787 1.376 1.376 0 0 1-.558.115H2v.984h.09c.195 0 .38.04.556.121l.001.001c.178.078.329.184.455.318l.002.002c.13.13.233.285.307.465l.001.002c.078.18.117.368.117.566 0 .29-.006.58-.018.869-.012.296-.004.585.024.87v.001c.033.283.099.558.197.824v.001c.106.273.271.524.494.753.223.23.482.407.775.53.293.123.596.185.91.185H6v-.984h-.09c-.2 0-.387-.038-.563-.115a1.613 1.613 0 0 1-.457-.32 1.659 1.659 0 0 1-.309-.467c-.074-.18-.11-.37-.11-.573 0-.228.003-.453.011-.672.008-.228.008-.45 0-.665a4.639 4.639 0 0 0-.055-.64 2.682 2.682 0 0 0-.168-.609A2.284 2.284 0 0 0 3.522 8a2.284 2.284 0 0 0 .738-.955c.08-.192.135-.393.168-.602.033-.21.051-.423.055-.64.008-.22.008-.442 0-.666-.008-.224-.012-.45-.012-.678a1.47 1.47 0 0 1 .877-1.354 1.33 1.33 0 0 1 .563-.121H6zm4 10.032V14h.09c.313 0 .616-.062.909-.185.293-.123.552-.3.775-.53.223-.23.388-.48.493-.753v-.001c.1-.266.165-.543.198-.83v-.002c.028-.28.036-.567.024-.863-.012-.29-.018-.58-.018-.869 0-.203.04-.393.117-.572v-.001a1.502 1.502 0 0 1 .765-.787 1.38 1.38 0 0 1 .558-.115H14v-.984h-.09c-.196 0-.381-.04-.557-.121l-.001-.001a1.376 1.376 0 0 1-.455-.318l-.002-.002a1.415 1.415 0 0 1-.307-.465v-.002a1.405 1.405 0 0 1-.118-.566c0-.29.006-.58.018-.869a6.174 6.174 0 0 0-.024-.87v-.001a3.537 3.537 0 0 0-.197-.824v-.001a2.23 2.23 0 0 0-.494-.753 2.331 2.331 0 0 0-.775-.53 2.325 2.325 0 0 0-.91-.185H10v.984h.09c.2 0 .387.038.562.115.174.082.326.188.457.32.127.134.23.29.309.467.074.18.11.37.11.573 0 .228-.003.452-.011.672-.008.228-.008.45 0 .665.004.222.022.435.055.64.033.214.089.416.168.609a2.285 2.285 0 0 0 .738.955 2.285 2.285 0 0 0-.738.955 2.689 2.689 0 0 0-.168.602c-.033.21-.051.423-.055.64a9.15 9.15 0 0 0 0 .666c.008.224.012.45.012.678a1.471 1.471 0 0 1-.877 1.354 1.33 1.33 0 0 1-.563.121H10z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscKebabVertical = function VscKebabVertical(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.444 13.832a1 1 0 1 0 1.111-1.663 1 1 0 0 0-1.11 1.662zM8 9a1 1 0 1 1 0-2 1 1 0 0 1 0 2zm0-5a1 1 0 1 1 0-2 1 1 0 0 1 0 2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscKey = function VscKey(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.351 1.091a4.528 4.528 0 0 1 3.44 3.16c.215.724.247 1.49.093 2.23a4.583 4.583 0 0 1-4.437 3.6c-.438 0-.874-.063-1.293-.19l-.8.938-.379.175H7v1.5l-.5.5H5v1.5l-.5.5h-3l-.5-.5v-2.307l.146-.353L6.12 6.87a4.464 4.464 0 0 1-.2-1.405 4.528 4.528 0 0 1 5.431-4.375zm1.318 7.2a3.568 3.568 0 0 0 1.239-2.005l.004.005A3.543 3.543 0 0 0 9.72 2.08a3.576 3.576 0 0 0-2.8 3.4c-.01.456.07.908.239 1.33l-.11.543L2 12.404v1.6h2v-1.5l.5-.5H6v-1.5l.5-.5h1.245l.876-1.016.561-.14a3.47 3.47 0 0 0 1.269.238 3.568 3.568 0 0 0 2.218-.795zm-.838-2.732a1 1 0 1 0-1.662-1.11 1 1 0 0 0 1.662 1.11z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscLaw = function VscLaw(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14.63 7 13 3h1V2H9V1H8v1H3v1h1L2.38 7H2v1h.15c.156.498.473.93.9 1.23a2.47 2.47 0 0 0 2.9 0A2.44 2.44 0 0 0 6.86 8H7V7h-.45L4.88 3H8v8H6l-.39.18-2 2.51.39.81h9l.39-.81-2-2.51L11 11H9V3h3.13l-1.67 4H10v1h.15a2.48 2.48 0 0 0 4.71 0H15V7h-.37zM5.22 8.51a1.52 1.52 0 0 1-.72.19 1.45 1.45 0 0 1-.71-.19A1.47 1.47 0 0 1 3.25 8h2.5a1.52 1.52 0 0 1-.53.51zM5.47 7h-2l1-2.4 1 2.4zm5.29 5L12 13.5H5L6.24 12h4.52zm1.78-7.38 1 2.4h-2l1-2.4zm.68 3.91a1.41 1.41 0 0 1-.72.19 1.35 1.35 0 0 1-.71-.19 1.55 1.55 0 0 1-.54-.53h2.5a1.37 1.37 0 0 1-.53.53z"
  }));
};

module.exports.VscLayersActive = function VscLayersActive(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m8.185 1.083-.558.004-5.909 4.037.004.828L7.63 9.915l.55.004 6.092-3.963.003-.836-6.09-4.037Zm-5.293 4.45 5.021-3.43 5.176 3.43-5.176 3.368-5.021-3.368Zm4.739 6.882L1.793 8.5h1.795l4.325 2.9 4.459-2.9h1.833l-.8.52a4.002 4.002 0 0 0-4.21 2.739l-1.013.66-.551-.004Zm1.373.776-1.09.71L3.587 11H1.793l5.838 3.915.55.004 1.02-.663a3.988 3.988 0 0 1-.197-1.065ZM11.333 10.506a3 3 0 1 1 3.333 4.987 3 3 0 0 1-3.333-4.987Zm1.698 3.817 1.79-2.387-.8-.6-1.48 1.974-.876-.7-.624.78 1.278 1.023.712-.09Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscLayersDot = function VscLayersDot(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m8.185 1.083-.558.004-5.909 4.037.004.828L7.63 9.915l.55.004 6.092-3.963.003-.836-6.09-4.037Zm-5.293 4.45 5.021-3.43 5.176 3.43-5.176 3.368-5.021-3.368Zm4.739 6.882L1.793 8.5h1.795l4.325 2.9 4.459-2.9h1.833l-.8.52a4.002 4.002 0 0 0-4.21 2.739l-1.013.66-.551-.004Zm1.373.776-1.09.71L3.587 11H1.793l5.838 3.915.55.004 1.02-.663a3.988 3.988 0 0 1-.197-1.065Z",
    "clip-rule": "evenodd"
  }), React.createElement("circle", {
    cx: "13",
    cy: "13",
    r: "3"
  }));
};

module.exports.VscLayers = function VscLayers(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m7.627 1.087.558-.004 6.091 4.037-.003.836L8.182 9.92l-.551-.004-5.91-3.963-.003-.828 5.91-4.037Zm.286 1.016-5.021 3.43 5.021 3.368 5.176-3.368-5.176-3.43ZM1.793 8.5l5.838 3.915.55.004L14.206 8.5h-1.833l-4.459 2.9-4.325-2.9H1.793Zm5.838 6.415L1.793 11h1.795l4.325 2.9 4.459-2.9h1.833l-6.023 3.92-.551-.005Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscLibrary = function VscLibrary(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m5 2.5.5-.5h2l.5.5v11l-.5.5h-2l-.5-.5v-11zM6 3v10h1V3H6zm3.171.345.299-.641 1.88-.684.64.299 3.762 10.336-.299.641-1.879.684-.64-.299L9.17 3.345zm1.11.128 3.42 9.396.94-.341-3.42-9.397-.94.342zM1 2.5l.5-.5h2l.5.5v11l-.5.5h-2l-.5-.5v-11zM2 3v10h1V3H2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscLightbulbAutofix = function VscLightbulbAutofix(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M12 9a3 3 0 1 0 0 6 3 3 0 0 0 0-6zm1.31 5L12 13l-1.3 1 .5-1.53-1.2-.83h1.47L12 10l.54 1.64H14l-1.2.83.51 1.53z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.17 8.085A3.979 3.979 0 0 0 8.288 10.5H6.409v2.201c0 .081.028.15.09.212a.29.29 0 0 0 .213.09h1.413c.089.348.223.678.396.982-.066.01-.134.015-.203.015H6.712a1.285 1.285 0 0 1-.922-.379 1.303 1.303 0 0 1-.38-.92v-1.6c0-.479-.092-.921-.274-1.329a3.556 3.556 0 0 0-.776-1.114 4.689 4.689 0 0 1-1.006-1.437A4.187 4.187 0 0 1 3 5.5a4.432 4.432 0 0 1 .616-2.27c.197-.336.432-.64.705-.914a4.6 4.6 0 0 1 .911-.702c.338-.196.7-.348 1.084-.454a4.45 4.45 0 0 1 1.2-.16 4.476 4.476 0 0 1 2.276.614 4.475 4.475 0 0 1 1.622 1.616 4.438 4.438 0 0 1 .616 2.27c0 .617-.117 1.191-.353 1.721a4.537 4.537 0 0 1-.506.864z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscLightbulb = function VscLightbulb(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.67 8.658a3.661 3.661 0 0 0-.781 1.114 3.28 3.28 0 0 0-.268 1.329v1.6a1.304 1.304 0 0 1-.794 1.197 1.282 1.282 0 0 1-.509.102H7.712a1.285 1.285 0 0 1-.922-.379 1.303 1.303 0 0 1-.38-.92v-1.6c0-.479-.092-.921-.274-1.329a3.556 3.556 0 0 0-.776-1.114 4.689 4.689 0 0 1-1.006-1.437A4.187 4.187 0 0 1 4 5.5a4.432 4.432 0 0 1 .616-2.27c.197-.336.432-.64.705-.914a4.6 4.6 0 0 1 .911-.702c.338-.196.7-.348 1.084-.454a4.45 4.45 0 0 1 1.2-.16 4.476 4.476 0 0 1 2.276.614 4.475 4.475 0 0 1 1.622 1.616 4.438 4.438 0 0 1 .616 2.27c0 .617-.117 1.191-.353 1.721a4.69 4.69 0 0 1-1.006 1.437zM9.623 10.5H7.409v2.201c0 .081.028.15.09.212a.29.29 0 0 0 .213.09h1.606a.289.289 0 0 0 .213-.09.286.286 0 0 0 .09-.212V10.5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscLinkExternal = function VscLinkExternal(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M1.5 1H6v1H2v12h12v-4h1v4.5l-.5.5h-13l-.5-.5v-13l.5-.5z"
  }), React.createElement("path", {
    d: "M15 1.5V8h-1V2.707L7.243 9.465l-.707-.708L13.293 2H8V1h6.5l.5.5z"
  }));
};

module.exports.VscLink = function VscLink(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4.4 3h3.085a3.4 3.4 0 0 1 3.4 3.4v.205A3.4 3.4 0 0 1 7.485 10H7V9h.485A2.4 2.4 0 0 0 9.88 6.61V6.4A2.4 2.4 0 0 0 7.49 4H4.4A2.4 2.4 0 0 0 2 6.4v.205A2.394 2.394 0 0 0 4 8.96v1a3.4 3.4 0 0 1-3-3.35V6.4A3.405 3.405 0 0 1 4.4 3zM12 7.04v-1a3.4 3.4 0 0 1 3 3.36v.205A3.405 3.405 0 0 1 11.605 13h-3.09A3.4 3.4 0 0 1 5.12 9.61V9.4A3.4 3.4 0 0 1 8.515 6H9v1h-.485A2.4 2.4 0 0 0 6.12 9.4v.205A2.4 2.4 0 0 0 8.515 12h3.09A2.4 2.4 0 0 0 14 9.61V9.4a2.394 2.394 0 0 0-2-2.36z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscListFilter = function VscListFilter(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M6 12v-1h4v1H6zM4 7h8v1H4V7zm10-4v1H2V3h12z"
  }));
};

module.exports.VscListFlat = function VscListFlat(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M2 10V9h12v1H2zm0-4h12v1H2V6zm12-3v1H2V3h12zM2 12v1h12v-1H2z"
  }));
};

module.exports.VscListOrdered = function VscListOrdered(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2.287 2.326 2.692 2h.677v3h-.708V2.792l-.374.281v-.747zM5 3h10v1H5V3zm0 4h10v1H5V7zm10 4H5v1h10v-1zM3.742 7.626l.029-.039.065-.09a.84.84 0 0 0 .156-.507c0-.12-.02-.24-.057-.354a.848.848 0 0 0-.492-.529 1.123 1.123 0 0 0-.452-.082 1.094 1.094 0 0 0-.458.087.867.867 0 0 0-.479.522A1.038 1.038 0 0 0 2 6.957v.05h.81v-.05a.3.3 0 0 1 .046-.157.174.174 0 0 1 .057-.054.19.19 0 0 1 .172 0 .188.188 0 0 1 .056.06.24.24 0 0 1 .031.081.445.445 0 0 1-.036.29 1.309 1.309 0 0 1-.12.182l-1 1.138-.012.013v.54h1.988v-.7h-.9l.65-.724zm-.037 3.817c.046.032.086.07.12.114a.841.841 0 0 1 .167.55c0 .107-.017.213-.05.314a.792.792 0 0 1-.487.5 1.288 1.288 0 0 1-.48.079c-.115 0-.23-.016-.341-.049a.94.94 0 0 1-.258-.123.751.751 0 0 1-.182-.177 1.063 1.063 0 0 1-.116-.2A1.038 1.038 0 0 1 2 12.078v-.049h.814v.049c0 .027.003.055.009.082a.207.207 0 0 0 .03.074.14.14 0 0 0 .053.052.2.2 0 0 0 .157.008.159.159 0 0 0 .056-.039.22.22 0 0 0 .042-.075.417.417 0 0 0 .017-.126.483.483 0 0 0-.022-.163.2.2 0 0 0-.051-.08.138.138 0 0 0-.06-.029.537.537 0 0 0-.077-.007h-.161v-.645h.168a.241.241 0 0 0 .069-.011.164.164 0 0 0 .065-.034.175.175 0 0 0 .048-.067.286.286 0 0 0 .021-.121.28.28 0 0 0-.016-.1.166.166 0 0 0-.097-.099.2.2 0 0 0-.156.007.164.164 0 0 0-.055.053.344.344 0 0 0-.04.156v.049H2v-.049a.987.987 0 0 1 .18-.544.8.8 0 0 1 .179-.186.87.87 0 0 1 .262-.133c.114-.036.234-.053.354-.051.116-.001.231.01.344.036.092.021.18.055.263.1a.757.757 0 0 1 .32.318.73.73 0 0 1 .09.347.81.81 0 0 1-.167.528.562.562 0 0 1-.12.114z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscListSelection = function VscListSelection(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M1 12v-1h9v1H1zm0-5h14v1H1V7zm11-4v1H1V3h11z"
  }));
};

module.exports.VscListTree = function VscListTree(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14 3v1H2V3h12zm-1 3v1H6V6h7zm0 3v1H5V9h8zm0 3v1H5v-1h8z"
  }), React.createElement("path", {
    d: "M5 4h1v9H5z"
  }));
};

module.exports.VscListUnordered = function VscListUnordered(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2 3H1v1h1V3zm0 3H1v1h1V6zM1 9h1v1H1V9zm1 3H1v1h1v-1zm2-9h11v1H4V3zm11 3H4v1h11V6zM4 9h11v1H4V9zm11 3H4v1h11v-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscLiveShare = function VscLiveShare(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13.735 1.694 15.178 1l8.029 6.328v1.388l-8.029 6.072-1.443-.694v-2.776h-.59c-4.06-.02-6.71.104-10.61 5.163l-1.534-.493a8.23 8.23 0 0 1 .271-2.255 11.026 11.026 0 0 1 3.92-6.793 11.339 11.339 0 0 1 7.502-2.547h1.04v-2.7zm1.804 7.917v2.776l5.676-4.281-5.648-4.545v2.664h-2.86A9.299 9.299 0 0 0 5.77 8.848a10.444 10.444 0 0 0-2.401 4.122c3.351-3.213 6.19-3.359 9.798-3.359h2.373zm-7.647 5.896a4.31 4.31 0 1 1 4.788 7.166 4.31 4.31 0 0 1-4.788-7.166zm.955 5.728a2.588 2.588 0 1 0 2.878-4.302 2.588 2.588 0 0 0-2.878 4.302z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscLoading = function VscLoading(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13.917 7A6.002 6.002 0 0 0 2.083 7H1.071a7.002 7.002 0 0 1 13.858 0h-1.012z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscLocation = function VscLocation(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M10.832 2.688A4.056 4.056 0 0 0 8.02 1.5h-.04a4.056 4.056 0 0 0-4 4c-.013.75.198 1.487.606 2.117L7.734 14h.533l3.147-6.383c.409-.63.62-1.367.606-2.117a4.056 4.056 0 0 0-1.188-2.812zM7.925 2.5l.082.01.074-.01a3.075 3.075 0 0 1 2.941 3.037 2.74 2.74 0 0 1-.467 1.568l-.02.034-.017.035L8 12.279l-2.517-5.1-.017-.039-.02-.034a2.74 2.74 0 0 1-.467-1.568A3.074 3.074 0 0 1 7.924 2.5zm.612 2.169a1 1 0 1 0-1.112 1.663 1 1 0 0 0 1.112-1.663zM6.87 3.837a2 2 0 1 1 2.22 3.326 2 2 0 0 1-2.22-3.326z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscLockSmall = function VscLockSmall(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m3 8 1-1h8l1 1v5l-1 1H4l-1-1V8Zm1 0v5h8V8H4ZM11 7V5a3 3 0 0 0-6 0v2h1V5a2 2 0 1 1 4 0v2h1Z"
  }));
};

module.exports.VscLock = function VscLock(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13 7h-1V5a4 4 0 1 0-8 0v2H3L2 8v6l1 1h10l1-1V8l-1-1zM5 5a3 3 0 1 1 6 0v2H5V5zm8 9H3V8h10v6z"
  }));
};

module.exports.VscMagnet = function VscMagnet(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8 1.5c-3.9 0-7 3.1-7 7v5l1 1h3l1-1v-5c0-1.1.9-2 2-2s2 .9 2 2v5l1 1h3l1-1v-5c0-3.9-3.1-7-7-7zm-3 12H2v-3h3v3zm9 0h-3v-3h3v3zm-3-4v-1c0-1.7-1.3-3-3-3-1.6 0-2.9 1.3-3 2.8v1.2H2v-1c0-3.3 2.7-6 6-6s6 2.7 6 6v1h-3z"
  }));
};

module.exports.VscMailRead = function VscMailRead(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8.25 1.57h-.51L1 5.56v7.94l.5.5h13l.5-.5V5.56L8.25 1.57zM8 2.58l5.63 3.32-1.37 1.59H3.74L2.43 5.9 8 2.58zM14 13H2V6.92L3.11 8.3l.39.19h9l.39-.19L14 6.92V13z"
  }));
};

module.exports.VscMail = function VscMail(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m1 3.5.5-.5h13l.5.5v9l-.5.5h-13l-.5-.5v-9zm1 1.035V12h12V4.536L8.31 8.9H7.7L2 4.535zM13.03 4H2.97L8 7.869 13.03 4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscMarkdown = function VscMarkdown(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M6.345 5h2.1v6.533H6.993l.055-5.31-1.774 5.31H4.072l-1.805-5.31c.04.644.06 5.31.06 5.31H1V5h2.156s1.528 4.493 1.577 4.807L6.345 5zm6.71 3.617v-3.5H11.11v3.5H9.166l2.917 2.916L15 8.617h-1.945z"
  }));
};

module.exports.VscMegaphone = function VscMegaphone(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m2 6.77 12.33-3.43.67.53v8.6l-.67.53-6.089-1.595a2.16 2.16 0 1 1-4.178-1.095L2 9.77l-.42-.53V7.3L2 6.77zm3.006 3.787a1.13 1.13 0 0 0-.04.242 1.17 1.17 0 0 0 2.288.347l-2.248-.589zM2.58 8.82 14 11.83V4.5L2.58 7.72v1.1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscMention = function VscMention(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M10.465 12.36a5.516 5.516 0 0 1-3.053.736 4.262 4.262 0 0 1-4.57-4.543 5.381 5.381 0 0 1 5.391-5.571c2.377 0 4.413 1.375 4.413 4.006 0 2.182-1.292 3.66-2.9 3.66-.676 0-1.1-.274-1.126-.917a2.012 2.012 0 0 1-1.756.913c-.969 0-1.629-.645-1.629-1.923 0-1.763 1.148-3.4 2.62-3.4a1.314 1.314 0 0 1 1.427.93l.211-.809h.9L9.6 8.646c-.226.916-.13 1.215.342 1.215.984 0 1.833-1.21 1.833-2.825 0-2.068-1.445-3.265-3.61-3.265-2.643 0-4.374 2.132-4.382 4.786a3.443 3.443 0 0 0 3.686 3.717c.973.04 1.94-.179 2.8-.634l.196.72zM6.217 8.639c0 .788.307 1.206.913 1.206.758 0 1.38-.6 1.683-1.831C9.136 6.746 8.85 6.1 7.94 6.1c-1.04 0-1.723 1.339-1.723 2.539z"
  }));
};

module.exports.VscMenu = function VscMenu(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M16 5H0V4h16v1zm0 8H0v-1h16v1zm0-4.008H0V8h16v.992z"
  }));
};

module.exports.VscMerge = function VscMerge(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M10.5 4.646 8.354 2.5h-.707L5.5 4.646l.707.707L7.3 4.261V5.28h-.02v.456l.025.001.006.319c.004.187.02.379.05.574.03.195.069.39.117.586.048.195.114.404.2.627.155.379.343.722.565 1.031.221.309.46.598.715.867.255.27.508.535.76.797.25.262.478.541.681.838.203.297.368.621.494.973.125.351.188.755.188 1.213v.884H12.5v-.884a5.991 5.991 0 0 0-.166-1.39 4.638 4.638 0 0 0-.427-1.1 5.875 5.875 0 0 0-.604-.897c-.222-.27-.453-.527-.693-.774-.24-.246-.471-.492-.693-.738a6.39 6.39 0 0 1-.604-.785 3.794 3.794 0 0 1-.433-.914 3.676 3.676 0 0 1-.16-1.13V5.28h-.001v-1l1.074 1.074.707-.708zM7.042 9.741a8.19 8.19 0 0 0 .329-.369 6.06 6.06 0 0 1-.62-1.15L6.744 8.2a7.26 7.26 0 0 1-.095-.263c-.17.256-.359.498-.565.726-.222.246-.453.492-.693.738-.24.247-.47.504-.693.774-.221.27-.423.568-.604.896a4.643 4.643 0 0 0-.427 1.102 5.995 5.995 0 0 0-.166 1.389v.884h1.42v-.884c0-.457.062-.862.188-1.213.125-.352.29-.676.493-.973.203-.297.43-.576.682-.838.251-.262.504-.527.76-.797z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscMilestone = function VscMilestone(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 1H7v2H1.5l-.5.5v4l.5.5H7v7h1V8h4.49l.34-.13 2.18-2v-.74l-2.18-2L12.5 3H8V1zm4.29 6H2V4h10.29l1.63 1.5L12.29 7zM5 5h5v1H5V5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscMirror = function VscMirror(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m8.57 1 6.2 4 .23.38v9.2l-.76.42L8 11l-6.24 4-.76-.42v-9.2L1.23 5l6.2-4h1.14zm-.06 9.13L14 13.67V5.65l-5.49-3.5V5h-1V2.13L2 5.67v8l5.51-3.56v.02h1zm.9-4.78.71-.7 2.47 2.48v.71l-2.46 2.46-.7-.7L11.02 8h-6L6.6 9.6l-.7.7-2.46-2.46v-.71l2.48-2.48.7.7L4.98 7h6.08L9.41 5.35z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscMortarBoard = function VscMortarBoard(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M15 5.66 8.18 3h-.36L1 5.66V12h1V7l2.31.9a4.35 4.35 0 0 0-.79 2.48c-.01.11-.01.22 0 .33v.11l.28.4L7.78 13h.41l3.94-1.81.28-.4v-.44a4.39 4.39 0 0 0-.78-2.47L15 6.57v-.91zm-3.52 4.68v.07L8 12l-3.5-1.6a.13.13 0 0 1 0-.06 3.44 3.44 0 0 1 .75-2.12l2.58 1h.36l2.56-1a3.4 3.4 0 0 1 .73 2.12zM8 8.25 2.52 6.12 8 4l5.48 2.14L8 8.25z"
  }));
};

module.exports.VscMove = function VscMove(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M10.232 10.707 8.5 12.44V9h-1v3.44l-1.732-1.733-.707.707L7.646 14h.708l2.585-2.586-.707-.707zM5.061 3.586l.707.707L7.5 2.56V6h1V2.56l1.732 1.733.707-.707L8.354 1h-.708L5.061 3.586zm-.268 1.682L3.06 7H6.5v1H3.06l1.733 1.732-.707.707L1.5 7.854v-.708l2.586-2.585.707.707zM9.5 7h3.44l-1.733-1.732.707-.707L14.5 7.146v.708l-2.586 2.585-.707-.707L12.94 8H9.5V7z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscMultipleWindows = function VscMultipleWindows(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m6 1.5.5-.5h8l.5.5v7l-.5.5H12V8h2V4H7v1H6V1.5zM7 2v1h7V2H7zM1.5 7l-.5.5v7l.5.5h8l.5-.5v-7L9.5 7h-8zM2 9V8h7v1H2zm0 1h7v4H2v-4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscMute = function VscMute(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 5h2.79l3.86-3.83.85.35v13l-.85.33L4.29 11H1.5l-.5-.5v-5l.5-.5zm3.35 5.17L8 13.31V2.73L4.85 5.85 4.5 6H2v4h2.5l.35.17zm9.381-4.108.707.707L13.207 8.5l1.731 1.732-.707.707L12.5 9.207l-1.732 1.732-.707-.707L11.793 8.5 10.06 6.77l.707-.707 1.733 1.73 1.731-1.731z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscNewFile = function VscNewFile(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m9.5 1.1 3.4 3.5.1.4v2h-1V6H8V2H3v11h4v1H2.5l-.5-.5v-12l.5-.5h6.7l.3.1zM9 2v3h2.9L9 2zm4 14h-1v-3H9v-1h3V9h1v3h3v1h-3v3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscNewFolder = function VscNewFolder(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14.5 2H7.71l-.85-.85L6.51 1h-5l-.5.5v11l.5.5H7v-1H1.99V6h4.49l.35-.15.86-.86H14v1.5l-.001.51h1.011V2.5L14.5 2zm-.51 2h-6.5l-.35.15-.86.86H2v-3h4.29l.85.85.36.15H14l-.01.99zM13 16h-1v-3H9v-1h3V9h1v3h3v1h-3v3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscNewline = function VscNewline(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M12 5.5v1.984a.5.5 0 0 1-.5.5H4.618l1.633-1.633-.707-.707-2.121 2.121L3 8.188v.568L5.544 11.3l.707-.707-1.61-1.609H11.5a1.5 1.5 0 0 0 1.5-1.5V5.5h-1Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscNoNewline = function VscNoNewline(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2.333 5.506a3 3 0 1 1 3.334 4.989 3 3 0 0 1-3.334-4.99zm2.677.777A1.986 1.986 0 0 0 2 8.009c.004.353.102.698.283 1.001L5.01 6.283zM2.99 9.717A1.986 1.986 0 0 0 6 7.991a1.988 1.988 0 0 0-.283-1.001L2.99 9.717zM14 5v1.984a.5.5 0 0 1-.5.5H9.367L11 5.851l-.707-.707-2.121 2.121-.423.423v.568l2.544 2.544.707-.707-1.61-1.609h4.11a1.5 1.5 0 0 0 1.5-1.5V5h-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscNote = function VscNote(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 2h13l.5.5v10l-.5.5h-13l-.5-.5v-10l.5-.5zM2 3v9h12V3H2zm2 2h8v1H4V5zm6 2H4v1h6V7zM4 9h4v1H4V9z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscNotebookTemplate = function VscNotebookTemplate(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M1 5H0V4h1v1zm0 2H0V6h1v1zm0 2H0V8h1v1zm0 2H0v-1h1v1zm0 2H0v-1h1v1zm0 1v1H0v-1h1zm0 1h1v1H1v-1zm2 0h1v1H3v-1zM1 1H0V0h1v1zm2 0H2V0h1v1zm1-1h1v1H4V0zm3 1H6V0h1v1zm2 0H8V0h1v1zm2 0h-1V0h1v1zm0 1V1h1v1h-1zm1 2h-1V3h1v1zM1 3H0V2h1v1z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m5 6 1-1h7l1 1v9l-1 1H6l-1-1V6zm1 0v9h7V6H6z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    d: "M15 7h1v2h-1V7zm0 3h1v2h-1v-2zm0 3h1v2h-1v-2zM7 8h5v1H7z"
  }));
};

module.exports.VscNotebook = function VscNotebook(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m2 2 1-1h9l1 1v12l-1 1H3l-1-1V2zm1 0v12h9V2H3zm1 2 1-1h5l1 1v1l-1 1H5L4 5V4zm1 0v1h5V4H5zm10 1h-1v2h1V5zm-1 3h1v2h-1V8zm1 3h-1v2h1v-2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscOctoface = function VscOctoface(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13.863 5.673c.113-.28.48-1.392-.114-2.897 0 0-.919-.288-3.01 1.138-.875-.245-1.812-.28-2.739-.28-.928 0-1.864.035-2.739.28-2.091-1.435-3.01-1.138-3.01-1.138-.595 1.505-.227 2.617-.113 2.897C1.428 6.433 1 7.413 1 8.603c0 4.507 2.914 5.522 6.982 5.522 4.07 0 7.018-1.015 7.018-5.521 0-1.19-.429-2.17-1.137-2.931zM8 13.268c-2.888 0-5.232-.132-5.232-2.932 0-.665.332-1.295.892-1.811.936-.857 2.537-.402 4.34-.402 1.811 0 3.395-.455 4.34.402.569.516.893 1.138.893 1.811 0 2.791-2.346 2.931-5.233 2.931zM5.804 8.883c-.578 0-1.05.7-1.05 1.557 0 .858.472 1.566 1.05 1.566.577 0 1.05-.7 1.05-1.566 0-.866-.473-1.557-1.05-1.557zm4.392 0c-.577 0-1.05.691-1.05 1.557s.473 1.566 1.05 1.566c.578 0 1.05-.7 1.05-1.566 0-.866-.463-1.557-1.05-1.557z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscOpenPreview = function VscOpenPreview(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3 1h11l1 1v5.3a3.21 3.21 0 0 0-1-.3V2H9v10.88L7.88 14H3l-1-1V2l1-1zm0 12h5V2H3v11zm10.379-4.998a2.53 2.53 0 0 0-1.19.348h-.03a2.51 2.51 0 0 0-.799 3.53L9 14.23l.71.71 2.35-2.36c.325.22.7.358 1.09.4a2.47 2.47 0 0 0 1.14-.13 2.51 2.51 0 0 0 1-.63 2.46 2.46 0 0 0 .58-1 2.63 2.63 0 0 0 .07-1.15 2.53 2.53 0 0 0-1.35-1.81 2.53 2.53 0 0 0-1.211-.258zm.24 3.992a1.5 1.5 0 0 1-.979-.244 1.55 1.55 0 0 1-.56-.68 1.49 1.49 0 0 1-.08-.86 1.49 1.49 0 0 1 1.18-1.18 1.49 1.49 0 0 1 .86.08c.276.117.512.311.68.56a1.5 1.5 0 0 1-1.1 2.324z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscOrganization = function VscOrganization(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M9.111 4.663A2 2 0 1 1 6.89 1.337a2 2 0 0 1 2.222 3.326zm-.555-2.494A1 1 0 1 0 7.444 3.83a1 1 0 0 0 1.112-1.66zm2.61.03a1.494 1.494 0 0 1 1.895.188 1.513 1.513 0 0 1-.487 2.46 1.492 1.492 0 0 1-1.635-.326 1.512 1.512 0 0 1 .228-2.321zm.48 1.61a.499.499 0 1 0 .705-.708.509.509 0 0 0-.351-.15.499.499 0 0 0-.5.503.51.51 0 0 0 .146.356zM3.19 12.487H5v1.005H3.19a1.197 1.197 0 0 1-.842-.357 1.21 1.21 0 0 1-.348-.85v-1.81a.997.997 0 0 1-.71-.332A1.007 1.007 0 0 1 1 9.408V7.226c.003-.472.19-.923.52-1.258.329-.331.774-.52 1.24-.523H4.6a2.912 2.912 0 0 0-.55 1.006H2.76a.798.798 0 0 0-.54.232.777.777 0 0 0-.22.543v2.232h1v2.826a.202.202 0 0 0 .05.151.24.24 0 0 0 .14.05zm7.3-6.518a1.765 1.765 0 0 0-1.25-.523H6.76a1.765 1.765 0 0 0-1.24.523c-.33.335-.517.786-.52 1.258v3.178a1.06 1.06 0 0 0 .29.734 1 1 0 0 0 .71.332v2.323a1.202 1.202 0 0 0 .35.855c.18.168.407.277.65.312h2a1.15 1.15 0 0 0 1-1.167V11.47a.997.997 0 0 0 .71-.332 1.006 1.006 0 0 0 .29-.734V7.226a1.8 1.8 0 0 0-.51-1.258zM10 10.454H9v3.34a.202.202 0 0 1-.06.14.17.17 0 0 1-.14.06H7.19a.21.21 0 0 1-.2-.2v-3.34H6V7.226c0-.203.079-.398.22-.543a.798.798 0 0 1 .54-.232h2.48a.778.778 0 0 1 .705.48.748.748 0 0 1 .055.295v3.228zm2.81 3.037H11v-1.005h1.8a.24.24 0 0 0 .14-.05.2.2 0 0 0 .06-.152V9.458h1V7.226a.777.777 0 0 0-.22-.543.798.798 0 0 0-.54-.232h-1.29a2.91 2.91 0 0 0-.55-1.006h1.84a1.77 1.77 0 0 1 1.24.523c.33.335.517.786.52 1.258v2.182c0 .273-.103.535-.289.733-.186.199-.44.318-.711.333v1.81c0 .319-.125.624-.348.85a1.197 1.197 0 0 1-.842.357zM4 1.945a1.494 1.494 0 0 0-1.386.932A1.517 1.517 0 0 0 2.94 4.52 1.497 1.497 0 0 0 5.5 3.454c0-.4-.158-.784-.44-1.067A1.496 1.496 0 0 0 4 1.945zm0 2.012a.499.499 0 0 1-.5-.503.504.504 0 0 1 .5-.503.509.509 0 0 1 .5.503.504.504 0 0 1-.5.503z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscOutput = function VscOutput(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M19.5 0v1.5L21 3v19.5L19.5 24h-15L3 22.5V3l1.5-1.5V0H6v1.5h3V0h1.5v1.5h3V0H15v1.5h3V0h1.5zm-15 22.5h15V3h-15v19.5zM7.5 6h9v1.5h-9V6zm9 6h-9v1.5h9V12zm-9 6h9v1.5h-9V18z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscPackage = function VscPackage(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m8.61 3 5.74 1.53L15 5v6.74l-.37.48-6.13 1.69-6.14-1.69-.36-.48V5l.61-.47L8.34 3h.27zm-.09 1-4 1 .55.2 3.43.9 3-.81.95-.29-3.93-1zM3 11.36l5 1.37V7L3 5.66v5.7zM9 7v5.73l5-1.37V5.63l-2.02.553V8.75l-1 .26V6.457L9 7z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscPaintcan = function VscPaintcan(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m14.54 11.811-1.14-3.12v-.06l-4.91-4.91v-1.24a1.66 1.66 0 0 0-.11-.58 1.48 1.48 0 0 0-.83-.8 1.42 1.42 0 0 0-.58-.1 1.47 1.47 0 0 0-1.48 1.48v3.26l-3.06 3a1.52 1.52 0 0 0 0 2.12l3.63 3.63c.14.141.307.253.49.33a1.53 1.53 0 0 0 1.14 0 1.51 1.51 0 0 0 .49-.33l4.93-4.92-.66 2.2a1.19 1.19 0 0 0 0 .46c.033.152.098.296.19.42.098.121.216.223.35.3.14.07.294.11.45.12a1 1 0 0 0 .48-.09 1.14 1.14 0 0 0 .39-.29.98.98 0 0 0 .22-.44c.032-.145.035-.294.01-.44zm-8-9.33a.46.46 0 0 1 0-.2.52.52 0 0 1 .12-.17.64.64 0 0 1 .18-.1.5.5 0 0 1 .21 0 .5.5 0 0 1 .32.15.5.5 0 0 1 .12.33v1.26l-1 1 .05-2.27zm1 11.35a.36.36 0 0 1-.16.11.47.47 0 0 1-.38 0 .361.361 0 0 1-.16-.11l-3.63-3.62a.5.5 0 0 1 0-.71l4.35-4.35v2.85a.74.74 0 0 0-.24.55.75.75 0 1 0 1.17-.55v-2.83l3.85 3.87-4.8 4.79z"
  }));
};

module.exports.VscPassFilled = function VscPassFilled(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 15A7 7 0 1 0 8 1a7 7 0 0 0 0 14zm-1.02-4.13h-.71L4 8.6l.71-.71 1.92 1.92 4.2-4.21.71.71-4.56 4.56z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscPass = function VscPass(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M6.27 10.87h.71l4.56-4.56-.71-.71-4.2 4.21-1.92-1.92L4 8.6l2.27 2.27z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8.6 1c1.6.1 3.1.9 4.2 2 1.3 1.4 2 3.1 2 5.1 0 1.6-.6 3.1-1.6 4.4-1 1.2-2.4 2.1-4 2.4-1.6.3-3.2.1-4.6-.7-1.4-.8-2.5-2-3.1-3.5C.9 9.2.8 7.5 1.3 6c.5-1.6 1.4-2.9 2.8-3.8C5.4 1.3 7 .9 8.6 1zm.5 12.9c1.3-.3 2.5-1 3.4-2.1.8-1.1 1.3-2.4 1.2-3.8 0-1.6-.6-3.2-1.7-4.3-1-1-2.2-1.6-3.6-1.7-1.3-.1-2.7.2-3.8 1-1.1.8-1.9 1.9-2.3 3.3-.4 1.3-.4 2.7.2 4 .6 1.3 1.5 2.3 2.7 3 1.2.7 2.6.9 3.9.6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscPersonAdd = function VscPersonAdd(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13 10h-1v2h-2v1h2v2h1v-2h2v-1h-2v-2zM8.556 2.169a1 1 0 1 0-1.112 1.663 1 1 0 0 0 1.112-1.663zm-1.667-.832A2 2 0 1 1 9.11 4.663a2 2 0 0 1-2.22-3.326zM6.77 5.49h2.46A1.77 1.77 0 0 1 11 7.26V8h-1v-.74a.76.76 0 0 0-.77-.77H6.77a.76.76 0 0 0-.77.77V10h1v3.31a.2.2 0 0 0 .2.2H8v1.02h-.8a1.2 1.2 0 0 1-1.2-1.2V11a1.06 1.06 0 0 1-1-1.1V7.26a1.77 1.77 0 0 1 1.77-1.77z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscPerson = function VscPerson(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8 2a1 1 0 1 1 0 2 1 1 0 0 1 0-2zm0-1a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm1.23 4.49H6.77A1.77 1.77 0 0 0 5 7.26V9.9A1.06 1.06 0 0 0 6 11v2.33a1.2 1.2 0 0 0 1.2 1.2h1.6a1.2 1.2 0 0 0 1.2-1.24V11a1.06 1.06 0 0 0 1-1.1V7.26a1.77 1.77 0 0 0-1.77-1.77zM6 10V7.26a.76.76 0 0 1 .77-.77h2.46a.76.76 0 0 1 .77.77V10H9v3.31a.2.2 0 0 1-.2.2H7.2a.2.2 0 0 1-.2-.2V10H6z"
  }));
};

module.exports.VscPieChart = function VscPieChart(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M10 6h3.9A5.006 5.006 0 0 0 10 2.1V6Zm0-4.917A6.005 6.005 0 0 1 15 7H9V1c.34 0 .675.028 1 .083ZM7 8l1 1h4.9A5.002 5.002 0 0 1 3 8a5.002 5.002 0 0 1 4-4.9V8Zm1 6a6.002 6.002 0 0 0 6-6H8V2a6.002 6.002 0 0 0 0 12Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscPin = function VscPin(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14 5v7h-.278c-.406 0-.778-.086-1.117-.258A2.528 2.528 0 0 1 11.73 11H8.87a3.463 3.463 0 0 1-.546.828 3.685 3.685 0 0 1-.735.633c-.27.177-.565.31-.882.398a3.875 3.875 0 0 1-.985.141h-.5V9H2l-1-.5L2 8h3.222V4h.5c.339 0 .664.047.977.14.312.094.607.227.883.4A3.404 3.404 0 0 1 8.87 6h2.859a2.56 2.56 0 0 1 .875-.734c.338-.172.71-.26 1.117-.266H14zm-.778 1.086a1.222 1.222 0 0 0-.32.156 1.491 1.491 0 0 0-.43.461L12.285 7H8.183l-.117-.336a2.457 2.457 0 0 0-.711-1.047C7.027 5.331 6.427 5.09 6 5v7c.427-.088 1.027-.33 1.355-.617.328-.287.565-.636.71-1.047L8.184 10h4.102l.18.297c.057.094.122.177.195.25.073.073.153.143.242.21.088.069.195.12.32.157V6.086z"
  }));
};

module.exports.VscPinnedDirty = function VscPinnedDirty(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4 2h7v.278c0 .406-.086.778-.258 1.117-.172.339-.42.63-.742.875v2.86c.307.145.583.328.828.546a3.7 3.7 0 0 1 .54.598 4.92 4.92 0 0 0-.896.412l-.007.004-.03.018a2.456 2.456 0 0 0-1.099-.774L9 7.817V3.715l.297-.18c.094-.057.177-.122.25-.195a2.28 2.28 0 0 0 .21-.242.968.968 0 0 0 .157-.32H5.086c.042.125.094.232.156.32a1.494 1.494 0 0 0 .461.43L6 3.715v4.102l-.336.117c-.411.146-.76.383-1.047.711C4.331 8.973 4.09 9.573 4 10h5.002a5.025 5.025 0 0 0-.481.778H8V14l-.5 1-.5-1v-3.222H3v-.5c0-.339.047-.664.14-.977.094-.312.227-.607.4-.883A3.404 3.404 0 0 1 5 7.13V4.27a2.561 2.561 0 0 1-.734-.875A2.505 2.505 0 0 1 4 2.278V2zm7.485 8.41a2.924 2.924 0 0 1 .718-.302c.256-.072.522-.108.797-.108s.541.036.797.108a2.956 2.956 0 0 1 1.321.773 2.956 2.956 0 0 1 .774 1.322c.072.256.108.522.108.797s-.036.541-.108.797a2.953 2.953 0 0 1-.774 1.324 3.013 3.013 0 0 1-1.321.774c-.256.07-.522.105-.797.105s-.541-.035-.797-.105a3.037 3.037 0 0 1-1.324-.774 3.037 3.037 0 0 1-.773-1.324A2.994 2.994 0 0 1 10 13c0-.275.035-.541.105-.797a3.013 3.013 0 0 1 .883-1.425c.154-.14.32-.262.497-.368z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscPinned = function VscPinned(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M4 2h7v.278c0 .406-.086.778-.258 1.117-.172.339-.42.63-.742.875v2.86c.307.145.583.328.828.546.245.219.456.464.633.735.177.27.31.565.398.882.089.318.136.646.141.985v.5H8V14l-.5 1-.5-1v-3.222H3v-.5c0-.339.047-.664.14-.977.094-.312.227-.607.4-.883A3.404 3.404 0 0 1 5 7.13V4.27a2.561 2.561 0 0 1-.734-.875A2.505 2.505 0 0 1 4 2.278V2zm1.086.778c.042.125.094.232.156.32a1.494 1.494 0 0 0 .461.43L6 3.715v4.102l-.336.117c-.411.146-.76.383-1.047.711C4.331 8.973 4.09 9.573 4 10h7c-.088-.427-.33-1.027-.617-1.355a2.456 2.456 0 0 0-1.047-.71L9 7.816V3.715l.297-.18c.094-.057.177-.122.25-.195a2.28 2.28 0 0 0 .21-.242.968.968 0 0 0 .157-.32H5.086z"
  }));
};

module.exports.VscPlayCircle = function VscPlayCircle(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8.6 1c1.6.1 3.1.9 4.2 2 1.3 1.4 2 3.1 2 5.1 0 1.6-.6 3.1-1.6 4.4-1 1.2-2.4 2.1-4 2.4-1.6.3-3.2.1-4.6-.7-1.4-.8-2.5-2-3.1-3.5C.9 9.2.8 7.5 1.3 6c.5-1.6 1.4-2.9 2.8-3.8C5.4 1.3 7 .9 8.6 1zm.5 12.9c1.3-.3 2.5-1 3.4-2.1.8-1.1 1.3-2.4 1.2-3.8 0-1.6-.6-3.2-1.7-4.3-1-1-2.2-1.6-3.6-1.7-1.3-.1-2.7.2-3.8 1-1.1.8-1.9 1.9-2.3 3.3-.4 1.3-.4 2.7.2 4 .6 1.3 1.5 2.3 2.7 3 1.2.7 2.6.9 3.9.6z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m6 5 .777-.416 4.5 3v.832l-4.5 3L6 11V5zm1 .934v4.132L10.099 8 7 5.934z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscPlay = function VscPlay(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M3.78 2 3 2.41v12l.78.42 9-6V8l-9-6zM4 13.48V3.35l7.6 5.07L4 13.48z"
  }));
};

module.exports.VscPlug = function VscPlug(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7 1H6v3H4.5l-.5.5V8a4 4 0 0 0 3.5 3.969V15h1v-3.031A4 4 0 0 0 12 8V4.5l-.5-.5H10V1H9v3H7V1zm3.121 9.121A3 3 0 0 1 5 8V5h6v3a3 3 0 0 1-.879 2.121z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscPreserveCase = function VscPreserveCase(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8.514 11h-1l-.816-2.16H3.433L2.664 11H1.66l2.954-7.702h.935L8.514 11ZM6.403 8.03 5.194 4.748a3.144 3.144 0 0 1-.118-.516h-.021c-.036.219-.077.39-.124.516L3.733 8.03h2.67ZM9.597 11V3.298h2.192c.666 0 1.194.163 1.584.489.39.325.586.75.586 1.273 0 .436-.119.816-.355 1.138a1.911 1.911 0 0 1-.977.688v.021c.519.061.934.258 1.246.591.311.33.467.76.467 1.29 0 .658-.236 1.191-.71 1.6-.472.408-1.068.612-1.788.612H9.597Zm.903-6.886v2.487h.923c.495 0 .883-.118 1.166-.354.283-.24.424-.577.424-1.01 0-.749-.492-1.123-1.477-1.123H10.5Zm0 3.298v2.772h1.224c.53 0 .94-.126 1.23-.376.294-.251.44-.595.44-1.032 0-.91-.619-1.364-1.858-1.364H10.5Z"
  }));
};

module.exports.VscPreview = function VscPreview(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2 2h12l1 1v10l-1 1H2l-1-1V3l1-1zm0 11h12V3H2v10zm11-9H3v3h10V4zm-1 2H4V5h8v1zm-3 6h4V8H9v4zm1-3h2v2h-2V9zM7 8H3v1h4V8zm-4 3h4v1H3v-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscPrimitiveSquare = function VscPrimitiveSquare(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m3.5 4 .5-.5h8l.5.5v8l-.5.5H4l-.5-.5V4zm1 .5v7h7v-7h-7z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscProject = function VscProject(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 1h13l.5.5v13l-.5.5h-13l-.5-.5v-13l.5-.5zM2 14h12V2H2v12zM3 3h2v10H3V3zm6 0H7v6h2V3zm2 0h2v8h-2V3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscPulse = function VscPulse(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M11.8 9 10 3H9L7.158 9.64 5.99 4.69h-.97L3.85 9H1v.99h3.23l.49-.37.74-2.7L6.59 12h1.03l1.87-7.04 1.46 4.68.48.36H15V9h-3.2z"
  }));
};

module.exports.VscQuestion = function VscQuestion(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.5 1a6.5 6.5 0 1 0 0 13 6.5 6.5 0 0 0 0-13zm0 12a5.5 5.5 0 1 1 0-11 5.5 5.5 0 0 1 0 11zm1.55-8.42a1.84 1.84 0 0 0-.61-.42A2.25 2.25 0 0 0 7.53 4a2.16 2.16 0 0 0-.88.17c-.239.1-.45.254-.62.45a1.89 1.89 0 0 0-.38.62 3 3 0 0 0-.15.72h1.23a.84.84 0 0 1 .506-.741.72.72 0 0 1 .304-.049.86.86 0 0 1 .27 0 .64.64 0 0 1 .22.14.6.6 0 0 1 .16.22.73.73 0 0 1 .06.3c0 .173-.037.343-.11.5a2.4 2.4 0 0 1-.27.46l-.35.42c-.12.13-.24.27-.35.41a2.33 2.33 0 0 0-.27.45 1.18 1.18 0 0 0-.1.5v.66H8v-.49a.94.94 0 0 1 .11-.42 3.09 3.09 0 0 1 .28-.41l.36-.44a4.29 4.29 0 0 0 .36-.48 2.59 2.59 0 0 0 .28-.55 1.91 1.91 0 0 0 .11-.64 2.18 2.18 0 0 0-.1-.67 1.52 1.52 0 0 0-.35-.55zM6.8 9.83h1.17V11H6.8V9.83z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscQuote = function VscQuote(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.16 3.5C4.73 5.06 3.55 6.67 3.55 9.36c.16-.05.3-.05.44-.05 1.27 0 2.5.86 2.5 2.41 0 1.61-1.03 2.61-2.5 2.61-1.9 0-2.99-1.52-2.99-4.25 0-3.8 1.75-6.53 5.02-8.42L7.16 3.5zm7 0c-2.43 1.56-3.61 3.17-3.61 5.86.16-.05.3-.05.44-.05 1.27 0 2.5.86 2.5 2.41 0 1.61-1.03 2.61-2.5 2.61-1.89 0-2.98-1.52-2.98-4.25 0-3.8 1.75-6.53 5.02-8.42l1.14 1.84h-.01z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRadioTower = function VscRadioTower(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2.998 5.58a5.55 5.55 0 0 1 1.62-3.88l-.71-.7a6.45 6.45 0 0 0 0 9.16l.71-.7a5.55 5.55 0 0 1-1.62-3.88zm1.06 0a4.42 4.42 0 0 0 1.32 3.17l.71-.71a3.27 3.27 0 0 1-.76-1.12 3.45 3.45 0 0 1 0-2.67 3.22 3.22 0 0 1 .76-1.13l-.71-.71a4.46 4.46 0 0 0-1.32 3.17zm7.65 3.21-.71-.71c.33-.32.59-.704.76-1.13a3.449 3.449 0 0 0 0-2.67 3.22 3.22 0 0 0-.76-1.13l.71-.7a4.468 4.468 0 0 1 0 6.34zM13.068 1l-.71.71a5.43 5.43 0 0 1 0 7.74l.71.71a6.45 6.45 0 0 0 0-9.16zM9.993 5.43a1.5 1.5 0 0 1-.245.98 2 2 0 0 1-.27.23l3.44 7.73-.92.4-.77-1.73h-5.54l-.77 1.73-.92-.4 3.44-7.73a1.52 1.52 0 0 1-.33-1.63 1.55 1.55 0 0 1 .56-.68 1.5 1.5 0 0 1 2.325 1.1zm-1.595-.34a.52.52 0 0 0-.25.14.52.52 0 0 0-.11.22.48.48 0 0 0 0 .29c.04.09.102.17.18.23a.54.54 0 0 0 .28.08.51.51 0 0 0 .5-.5.54.54 0 0 0-.08-.28.58.58 0 0 0-.23-.18.48.48 0 0 0-.29 0zm.23 2.05h-.27l-.87 1.94h2l-.86-1.94zm2.2 4.94-.89-2h-2.88l-.89 2h4.66z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscReactions = function VscReactions(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M12 7.5c0 .169-.01.336-.027.5h1.005A5.5 5.5 0 1 0 8 12.978v-1.005A4.5 4.5 0 1 1 12 7.5zM5.5 7a1 1 0 1 0 0-2 1 1 0 0 0 0 2zm2 2.5c.712 0 1.355-.298 1.81-.776l.707.708A3.49 3.49 0 0 1 7.5 10.5a3.49 3.49 0 0 1-2.555-1.108l.707-.708A2.494 2.494 0 0 0 7.5 9.5zm2-2.5a1 1 0 1 0 0-2 1 1 0 0 0 0 2zm2.5 3h1v2h2v1h-2v2h-1v-2h-2v-1h2v-2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRecordKeys = function VscRecordKeys(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14 3H3a1 1 0 0 0-1 1v7a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1V4a1 1 0 0 0-1-1zm0 8H3V4h11v7zm-3-6h-1v1h1V5zm-1 2H9v1h1V7zm2-2h1v1h-1V5zm1 4h-1v1h1V9zM6 9h5v1H6V9zm7-2h-2v1h2V7zM8 5h1v1H8V5zm0 2H7v1h1V7zM4 9h1v1H4V9zm0-4h1v1H4V5zm3 0H6v1h1V5zM4 7h2v1H4V7z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRecord = function VscRecord(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8 10a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8.6 1c1.6.1 3.1.9 4.2 2 1.3 1.4 2 3.1 2 5.1 0 1.6-.6 3.1-1.6 4.4-1 1.2-2.4 2.1-4 2.4-1.6.3-3.2.1-4.6-.7-1.4-.8-2.5-2-3.1-3.5C.9 9.2.8 7.5 1.3 6c.5-1.6 1.4-2.9 2.8-3.8C5.4 1.3 7 .9 8.6 1zm.5 12.9c1.3-.3 2.5-1 3.4-2.1.8-1.1 1.3-2.4 1.2-3.8 0-1.6-.6-3.2-1.7-4.3-1-1-2.2-1.6-3.6-1.7-1.3-.1-2.7.2-3.8 1-1.1.8-1.9 1.9-2.3 3.3-.4 1.3-.4 2.7.2 4 .6 1.3 1.5 2.3 2.7 3 1.2.7 2.6.9 3.9.6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRedo = function VscRedo(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M12.5 2v3.5L12 6H8.5V5h2.521l-.941-.941a3.552 3.552 0 1 0-5.023 5.023l5.197 5.198-.72.72-5.198-5.198A4.57 4.57 0 0 1 10.8 3.339l.7.7V2h1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscReferences = function VscReferences(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m11.105 4.561-3.43 3.427-1.134-1.12 2.07-2.08h-4.8a2.4 2.4 0 1 0 0 4.8h.89v1.6h-.88a4 4 0 0 1 0-7.991h4.8L6.54 1.13 7.675 0l3.43 3.432v1.13zM16.62 24h-9.6l-.8-.8V10.412l.8-.8h9.6l.8.8V23.2l-.8.8zm-8.8-1.6h8V11.212h-8V22.4zm5.6-20.798h9.6l.8.8v12.786l-.8.8h-4v-1.6h3.2V3.2h-8v4.787h-1.6V2.401l.8-.8zm.8 11.186h-4.8v1.6h4.8v-1.6zm-4.8 3.2h4.8v1.6h-4.8v-1.6zm4.8 3.2h-4.8v1.6h4.8v-1.6zm1.6-14.4h4.8v1.6h-4.8v-1.6zm4.8 6.4h-1.6v1.6h1.6v-1.6zm-3.338-3.176v-.024h3.338v1.6h-1.762l-1.576-1.576z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRefresh = function VscRefresh(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4.681 3H2V2h3.5l.5.5V6H5V4a5 5 0 1 0 4.53-.761l.302-.954A6 6 0 1 1 4.681 3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRegex = function VscRegex(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M10.012 2h.976v3.113l2.56-1.557.486.885L11.47 6l2.564 1.559-.485.885-2.561-1.557V10h-.976V6.887l-2.56 1.557-.486-.885L9.53 6 6.966 4.441l.485-.885 2.561 1.557V2zM2 10h4v4H2v-4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRemoteExplorer = function VscRemoteExplorer(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.344 2.125h20.312l.782.781v8.599a7.825 7.825 0 0 0-1.563-.912V3.688H2.125V17.75h7.813a7.813 7.813 0 0 0 1.562 4.688H5.25v-1.563h4.688v-1.563H1.344l-.782-.78V2.905l.782-.781zM17.75 11.5a6.25 6.25 0 1 0 0 12.5 6.25 6.25 0 0 0 0-12.5zm0 10.938a4.688 4.688 0 1 1 0-9.377 4.688 4.688 0 0 1 0 9.377zm2.603-3.132L18.2 17.153 20.353 15l.647.646-1.506 1.507L21 18.659l-.647.647zM15 17.246l1.506 1.507L15 20.259l.647.647 2.153-2.153-2.153-2.153-.647.646z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRemote = function VscRemote(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M12.904 9.57 8.928 5.596l3.976-3.976-.619-.62L8 5.286v.619l4.285 4.285.62-.618zM3 5.62l4.072 4.07L3 13.763l.619.618L8 10v-.619L3.619 5 3 5.619z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRemove = function VscRemove(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M15 8H1V7h14v1z"
  }));
};

module.exports.VscReplaceAll = function VscReplaceAll(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.6 2.677c.147-.31.356-.465.626-.465.248 0 .44.118.573.353.134.236.201.557.201.966 0 .443-.078.798-.235 1.067-.156.268-.365.402-.627.402-.237 0-.416-.125-.537-.374h-.008v.31H11V1h.593v1.677h.008zm-.016 1.1a.78.78 0 0 0 .107.426c.071.113.163.169.274.169.136 0 .24-.072.314-.216.075-.145.113-.35.113-.615 0-.22-.035-.39-.104-.514-.067-.124-.164-.187-.29-.187-.12 0-.219.062-.297.185a.886.886 0 0 0-.117.48v.272zM4.12 7.695 2 5.568l.662-.662 1.006 1v-1.51A1.39 1.39 0 0 1 5.055 3H7.4v.905H5.055a.49.49 0 0 0-.468.493l.007 1.5.949-.944.656.656-2.08 2.085zM9.356 4.93H10V3.22C10 2.408 9.685 2 9.056 2c-.135 0-.285.024-.45.073a1.444 1.444 0 0 0-.388.167v.665c.237-.203.487-.304.75-.304.261 0 .392.156.392.469l-.6.103c-.506.086-.76.406-.76.961 0 .263.061.473.183.631A.61.61 0 0 0 8.69 5c.29 0 .509-.16.657-.48h.009v.41zm.004-1.355v.193a.75.75 0 0 1-.12.436.368.368 0 0 1-.313.17.276.276 0 0 1-.22-.095.38.38 0 0 1-.08-.248c0-.222.11-.351.332-.389l.4-.067zM7 12.93h-.644v-.41h-.009c-.148.32-.367.48-.657.48a.61.61 0 0 1-.507-.235c-.122-.158-.183-.368-.183-.63 0-.556.254-.876.76-.962l.6-.103c0-.313-.13-.47-.392-.47-.263 0-.513.102-.75.305v-.665c.095-.063.224-.119.388-.167.165-.049.315-.073.45-.073.63 0 .944.407.944 1.22v1.71zm-.64-1.162v-.193l-.4.068c-.222.037-.333.166-.333.388 0 .1.027.183.08.248a.276.276 0 0 0 .22.095.368.368 0 0 0 .312-.17c.08-.116.12-.26.12-.436zM9.262 13c.321 0 .568-.058.738-.173v-.71a.9.9 0 0 1-.552.207.619.619 0 0 1-.5-.215c-.12-.145-.181-.345-.181-.598 0-.26.063-.464.189-.612a.644.644 0 0 1 .516-.223c.194 0 .37.069.528.207v-.749c-.129-.09-.338-.134-.626-.134-.417 0-.751.14-1.001.422-.249.28-.373.662-.373 1.148 0 .42.116.764.349 1.03.232.267.537.4.913.4zM2 9l1-1h9l1 1v5l-1 1H3l-1-1V9zm1 0v5h9V9H3zm3-2 1-1h7l1 1v5l-1 1V7H6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscReplace = function VscReplace(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m3.221 3.739 2.261 2.269L7.7 3.784l-.7-.7-1.012 1.007-.008-1.6a.523.523 0 0 1 .5-.526H8V1H6.48A1.482 1.482 0 0 0 5 2.489V4.1L3.927 3.033l-.706.706zm6.67 1.794h.01c.183.311.451.467.806.467.393 0 .706-.168.94-.503.236-.335.353-.78.353-1.333 0-.511-.1-.913-.301-1.207-.201-.295-.488-.442-.86-.442-.405 0-.718.194-.938.581h-.01V1H9v4.919h.89v-.386zm-.015-1.061v-.34c0-.248.058-.448.175-.601a.54.54 0 0 1 .445-.23.49.49 0 0 1 .436.233c.104.154.155.368.155.643 0 .33-.056.587-.169.768a.524.524 0 0 1-.47.27.495.495 0 0 1-.411-.211.853.853 0 0 1-.16-.532zM9 12.769c-.256.154-.625.231-1.108.231-.563 0-1.02-.178-1.369-.533-.349-.355-.523-.813-.523-1.374 0-.648.186-1.158.56-1.53.374-.376.875-.563 1.5-.563.433 0 .746.06.94.179v.998a1.26 1.26 0 0 0-.792-.276c-.325 0-.583.1-.774.298-.19.196-.283.468-.283.816 0 .338.09.603.272.797.182.191.431.287.749.287.282 0 .558-.092.828-.276v.946zM4 7 3 8v6l1 1h7l1-1V8l-1-1H4zm0 1h7v6H4V8z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscReply = function VscReply(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m6.306 2.146-4.02 4.02v.708l4.02 4.02.708-.707L3.807 6.98H5.69c2.813 0 4.605.605 5.705 1.729 1.102 1.125 1.615 2.877 1.615 5.421v.35h1v-.35c0-2.646-.527-4.72-1.9-6.121C10.735 6.605 8.617 5.98 5.69 5.98H3.887l3.127-3.126-.708-.708z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRepoClone = function VscRepoClone(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13 10H4V2h4V1H3.74a1.9 1.9 0 0 0-.67.13 1.66 1.66 0 0 0-.57.41 1.73 1.73 0 0 0-.37.59 1.68 1.68 0 0 0-.13.62v9.5a1.75 1.75 0 0 0 1.07 1.62 1.9 1.9 0 0 0 .67.13H4v-1h-.26a.72.72 0 0 1-.29-.06.78.78 0 0 1-.4-.4.93.93 0 0 1 0-.29v-.5a.93.93 0 0 1 0-.29.78.78 0 0 1 .4-.4.72.72 0 0 1 .29-.06H13v2H9v1h4.5l.5-.5V9h-1v1zM6 3H5v1h1V3zM5 5h1v1H5V5zm0 2h1v1H5V7zm.28 8H5v-3h3v3h-.28L6.5 13.49 5.28 15zM10 1h4.5l.5.5v6l-.5.5H12v1h-1V8h-1a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1zm.5 6h.5V6h-.5a.5.5 0 0 0 0 1zM12 7h2V6h-2v1zm-1-2h3V2h-3v3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRepoForcePush = function VscRepoForcePush(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3.74 1h9.76l.5.5v12l-.5.5H10v-1h3v-2h-3v-1h3V2H4v8h3v1H3.74a.74.74 0 0 0-.74.75v.5a.74.74 0 0 0 .74.75H7v1H3.74A1.74 1.74 0 0 1 2 12.25v-9.5A1.74 1.74 0 0 1 3.74 1zm1.6 4.83.71.7L8 4.58v1.45L5.38 8.65l.71.7 1.92-1.92V15h1V7.328l2.03 2.022.7-.7L9 5.9V4.538l2 1.992.7-.7L8.88 3h-.71L5.34 5.83z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRepoForked = function VscRepoForked(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14 4a2 2 0 1 0-2.47 1.94V7a.48.48 0 0 1-.27.44L8.49 8.88l-2.76-1.4A.49.49 0 0 1 5.46 7V5.94a2 2 0 1 0-1 0V7a1.51 1.51 0 0 0 .82 1.34L8 9.74v1.32a2 2 0 1 0 1 0V9.74l2.7-1.36A1.49 1.49 0 0 0 12.52 7V5.92A2 2 0 0 0 14 4zM4 4a1 1 0 1 1 2 0 1 1 0 0 1-2 0zm5.47 9a1 1 0 1 1-2 0 1 1 0 0 1 2 0zM12 5a1 1 0 1 1 0-2 1 1 0 0 1 0 2z"
  }));
};

module.exports.VscRepoPull = function VscRepoPull(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13 1.5V3h-1V2H3v8h10v3.5l-.5.5H8v-1h4v-2H2.735a.72.72 0 0 0-.285.06.74.74 0 0 0-.4.4.93.93 0 0 0-.05.29v.5a.93.93 0 0 0 .05.29.74.74 0 0 0 .4.4c.091.04.19.06.29.06H3v1h-.26a1.9 1.9 0 0 1-.67-.13 1.77 1.77 0 0 1-.94-.95 1.7 1.7 0 0 1-.13-.67v-9.5a1.7 1.7 0 0 1 .13-.62 1.77 1.77 0 0 1 .94-1A1.9 1.9 0 0 1 2.74 1h9.76l.5.5zM2 10.17V2.748v7.422zM5 3H4v1h1V3zm0 2H4v1h1V5zM4 7h1v1H4V7zm8.07-3.61-.7.71 1.92 1.92H7v1h6.39l-2.02 2.03.7.7 2.83-2.82v-.71l-2.83-2.83zM5.5 13.49 4.28 15H4v-3h3v3h-.28L5.5 13.49z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRepoPush = function VscRepoPush(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13.5 1H3.74A1.74 1.74 0 0 0 2 2.75v9.5A1.74 1.74 0 0 0 3.74 14H7v-1H3.74a.74.74 0 0 1-.74-.75v-.5a.74.74 0 0 1 .74-.75H7v-1H4V2h9v8h-3v1h3v2h-3v1h3.5l.5-.5v-12l-.5-.5zM3 2.73a.75.75 0 0 0 0 .02v7.42-7.44zM6 3H5v1h1V3zm-.62 5.65.71.7 1.92-1.92V15h1V7.328l2.03 2.022.7-.7-2.82-2.83h-.71L5.38 8.65zM5 5h1v1H5V5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRepo = function VscRepo(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14 10V1.5l-.5-.5H3.74a1.9 1.9 0 0 0-.67.13 1.77 1.77 0 0 0-.94 1 1.7 1.7 0 0 0-.13.62v9.5a1.7 1.7 0 0 0 .13.67c.177.427.515.768.94.95a1.9 1.9 0 0 0 .67.13H4v-1h-.26a.72.72 0 0 1-.29-.06.74.74 0 0 1-.4-.4.93.93 0 0 1-.05-.29v-.5a.93.93 0 0 1 .05-.29.74.74 0 0 1 .4-.4.72.72 0 0 1 .286-.06H13v2H9v1h4.5l.5-.5V10zM4 10V2h9v8H4zm1-7h1v1H5V3zm0 2h1v1H5V5zm1 2H5v1h1V7zm.5 6.49L5.28 15H5v-3h3v3h-.28L6.5 13.49z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscReport = function VscReport(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 1h13l.5.5v10l-.5.5H7.707l-2.853 2.854L4 14.5V12H1.5l-.5-.5v-10l.5-.5zm6 10H14V2H2v9h2.5l.5.5v1.793l2.146-2.147L7.5 11zm0-8h1v5h-1V3zm0 7h1V9h-1v1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRequestChanges = function VscRequestChanges(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14.5 1h-13l-.5.5v10l.5.5H4v2.5l.854.354L7.707 12H14.5l.5-.5v-10l-.5-.5zM14 11H7.5l-.354.146L5 13.293V11.5l-.5-.5H2V2h12v9zm-4-1H6V8.979h4V10zM7.5 3h1v2h2v1h-2v2h-1V6h-2V5h2V3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRocket = function VscRocket(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14.491 1c-3.598.004-6.654 1.983-8.835 4H1.5l-.5.5v3l.147.354.991.991.001.009 4 4 .009.001.999.999L7.5 15h3l.5-.5v-4.154c2.019-2.178 3.996-5.233 3.992-8.846l-.501-.5zM2 6h2.643a23.828 23.828 0 0 0-2.225 2.71L2 8.294V6zm5.7 8-.42-.423a23.59 23.59 0 0 0 2.715-2.216V14H7.7zm-1.143-1.144L3.136 9.437C4.128 8 8.379 2.355 13.978 2.016c-.326 5.612-5.987 9.853-7.421 10.84zM4 15v-1H2v-2H1v3h3zm6.748-7.667a1.5 1.5 0 1 0-2.496-1.666 1.5 1.5 0 0 0 2.495 1.666z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRootFolderOpened = function VscRootFolderOpened(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1 6.257V2.5l.5-.5h5l.35.15.86.85h5.79l.5.5V6h1.13l.48.63-2.63 7-.48.37H8.743a5.48 5.48 0 0 0 .657-1h2.73l2.37-6H8.743a5.534 5.534 0 0 0-.72-.724l.127-.126L8.5 6H13V4H7.5l-.35-.15L6.29 3H2l.01 2.594c-.361.184-.7.407-1.01.663z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    d: "M6 10.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 10.5a3.5 3.5 0 1 1-7 0 3.5 3.5 0 0 1 7 0zM4.5 13a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRootFolder = function VscRootFolder(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.71 3h6.79l.51.5v10l-.5.5H8.743a5.48 5.48 0 0 0 .657-1h4.59v-1.51l.01-4v-1.5H7.69l-.017.017a5.494 5.494 0 0 0-.881-.508l.348-.349.35-.15h6.5l.01-.99H7.5l-.36-.15-.85-.85H2V5.6a5.45 5.45 0 0 0-.99.649V2.5l.5-.5h5l.35.15.85.85z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    d: "M6 10.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 10.5a3.5 3.5 0 1 1-7 0 3.5 3.5 0 0 1 7 0zM4.5 13a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRss = function VscRss(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5 13H3v-2c1.11 0 2 .89 2 2zM3 3v1a9 9 0 0 1 9 9h1C13 7.48 8.52 3 3 3zm0 4v1c2.75 0 5 2.25 5 5h1c0-3.31-2.69-6-6-6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRuby = function VscRuby(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m1 7.19 6.64 6.64h.72L15 7.19v-.72l-3.32-3.32-.36-.15H4.68l-.36.15L1 6.47v.72zm7 5.56L2.08 6.83 4.89 4h6.22l2.81 2.83L8 12.75zm0-7.73h2.69l1.81 1.81-4.5 4.4V5.02z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRunAbove = function VscRunAbove(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M1.77 1.01 1 1.42v12l.78.42 9-6v-.83l-9.01-6zM2 12.49V2.36l7.6 5.07L2 12.49zM12.15 8h.71l2.5 2.5-.71.71L13 9.56V15h-1V9.55l-1.65 1.65-.7-.7 2.5-2.5z"
  }));
};

module.exports.VscRunAll = function VscRunAll(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M2.78 2 2 2.41v12l.78.42 9-6V8l-9-6zM3 13.48V3.35l7.6 5.07L3 13.48z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m6 14.683 8.78-5.853V8L6 2.147V3.35l7.6 5.07L6 13.48v1.203z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscRunBelow = function VscRunBelow(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m1.8 1.01-.78.41v12l.78.42 9-6v-.83l-9-6zm.22 11.48V2.36l7.6 5.07-7.6 5.06zM12.85 15h-.71l-2.5-2.5.71-.71L12 13.44V8h1v5.45l1.65-1.65.71.71L12.85 15z"
  }));
};

module.exports.VscRunErrors = function VscRunErrors(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5 2.41 5.78 2l9 6v.83L9 12.683v-1.2l4.6-3.063L6 3.35V7H5V2.41Z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4.872 7.808c-.85-.053-1.705.159-2.403.641-.753.485-1.233 1.184-1.499 2.03-.269.81-.213 1.72.106 2.518a3.774 3.774 0 0 0 1.658 1.873c.756.432 1.616.537 2.467.378.861-.162 1.61-.645 2.143-1.285l.005-.006c.529-.687.852-1.489.852-2.352a3.882 3.882 0 0 0-1.066-2.72l-.006-.005c-.585-.585-1.388-1.018-2.257-1.072ZM2.951 9.183c.512-.373 1.172-.517 1.792-.47h.001c.656.048 1.22.328 1.697.804.516.516.803 1.274.803 2.038v.014c.047.649-.183 1.26-.566 1.789-.426.518-.993.85-1.61.991-.61.141-1.266.047-1.83-.282-.572-.333-1-.808-1.288-1.43-.28-.607-.282-1.265-.091-1.885v-.004a2.865 2.865 0 0 1 1.092-1.565Zm1.554 1.83L3.292 9.748l-.638.637 1.22 1.27-1.22 1.27.638.637L4.505 12.3l1.213 1.264.638-.637-1.219-1.27 1.219-1.27-.638-.637-1.213 1.263Z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSaveAll = function VscSaveAll(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m14.85 2.65-1.5-1.5L13 1H4.48l-.5.5V4H1.5l-.5.5v10l.5.5h10l.5-.5V12h2.5l.5-.5V3l-.15-.35zM11 14H2V5h1v3.07h6V5h.79L11 6.21V14zM6 7V5h2v2H6zm8 4h-2V6l-.15-.35-1.5-1.5L10 4H5V2h7.81l1.21 1.21L14 11z"
  }));
};

module.exports.VscSaveAs = function VscSaveAs(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.04 1.33 12.71 3l.29.71v.33h-.5l-.5.5v-.83l-1.67-1.67H10v4H4v-4H2v10h3l-.5 1H2l-1-1v-10l1-1h8.33l.71.29zM7 5h2V2H7v3zm6.5 0L15 6.5l-.02.69-5.5 5.5-.13.12-.37.37-.1.09-3 1.5-.67-.67 1.5-3 .09-.1.37-.37.12-.13 5.5-5.5h.71zm-6.22 7.24-.52 1 1.04-.48-.52-.52zm.69-1.03.79.79 5.15-5.15-.79-.79-5.15 5.15z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSave = function VscSave(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m13.353 1.146 1.5 1.5L15 3v11.5l-.5.5h-13l-.5-.5v-13l.5-.5H13l.353.146zM2 2v12h12V3.208L12.793 2H11v4H4V2H2zm6 0v3h2V2H8z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscScreenFull = function VscScreenFull(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M3 12h10V4H3v8zm2-6h6v4H5V6zM2 6H1V2.5l.5-.5H5v1H2v3zm13-3.5V6h-1V3h-3V2h3.5l.5.5zM14 10h1v3.5l-.5.5H11v-1h3v-3zM2 13h3v1H1.5l-.5-.5V10h1v3z"
  }));
};

module.exports.VscScreenNormal = function VscScreenNormal(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M3.5 4H1V3h2V1h1v2.5l-.5.5zM13 3V1h-1v2.5l.5.5H15V3h-2zm-1 9.5V15h1v-2h2v-1h-2.5l-.5.5zM1 12v1h2v2h1v-2.5l-.5-.5H1zm11-1.5-.5.5h-7l-.5-.5v-5l.5-.5h7l.5.5v5zM10 7H6v2h4V7z"
  }));
};

module.exports.VscSearchStop = function VscSearchStop(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5.738 3.318a4.5 4.5 0 0 0-.877 5.123A4.48 4.48 0 0 0 6.1 10a4.62 4.62 0 0 0-.1 1v.17c-.16-.11-.32-.22-.47-.34L1.75 14.5 1 13.84l3.8-3.69a5.5 5.5 0 1 1 9.62-3.65c0 .268-.02.535-.06.8a5.232 5.232 0 0 0-.94-.68V6.5a4.5 4.5 0 0 0-7.682-3.182zm3.04 4.356a4 4 0 1 1 4.444 6.652 4 4 0 0 1-4.444-6.652zm.1 5.447A3 3 0 0 0 11 14a3 3 0 0 0 1.74-.55L8.55 9.26A3 3 0 0 0 8 11a3 3 0 0 0 .879 2.121zm.382-4.57 4.19 4.189A3 3 0 0 0 14 11a3 3 0 0 0-3-3 3 3 0 0 0-1.74.55z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSearch = function VscSearch(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    d: "M15.25 0a8.25 8.25 0 0 0-6.18 13.72L1 22.88l1.12 1 8.05-9.12A8.251 8.251 0 1 0 15.25.01V0zm0 15a6.75 6.75 0 1 1 0-13.5 6.75 6.75 0 0 1 0 13.5z"
  }));
};

module.exports.VscServerEnvironment = function VscServerEnvironment(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M6 3h4v1H6V3zm0 6h4v1H6V9zm0 2h4v1H6v-1zm9.14 5H.86l1.25-5H4V2a.95.95 0 0 1 .078-.383c.052-.12.123-.226.211-.32a.922.922 0 0 1 .32-.219A1.01 1.01 0 0 1 5 1h6a.95.95 0 0 1 .383.078c.12.052.226.123.32.211a.922.922 0 0 1 .219.32c.052.125.078.256.078.391v9h1.89l1.25 5zM5 13h6V2H5v11zm8.86 2-.75-3H12v2H4v-2H2.89l-.75 3h11.72z"
  }));
};

module.exports.VscServerProcess = function VscServerProcess(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 2h13l.5.5V9h-1V6H2v7h7v1H1.5l-.5-.5v-11l.5-.5zM2 5h12V3H2v2zm5 7v-1.094a1.633 1.633 0 0 1-.469-.265l-.945.539-.5-.86.937-.547a1.57 1.57 0 0 1 0-.547l-.937-.546.5-.86.945.54c.151-.12.308-.209.469-.266V7h1v1.094a1.48 1.48 0 0 1 .469.265l.945-.539.5.86-.937.547a1.57 1.57 0 0 1 0 .546l.937.547-.5.86-.945-.54a1.807 1.807 0 0 1-.469.266V12H7zm-.25-2.5c0 .208.073.385.219.531a.723.723 0 0 0 .531.219.723.723 0 0 0 .531-.219.723.723 0 0 0 .219-.531.723.723 0 0 0-.219-.531.723.723 0 0 0-.531-.219.723.723 0 0 0-.531.219.723.723 0 0 0-.219.531zm5.334 5.5v-1.094a1.634 1.634 0 0 1-.469-.265l-.945.539-.5-.86.938-.547a1.572 1.572 0 0 1 0-.547l-.938-.546.5-.86.945.54c.151-.12.308-.209.47-.266V10h1v1.094a1.486 1.486 0 0 1 .468.265l.945-.539.5.86-.937.547a1.562 1.562 0 0 1 0 .546l.937.547-.5.86-.945-.54a1.806 1.806 0 0 1-.469.266V15h-1zm-.25-2.5c0 .208.073.385.219.531a.723.723 0 0 0 .531.219.723.723 0 0 0 .531-.219.723.723 0 0 0 .22-.531.723.723 0 0 0-.22-.531.723.723 0 0 0-.53-.219.723.723 0 0 0-.532.219.723.723 0 0 0-.219.531z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscServer = function VscServer(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2.5 5 2 4.5v-3l.5-.5h11l.5.5v3l-.5.5h-11zM10 2H9v1H8V2H7v1H6V2H5v1H4V2H3v2h10V2h-2v1h-1V2zm-7.5 8L2 9.5v-3l.5-.5h11l.5.5v3l-.5.5h-11zM6 7H5v1H4V7H3v2h10V7h-2v1h-1V7H9v1H8V7H7v1H6V7zm7.5 8 .5-.5v-3l-.5-.5h-11l-.5.5v3l.5.5h11zM3 14v-2h1v1h1v-1h1v1h1v-1h1v1h1v-1h1v1h1v-1h2v2H3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSettingsGear = function VscSettingsGear(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m19.85 8.75 4.15.83v4.84l-4.15.83 2.35 3.52-3.43 3.43-3.52-2.35-.83 4.15H9.58l-.83-4.15-3.52 2.35-3.43-3.43 2.35-3.52L0 14.42V9.58l4.15-.83L1.8 5.23 5.23 1.8l3.52 2.35L9.58 0h4.84l.83 4.15 3.52-2.35 3.43 3.43-2.35 3.52zm-1.57 5.07 4-.81v-2l-4-.81-.54-1.3 2.29-3.43-1.43-1.43-3.43 2.29-1.3-.54-.81-4h-2l-.81 4-1.3.54-3.43-2.29-1.43 1.43L6.38 8.9l-.54 1.3-4 .81v2l4 .81.54 1.3-2.29 3.43 1.43 1.43 3.43-2.29 1.3.54.81 4h2l.81-4 1.3-.54 3.43 2.29 1.43-1.43-2.29-3.43.54-1.3zm-8.186-4.672A3.43 3.43 0 0 1 12 8.57 3.44 3.44 0 0 1 15.43 12a3.43 3.43 0 1 1-5.336-2.852zm.956 4.274c.281.188.612.288.95.288A1.7 1.7 0 0 0 13.71 12a1.71 1.71 0 1 0-2.66 1.422z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSettings = function VscSettings(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3.5 2h-1v5h1V2zm6.1 5H6.4L6 6.45v-1L6.4 5h3.2l.4.5v1l-.4.5zm-5 3H1.4L1 9.5v-1l.4-.5h3.2l.4.5v1l-.4.5zm3.9-8h-1v2h1V2zm-1 6h1v6h-1V8zm-4 3h-1v3h1v-3zm7.9 0h3.19l.4-.5v-.95l-.4-.5H11.4l-.4.5v.95l.4.5zm2.1-9h-1v6h1V2zm-1 10h1v2h-1v-2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscShield = function VscShield(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8.246 14.713a27.792 27.792 0 0 1-1.505-.953c-.501-.34-.983-.707-1.444-1.1-.458-.395-.888-.82-1.288-1.274-.4-.455-.753-.95-1.05-1.478a7.8 7.8 0 0 1-.7-1.69A7.041 7.041 0 0 1 2 6.3V3.1l.5-.5c.333 0 .656-.011.97-.036.296-.023.591-.066.882-.128.284-.062.562-.148.832-.256.284-.118.557-.261.816-.427a4.83 4.83 0 0 1 1.184-.565 4.8 4.8 0 0 1 2-.142 4.018 4.018 0 0 1 1.237.383c.199.097.392.204.58.322.26.167.535.31.821.428.27.109.547.194.831.256.291.062.587.106.884.129.311.024.634.035.967.035l.5.5v3.2a7.043 7.043 0 0 1-.256 1.919 7.804 7.804 0 0 1-.7 1.69 8.751 8.751 0 0 1-1.05 1.478c-.4.452-.829.877-1.286 1.27a15.94 15.94 0 0 1-1.448 1.1 28.71 28.71 0 0 1-1.51.956h-.508zM3 3.59V6.3c-.004.555.07 1.11.22 1.645a6.7 6.7 0 0 0 .61 1.473c.263.467.575.905.93 1.308.37.417.766.81 1.188 1.174.432.368.883.712 1.352 1.03.4.267.8.523 1.2.769.4-.242.8-.498 1.2-.768.47-.319.923-.663 1.355-1.031.421-.364.817-.756 1.186-1.172a7.8 7.8 0 0 0 .93-1.308c.261-.465.466-.96.61-1.473.15-.537.223-1.09.22-1.647V3.59c-.159 0-.313-.012-.465-.023l-.079-.006a7.95 7.95 0 0 1-1.018-.147 6.112 6.112 0 0 1-1.976-.814 5.166 5.166 0 0 0-.482-.27 3.123 3.123 0 0 0-.943-.29 3.686 3.686 0 0 0-1.558.106c-.332.108-.649.26-.94.452-.312.2-.64.372-.983.513a6.4 6.4 0 0 1-1 .307c-.335.07-.675.12-1.017.146-.174.01-.355.02-.54.026zm6.065 4.3a1.5 1.5 0 1 0-1.13 0L7.5 10.5h2l-.435-2.61z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSignIn = function VscSignIn(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m11.02 3.77.01-.01.99.99V2.5l-.5-.5h-9l-.51.5v.493L2 3v10.29l.36.46 5 1.72L8 15v-1h3.52l.5-.5v-2.25l-1 1V13H8V4.71l-.33-.46L4.036 3h6.984v.77zM7 14.28l-4-1.34V3.72l4 1.34v9.22zm3.09-6.75h4.97v1h-4.93l1.59 1.6-.71.7-2.47-2.46v-.71l2.49-2.48.7.7-1.64 1.65z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSignOut = function VscSignOut(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11.02 3.77v1.56l1-.99V2.5l-.5-.5h-9l-.5.5v.486L2 3v10.29l.36.46 5 1.72L8 15v-1h3.52l.5-.5v-1.81l-1-1V13H8V4.71l-.33-.46L4.036 3h6.984v.77zM7 14.28l-4-1.34V3.72l4 1.34v9.22zm6.52-5.8H8.55v-1h4.93l-1.6-1.6.71-.7 2.47 2.46v.71l-2.49 2.48-.7-.7 1.65-1.65z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSmiley = function VscSmiley(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4.111 2.18a7 7 0 1 1 7.778 11.64A7 7 0 0 1 4.11 2.18zm.556 10.809a6 6 0 1 0 6.666-9.978 6 6 0 0 0-6.666 9.978zM6.5 7a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm5 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0zM8 11a3 3 0 0 1-2.65-1.58l-.87.48a4 4 0 0 0 7.12-.16l-.9-.43A3 3 0 0 1 8 11z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSortPrecedence = function VscSortPrecedence(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7 2 6 3v3h1V3h7v2.453l.207-.16.793.793V3l-1-1H7zm1 2h2v2H8V4zM5 9H3v2h2V9zM2 7 1 8v5l1 1h7l1-1V8L9 7H2zm0 6V8h7v5H2zm6-3H6v2h2v-2zm5-6h-1v3.864l-1.182-1.182-.707.707 2.035 2.036h.708l2.035-2.036-.707-.707L13 7.864V4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSourceControl = function VscSourceControl(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    d: "M21.007 8.222A3.738 3.738 0 0 0 15.045 5.2a3.737 3.737 0 0 0 1.156 6.583 2.988 2.988 0 0 1-2.668 1.67h-2.99a4.456 4.456 0 0 0-2.989 1.165V7.4a3.737 3.737 0 1 0-1.494 0v9.117a3.776 3.776 0 1 0 1.816.099 2.99 2.99 0 0 1 2.668-1.667h2.99a4.484 4.484 0 0 0 4.223-3.039 3.736 3.736 0 0 0 3.25-3.687zM4.565 3.738a2.242 2.242 0 1 1 4.484 0 2.242 2.242 0 0 1-4.484 0zm4.484 16.441a2.242 2.242 0 1 1-4.484 0 2.242 2.242 0 0 1 4.484 0zm8.221-9.715a2.242 2.242 0 1 1 0-4.485 2.242 2.242 0 0 1 0 4.485z"
  }));
};

module.exports.VscSplitHorizontal = function VscSplitHorizontal(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14 1H3L2 2v11l1 1h11l1-1V2l-1-1zM8 13H3V2h5v11zm6 0H9V2h5v11z"
  }));
};

module.exports.VscSplitVertical = function VscSplitVertical(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14 1H3L2 2v11l1 1h11l1-1V2l-1-1zm0 12H3V8h11v5zm0-6H3V2h11v5z"
  }));
};

module.exports.VscSquirrel = function VscSquirrel(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5.558 2.642a3.698 3.698 0 0 0-.123-.01A1.47 1.47 0 0 0 3.999 1.52v1.307a4.898 4.898 0 0 0-2.993 3.587v.39c.459.836 1.906 1.13 2.154 1.18.027.006.04.009.035.009-2.419.32-2.19 2.249-2.19 2.249a1 1 0 0 0 1 .93c.272-.019.538-.08.79-.18h2.06a3 3 0 0 0-.36 1h-.32a2.55 2.55 0 0 0-2.17 2.528.42.42 0 0 0 .39.48h6.677a3.76 3.76 0 0 0 3.929-4.158 3.649 3.649 0 0 0-.75-2.09l-.11-.14c-.43-.55-.68-.909-.55-1.289.13-.38.365-.4.365-.4s.185-.03.455.09c.22.128.46.22.71.27a1.58 1.58 0 0 0 1.736-.905c.095-.208.143-.435.143-.664.006-.718-.33-1.455-.725-2.088a4.998 4.998 0 0 0-1.554-1.57 3.998 3.998 0 0 0-2.639-.4 3.049 3.049 0 0 0-1.67.89 3.56 3.56 0 0 0-.779 1.359 4.358 4.358 0 0 0-.636-.747v-.159A1.47 1.47 0 0 0 5.558 1.52v1.122zm5.304 8.739c.111.741.22 1.821-.867 2.442-.296.103-.608.16-.923.167H3.215a1 1 0 0 1 .92-1h1.279v-.499a1.79 1.79 0 0 1 1.653-1.825l-.626-.887c-.236.067-.463.153-.577.233H2.655a.754.754 0 0 0-.264.07c-.138.055-.274.109-.396.03-.2-.13.11-1.12 1.01-1.12h1c.49 0 .57-.54.57-.54l.28-1.129a3.389 3.389 0 0 1-2.85-.93 3.389 3.389 0 0 1 3.14-2.658l.083.002c.26.008.435.014.776.168.93.42 2.149 2.469 2.149 2.469l.06.09h.17v-.07c-.06-.443-.02-1.464.116-1.89.137-.424.367-.814.673-1.14a2.349 2.349 0 0 1 1.3-.659 2.639 2.639 0 0 1 1.86.29c.46.284.85.67 1.139 1.127.289.457.476.836.535 1.374-.001.02 0 .047.002.081.007.143.02.39-.128.547-.127.135-.448.23-.67.18a1.57 1.57 0 0 1-.45-.18 1.33 1.33 0 0 0-1.139-.13 1.42 1.42 0 0 0-.94 1 2.318 2.318 0 0 0 .64 2.238l.11.14c.347.434.546.966.57 1.52a2.999 2.999 0 0 1-.306 1.425 2.708 2.708 0 0 0-.464-1.304l-.37.368zM4.24 5a.5.5 0 1 0 0 1 .5.5 0 0 0 0-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscStarEmpty = function VscStarEmpty(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M9.595 6.252 8 1 6.405 6.252H1l4.373 3.4L3.75 15 8 11.695 12.25 15l-1.623-5.348L15 6.252H9.595zm-7.247.47H6.72L8 2.507 6.72 6.722H2.348zm3.537 2.75-1.307 4.305 1.307-4.305zm7.767-2.75H9.28h4.372zm-8.75.9h2.366L8 5.214l.732 2.41h2.367l-1.915 1.49.731 2.409L8 10.032l-1.915 1.49.731-2.41-1.915-1.49z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscStarFull = function VscStarFull(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M9.595 6.252 8 1 6.405 6.252H1l4.373 3.4L3.75 15 8 11.695 12.25 15l-1.623-5.348L15 6.252H9.595z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscStarHalf = function VscStarHalf(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M6.405 6.252 8 1l1.595 5.252H15l-4.373 3.4L12.25 15 8 11.695 3.75 15l1.623-5.348L1 6.252h5.405zM8 10.032l1.915 1.49-.731-2.41 1.915-1.49H8.732L8 5.214v4.82zm0-7.525zm5.652 4.215H9.28h4.372z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscStopCircle = function VscStopCircle(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M6 6h4v4H6z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8.6 1c1.6.1 3.1.9 4.2 2 1.3 1.4 2 3.1 2 5.1 0 1.6-.6 3.1-1.6 4.4-1 1.2-2.4 2.1-4 2.4-1.6.3-3.2.1-4.6-.7-1.4-.8-2.5-2-3.1-3.5C.9 9.2.8 7.5 1.3 6c.5-1.6 1.4-2.9 2.8-3.8C5.4 1.3 7 .9 8.6 1zm.5 12.9c1.3-.3 2.5-1 3.4-2.1.8-1.1 1.3-2.4 1.2-3.8 0-1.6-.6-3.2-1.7-4.3-1-1-2.2-1.6-3.6-1.7-1.3-.1-2.7.2-3.8 1-1.1.8-1.9 1.9-2.3 3.3-.4 1.3-.4 2.7.2 4 .6 1.3 1.5 2.3 2.7 3 1.2.7 2.6.9 3.9.6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolArray = function VscSymbolArray(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m1.5 2-.5.5v11l.5.5H4v-1H2V3h2V2H1.5zm13 12 .5-.5v-11l-.5-.5H12v1h2v10h-2v1h2.5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolBoolean = function VscSymbolBoolean(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m1 3.5.5-.5h13l.5.5v9l-.5.5h-13l-.5-.5v-9zM14 4H8v3.493h-.5l-3.574-.005 2.09-2.09-.707-.707-2.955 2.955v.708l2.955 2.955.707-.707-2.114-2.114 3.996.005H8v-.986l3.907.005-2.114-2.114.707-.707 2.956 2.955v.708L10.5 11.309l-.707-.707 2.09-2.09L8 8.507V12h6V4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolClass = function VscSymbolClass(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M11.34 9.71h.71l2.67-2.67v-.71L13.38 5h-.7l-1.82 1.81h-5V5.56l1.86-1.85V3l-2-2H5L1 5v.71l2 2h.71l1.14-1.15v5.79l.5.5H10v.52l1.33 1.34h.71l2.67-2.67v-.71L13.37 10h-.7l-1.86 1.85h-5v-4H10v.48l1.34 1.38zm1.69-3.65.63.63-2 2-.63-.63 2-2zm0 5 .63.63-2 2-.63-.63 2-2zM3.35 6.65l-1.29-1.3 3.29-3.29 1.3 1.29-3.3 3.3z"
  }));
};

module.exports.VscSymbolColor = function VscSymbolColor(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 1.003a7 7 0 0 0-7 7v.43c.09 1.51 1.91 1.79 3 .7a1.87 1.87 0 0 1 2.64 2.64c-1.1 1.16-.79 3.07.8 3.2h.6a7 7 0 1 0 0-14l-.04.03zm0 13h-.52a.58.58 0 0 1-.36-.14.56.56 0 0 1-.15-.3 1.24 1.24 0 0 1 .35-1.08 2.87 2.87 0 0 0 0-4 2.87 2.87 0 0 0-4.06 0 1 1 0 0 1-.9.34.41.41 0 0 1-.22-.12.42.42 0 0 1-.1-.29v-.37a6 6 0 1 1 6 6l-.04-.04zM9 3.997a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm3 7.007a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm-7-5a1 1 0 1 0 0-2 1 1 0 0 0 0 2zm7-1a1 1 0 1 1-2 0 1 1 0 0 1 2 0zM13 8a1 1 0 1 1-2 0 1 1 0 0 1 2 0z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolConstant = function VscSymbolConstant(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4 6h8v1H4V6zm8 3H4v1h8V9z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m1 4 1-1h12l1 1v8l-1 1H2l-1-1V4zm1 0v8h12V4H2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolEnumMember = function VscSymbolEnumMember(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m7 3 1-1h6l1 1v5l-1 1h-4V8h4V3H8v3H7V3zm2 6V8L8 7H2L1 8v5l1 1h6l1-1V9zM8 8v5H2V8h6zm1.414-1L9 6.586V6h4v1H9.414zM9 4h4v1H9V4zm-2 6H3v1h4v-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolEnum = function VscSymbolEnum(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14 2H8L7 3v3h1V3h6v5h-4v1h4l1-1V3l-1-1zM9 6h4v1H9.41L9 6.59V6zM7 7H2L1 8v5l1 1h6l1-1V8L8 7H7zm1 6H2V8h6v5zM3 9h4v1H3V9zm0 2h4v1H3v-1zm6-7h4v1H9V4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolEvent = function VscSymbolEvent(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.414 1.56 8.312 1h3.294l.818 1.575L10.236 6h1.781l.72 1.695L5.618 15l-1.602-1.163L6.119 10H4.898L4 8.56l3.414-7zM7.78 9 4.9 14.305 12.018 7H8.312l3.294-5H8.312L4.898 9H7.78z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolField = function VscSymbolField(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m14.45 4.5-5-2.5h-.9l-7 3.5-.55.89v4.5l.55.9 5 2.5h.9l7-3.5.55-.9v-4.5l-.55-.89zm-8 8.64-4.5-2.25V7.17l4.5 2v3.97zm.5-4.8L2.29 6.23l6.66-3.34 4.67 2.34-6.67 3.11zm7 1.55-6.5 3.25V9.21l6.5-3v3.68z"
  }));
};

module.exports.VscSymbolFile = function VscSymbolFile(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m13.85 4.44-3.28-3.3-.35-.14H2.5l-.5.5v13l.5.5h11l.5-.5V4.8l-.15-.36zM13 5h-3V2l3 3zM3 14V2h6v3.5l.5.5H13v8H3z"
  }));
};

module.exports.VscSymbolInterface = function VscSymbolInterface(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M11.496 4a3.49 3.49 0 0 0-3.46 3h-3.1a2 2 0 1 0 0 1h3.1a3.5 3.5 0 1 0 3.46-4zm0 6a2.5 2.5 0 1 1 0-5 2.5 2.5 0 0 1 0 5z"
  }));
};

module.exports.VscSymbolKey = function VscSymbolKey(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.223 10.933c.326.192.699.29 1.077.282a2.159 2.159 0 0 0 1.754-.842 3.291 3.291 0 0 0 .654-2.113 2.886 2.886 0 0 0-.576-1.877 1.99 1.99 0 0 0-1.634-.733 2.294 2.294 0 0 0-1.523.567V3.475h-.991V11.1h.995v-.344c.076.066.158.125.244.177zM7.85 6.7c.186-.079.388-.113.59-.1a1.08 1.08 0 0 1 .896.428c.257.363.382.802.357 1.245a2.485 2.485 0 0 1-.4 1.484 1.133 1.133 0 0 1-.96.508 1.224 1.224 0 0 1-.976-.417A1.522 1.522 0 0 1 6.975 8.8v-.6a1.722 1.722 0 0 1 .393-1.145c.13-.154.296-.276.482-.355zM3.289 5.675a3.03 3.03 0 0 0-.937.162 2.59 2.59 0 0 0-.8.4l-.1.077v1.2l.423-.359a2.1 2.1 0 0 1 1.366-.572.758.758 0 0 1 .661.282c.15.232.23.503.231.779L2.9 7.825a2.6 2.6 0 0 0-1.378.575 1.65 1.65 0 0 0-.022 2.336 1.737 1.737 0 0 0 1.253.454 1.96 1.96 0 0 0 1.107-.332c.102-.068.197-.145.286-.229v.444h.941V7.715a2.193 2.193 0 0 0-.469-1.5 1.687 1.687 0 0 0-1.329-.54zm.857 3.041c.02.418-.12.829-.391 1.148a1.221 1.221 0 0 1-.955.422.832.832 0 0 1-.608-.2.833.833 0 0 1 0-1.091c.281-.174.6-.277.93-.3l1.02-.148.004.169zm8.313 2.317c.307.13.64.193.973.182.495.012.983-.114 1.41-.365l.123-.075.013-.007V9.615l-.446.32c-.316.224-.696.34-1.084.329A1.3 1.3 0 0 1 12.4 9.8a1.975 1.975 0 0 1-.4-1.312 2.01 2.01 0 0 1 .453-1.381A1.432 1.432 0 0 1 13.6 6.6a1.8 1.8 0 0 1 .971.279l.43.265V5.97l-.17-.073a2.9 2.9 0 0 0-1.17-.247 2.52 2.52 0 0 0-1.929.817 2.9 2.9 0 0 0-.747 2.049c-.028.707.21 1.4.67 1.939.222.249.497.446.804.578z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolKeyword = function VscSymbolKeyword(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M15 4h-5V3h5v1zm-1 3h-2v1h2V7zm-4 0H1v1h9V7zm2 6H1v1h11v-1zm-5-3H1v1h6v-1zm8 0h-5v1h5v-1zM8 2v3H1V2h7zM7 3H2v1h5V3z"
  }));
};

module.exports.VscSymbolMethod = function VscSymbolMethod(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m13.51 4-5-3h-1l-5 3-.49.86v6l.49.85 5 3h1l5-3 .49-.85v-6L13.51 4zm-6 9.56-4.5-2.7V5.7l4.5 2.45v5.41zM3.27 4.7l4.74-2.84 4.74 2.84-4.74 2.59L3.27 4.7zm9.74 6.16-4.5 2.7V8.15l4.5-2.45v5.16z"
  }));
};

module.exports.VscSymbolMisc = function VscSymbolMisc(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4 2h8v4c.341.035.677.112 1 .23V1H3v8.48l1-1.75V2zm2.14 8L5 8 4 9.75 3.29 11 1 15h8l-2.29-4-.57-1zm-3.42 4 1.72-3L5 10l.56 1 1.72 3H2.72zm6.836-6.41a3.5 3.5 0 1 1 3.888 5.82 3.5 3.5 0 0 1-3.888-5.82zm.555 4.989a2.5 2.5 0 1 0 2.778-4.157 2.5 2.5 0 0 0-2.778 4.157z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolNamespace = function VscSymbolNamespace(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M6 2.984V2h-.09c-.313 0-.616.062-.909.185a2.33 2.33 0 0 0-.775.53 2.23 2.23 0 0 0-.493.753v.001a3.542 3.542 0 0 0-.198.83v.002a6.08 6.08 0 0 0-.024.863c.012.29.018.58.018.869 0 .203-.04.393-.117.572v.001a1.504 1.504 0 0 1-.765.787 1.376 1.376 0 0 1-.558.115H2v.984h.09c.195 0 .38.04.556.121l.001.001c.178.078.329.184.455.318l.002.002c.13.13.233.285.307.465l.001.002c.078.18.117.368.117.566 0 .29-.006.58-.018.869-.012.296-.004.585.024.87v.001c.033.283.099.558.197.824v.001c.106.273.271.524.494.753.223.23.482.407.775.53.293.123.596.185.91.185H6v-.984h-.09c-.199 0-.387-.038-.562-.115a1.613 1.613 0 0 1-.457-.32 1.659 1.659 0 0 1-.309-.467c-.074-.18-.11-.37-.11-.573 0-.228.003-.453.011-.672.008-.228.008-.45 0-.665a4.639 4.639 0 0 0-.055-.64 2.682 2.682 0 0 0-.168-.609A2.284 2.284 0 0 0 3.522 8a2.284 2.284 0 0 0 .738-.955c.08-.192.135-.393.168-.602.033-.21.051-.423.055-.64.008-.22.008-.442 0-.666-.008-.224-.012-.45-.012-.678a1.47 1.47 0 0 1 .877-1.354 1.33 1.33 0 0 1 .563-.121H6zm4 10.032V14h.09c.313 0 .616-.062.909-.185.293-.123.552-.3.775-.53.223-.23.388-.48.493-.753v-.001c.1-.266.165-.543.198-.83v-.002c.028-.28.036-.567.024-.863-.012-.29-.018-.58-.018-.869 0-.203.04-.393.117-.572v-.001a1.504 1.504 0 0 1 .765-.787c.176-.077.362-.115.558-.115H14v-.984h-.09c-.195 0-.38-.04-.556-.121l-.001-.001a1.376 1.376 0 0 1-.455-.318l-.002-.002a1.414 1.414 0 0 1-.307-.465l-.001-.002a1.405 1.405 0 0 1-.117-.566c0-.29.006-.58.018-.869a6.19 6.19 0 0 0-.024-.87v-.001a3.542 3.542 0 0 0-.197-.824v-.001a2.23 2.23 0 0 0-.494-.753 2.33 2.33 0 0 0-.775-.53 2.325 2.325 0 0 0-.91-.185H10v.984h.09c.2 0 .386.038.562.115.174.082.326.188.457.32.127.134.23.29.309.467.074.18.11.37.11.573 0 .228-.003.452-.011.672-.008.228-.008.45 0 .665.004.222.022.435.055.64.033.214.089.416.168.609a2.282 2.282 0 0 0 .738.955 2.282 2.282 0 0 0-.738.955 2.7 2.7 0 0 0-.168.602c-.033.21-.051.423-.055.64-.008.22-.008.442 0 .666.008.224.012.45.012.678a1.47 1.47 0 0 1-.42 1.035 1.466 1.466 0 0 1-.457.319 1.33 1.33 0 0 1-.563.121H10z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolNumeric = function VscSymbolNumeric(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11 1v4h4v1h-4v4h4v1h-4v4h-1v-4H6v4H5v-4H1v-1h4V6H1V5h4V1h1v4h4V1h1zM6 6v4h4V6H6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolOperator = function VscSymbolOperator(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2.873 1.1c.335.136.602.398.745.73.072.17.109.352.107.537a1.34 1.34 0 0 1-.61 1.135 1.359 1.359 0 0 1-.753.223A1.355 1.355 0 0 1 1 2.362a1.355 1.355 0 0 1 .83-1.256A1.37 1.37 0 0 1 2.873 1.1zm-.298 1.765a.551.551 0 0 0 .332-.5.548.548 0 1 0-.332.5zM6.43 1.109 1.11 6.43l.686.687 5.32-5.32-.686-.687zM11.5 9h1v2.5H15v1h-2.5V15h-1v-2.5H9v-1h2.5V9zm-5.732.525.707.707L4.707 12l1.768 1.768-.707.707L4 12.707l-1.768 1.768-.707-.707L3.293 12l-1.768-1.768.707-.707L4 11.293l1.768-1.768zm1.35-4.195a1.353 1.353 0 0 0-1.256-.83 1.355 1.355 0 0 0-1.256.83 1.362 1.362 0 0 0 1.257 1.895A1.358 1.358 0 0 0 7.118 5.33zm-.753.745a.553.553 0 0 1-.289.29.547.547 0 0 1-.599-.117.529.529 0 0 1-.117-.173.544.544 0 0 1 .716-.715.565.565 0 0 1 .173.116.549.549 0 0 1 .116.599zM14 3h-4v1h4V3z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolParameter = function VscSymbolParameter(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M11 6h-1v-.5a.5.5 0 0 0-.5-.5H8.479v5.5a.5.5 0 0 0 .5.5h.5v1h-3v-1h.5a.5.5 0 0 0 .5-.5V5H6.5a.5.5 0 0 0-.5.5V6H5V4h6v2zm2.914 2.048-1.462-1.462.707-.707 1.816 1.816v.707l-1.768 1.767-.707-.707 1.414-1.414zM3.548 9.462 2.086 8 3.5 6.586l-.707-.707-1.768 1.767v.708l1.816 1.815.707-.707z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolProperty = function VscSymbolProperty(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M2.807 14.975a1.75 1.75 0 0 1-1.255-.556 1.684 1.684 0 0 1-.544-1.1A1.72 1.72 0 0 1 1.36 12.1c1.208-1.27 3.587-3.65 5.318-5.345a4.257 4.257 0 0 1 .048-3.078 4.095 4.095 0 0 1 1.665-1.969 4.259 4.259 0 0 1 4.04-.36l.617.268-2.866 2.951 1.255 1.259 2.944-2.877.267.619a4.295 4.295 0 0 1 .04 3.311 4.198 4.198 0 0 1-.923 1.392 4.27 4.27 0 0 1-.743.581 4.217 4.217 0 0 1-3.812.446c-1.098 1.112-3.84 3.872-5.32 5.254a1.63 1.63 0 0 1-1.084.423zm7.938-13.047a3.32 3.32 0 0 0-1.849.557c-.213.13-.412.284-.591.458a3.321 3.321 0 0 0-.657 3.733l.135.297-.233.227c-1.738 1.697-4.269 4.22-5.485 5.504a.805.805 0 0 0 .132 1.05.911.911 0 0 0 .298.22c.1.044.209.069.319.072a.694.694 0 0 0 .45-.181c1.573-1.469 4.612-4.539 5.504-5.44l.23-.232.294.135a3.286 3.286 0 0 0 3.225-.254 3.33 3.33 0 0 0 .591-.464 3.28 3.28 0 0 0 .964-2.358c0-.215-.021-.43-.064-.642L11.43 7.125 8.879 4.578l2.515-2.59a3.286 3.286 0 0 0-.65-.06z"
  }));
};

module.exports.VscSymbolRuler = function VscSymbolRuler(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M4 1 3 2v12l1 1h8l1-1V2l-1-1H4zm0 2V2h8v12H4v-1h2v-1H4v-2h4V9H4V7h2V6H4V4h4V3H4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolSnippet = function VscSymbolSnippet(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m2.5 1-.5.5V13h1V2h11v11h1V1.5l-.5-.5h-12zM2 15v-1h1v1H2zm3-1H4v1h1v-1zm1 0h1v1H6v-1zm3 0H8v1h1v-1zm1 0h1v1h-1v-1zm5 1v-1h-1v1h1zm-3-1h1v1h-1v-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolString = function VscSymbolString(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2 2 1 3v9l1 1h12l1-1V3l-1-1H2zm0 10V3h12v9H2zm3.356-3.07H6V7.22C6 6.408 5.685 6 5.056 6c-.135 0-.285.024-.45.073a1.444 1.444 0 0 0-.388.167v.665c.237-.203.487-.304.75-.304.261 0 .392.156.392.469l-.6.103c-.506.086-.76.406-.76.961 0 .263.061.473.183.631A.61.61 0 0 0 4.69 9c.29 0 .509-.16.657-.48h.009v.41zm.004-1.355v.193a.75.75 0 0 1-.12.436.368.368 0 0 1-.313.17.276.276 0 0 1-.22-.095.38.38 0 0 1-.08-.248c0-.222.11-.351.332-.389l.4-.067zM7.6 8.626h-.007v.31H7V5h.593v1.677h.008c.146-.31.355-.465.625-.465.248 0 .44.118.573.353.134.236.201.557.201.966 0 .443-.078.798-.235 1.067C8.61 8.866 8.4 9 8.138 9c-.237 0-.416-.125-.537-.374zm-.016-1.121v.272a.78.78 0 0 0 .107.426c.071.113.163.169.274.169.135 0 .24-.072.314-.216.075-.145.113-.35.113-.615 0-.22-.035-.39-.104-.514-.067-.124-.164-.187-.29-.187-.12 0-.219.062-.298.185a.887.887 0 0 0-.116.48zM11.262 9c.321 0 .567-.058.738-.173v-.71a.9.9 0 0 1-.552.207.619.619 0 0 1-.5-.215c-.12-.145-.181-.345-.181-.598 0-.26.063-.464.189-.612a.644.644 0 0 1 .516-.223c.194 0 .37.069.528.207v-.749c-.129-.09-.338-.134-.626-.134-.417 0-.751.14-1.001.422-.249.28-.373.662-.373 1.148 0 .42.116.764.349 1.03.232.267.537.4.913.4z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolStructure = function VscSymbolStructure(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2 2 1 3v3l1 1h12l1-1V3l-1-1H2zm0 1h12v3H2V3zm-1 7 1-1h3l1 1v3l-1 1H2l-1-1v-3zm2 0H2v3h3v-3H3zm7 0 1-1h3l1 1v3l-1 1h-3l-1-1v-3zm2 0h-1v3h3v-3h-2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSymbolVariable = function VscSymbolVariable(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2 5h2V4H1.5l-.5.5v8l.5.5H4v-1H2V5zm12.5-1H12v1h2v7h-2v1h2.5l.5-.5v-8l-.5-.5zm-2.74 2.57L12 7v2.51l-.3.45-4.5 2h-.46l-2.5-1.5-.24-.43v-2.5l.3-.46 4.5-2h.46l2.5 1.5zM5 9.71l1.5.9V9.28L5 8.38v1.33zm.58-2.15 1.45.87 3.39-1.5-1.45-.87-3.39 1.5zm1.95 3.17 3.5-1.56v-1.4l-3.5 1.55v1.41z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscSyncIgnored = function VscSyncIgnored(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m5.468 3.687-.757-.706a6 6 0 0 1 9.285 4.799L15.19 6.6l.75.76-2.09 2.07-.76-.01L11 7.31l.76-.76 1.236 1.25a5 5 0 0 0-7.528-4.113zm4.55 8.889.784.73a6 6 0 0 1-8.796-5.04L.78 9.5 0 8.73l2.09-2.07.76.01 2.09 2.12-.76.76-1.167-1.18a5 5 0 0 0 7.005 4.206z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    d: "m1.123 2.949.682-.732L13.72 13.328l-.682.732z"
  }));
};

module.exports.VscSync = function VscSync(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2.006 8.267.78 9.5 0 8.73l2.09-2.07.76.01 2.09 2.12-.76.76-1.167-1.18a5 5 0 0 0 9.4 1.983l.813.597a6 6 0 0 1-11.22-2.683zm10.99-.466L11.76 6.55l-.76.76 2.09 2.11.76.01 2.09-2.07-.75-.76-1.194 1.18a6 6 0 0 0-11.11-2.92l.81.594a5 5 0 0 1 9.3 2.346z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscTable = function VscTable(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13.5 2h-12l-.5.5v11l.5.5h12l.5-.5v-11l-.5-.5zM2 3h11v1H2V3zm7 4H6V5h3v2zm0 1v2H6V8h3zM2 5h3v2H2V5zm0 3h3v2H2V8zm0 5v-2h3v2H2zm4 0v-2h3v2H6zm7 0h-3v-2h3v2zm0-3h-3V8h3v2zm-3-3V5h3v2h-3z"
  }));
};

module.exports.VscTag = function VscTag(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13.2 2H8.017l-.353.146L1 8.81v.707L6.183 14.7h.707l2.215-2.215A4.48 4.48 0 0 0 15.65 9c.027-.166.044-.332.051-.5a4.505 4.505 0 0 0-2-3.74V2.5l-.5-.5zm-.5 2.259A4.504 4.504 0 0 0 11.2 4a.5.5 0 1 0 0 1 3.5 3.5 0 0 1 1.5.338v2.138L8.775 11.4a.506.506 0 0 0-.217.217l-2.022 2.022-4.475-4.476L8.224 3H12.7v1.259zm1 1.792a3.5 3.5 0 0 1 1 2.449 3.438 3.438 0 0 1-.051.5 3.487 3.487 0 0 1-4.793 2.735l3.698-3.698.146-.354V6.051z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscTasklist = function VscTasklist(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m3.57 6.699 5.693-4.936L8.585 1 3.273 5.596l-1.51-1.832L1 4.442l1.85 2.214.72.043zM15 5H6.824l2.307-2H15v2zM6 7h9v2H6V7zm9 4H6v2h9v-2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscTelescope = function VscTelescope(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m11.24 1 .59.24 2.11 4.93-.23.59-3.29 1.41-.59-.24-.17-.41L6.1 9l-.58-.19-.16-.38L2.8 9.49l-.58-.24-.72-1.67.28-.59 2.5-1.06-.18-.41.24-.58L7.9 3.41 7.72 3 8 2.42 11.24 1zM2.5 7.64l.35.85 2.22-.91-.37-.85-2.2.91zm2.74-2.12 1.11 2.45 3-1.28-1.11-2.44-3 1.27zM8.79 3l1.86 4.11 2.29-1.01L11.18 2 8.72 3h.07zM8.5 9.1l3.02 4.9h-1.17l-1.88-3.03v4h-1V9.82L5.58 14h-1.1l1.7-3.9 2.32-1z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscTerminalBash = function VscTerminalBash(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13.655 3.56 8.918.75a1.785 1.785 0 0 0-1.82 0L2.363 3.56a1.889 1.889 0 0 0-.921 1.628v5.624a1.889 1.889 0 0 0 .913 1.627l4.736 2.812a1.785 1.785 0 0 0 1.82 0l4.736-2.812a1.888 1.888 0 0 0 .913-1.627V5.188a1.889 1.889 0 0 0-.904-1.627zm-3.669 8.781v.404a.149.149 0 0 1-.07.124l-.239.137c-.038.02-.07 0-.07-.053v-.396a.78.78 0 0 1-.545.053.073.073 0 0 1-.027-.09l.086-.365a.153.153 0 0 1 .071-.096.048.048 0 0 1 .038 0 .662.662 0 0 0 .497-.063.662.662 0 0 0 .37-.567c0-.206-.112-.292-.384-.293-.344 0-.661-.066-.67-.574A1.47 1.47 0 0 1 9.6 9.437V9.03a.147.147 0 0 1 .07-.126l.231-.147c.038-.02.07 0 .07.054v.409a.754.754 0 0 1 .453-.055.073.073 0 0 1 .03.095l-.081.362a.156.156 0 0 1-.065.09.055.055 0 0 1-.035 0 .6.6 0 0 0-.436.072.549.549 0 0 0-.331.486c0 .185.098.242.425.248.438 0 .627.199.632.639a1.568 1.568 0 0 1-.576 1.185zm2.481-.68a.094.094 0 0 1-.036.092l-1.198.727a.034.034 0 0 1-.04.003.035.035 0 0 1-.016-.037v-.31a.086.086 0 0 1 .055-.076l1.179-.706a.035.035 0 0 1 .056.035v.273zm.827-6.914L8.812 7.515c-.559.331-.97.693-.97 1.367v5.52c0 .404.165.662.413.741a1.465 1.465 0 0 1-.248.025c-.264 0-.522-.072-.748-.207L2.522 12.15a1.558 1.558 0 0 1-.75-1.338V5.188a1.558 1.558 0 0 1 .75-1.34l4.738-2.81a1.46 1.46 0 0 1 1.489 0l4.736 2.812a1.548 1.548 0 0 1 .728 1.083c-.154-.334-.508-.427-.92-.185h.002z"
  }));
};

module.exports.VscTerminalCmd = function VscTerminalCmd(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m10.875 7 2.008 5h-.711l-2.008-5h.711zm-5.125.594c-.276 0-.526.041-.75.125a1.542 1.542 0 0 0-.578.375c-.162.166-.287.37-.375.61a2.364 2.364 0 0 0-.133.827c0 .287.04.547.117.781.078.235.196.433.352.594.156.162.346.29.57.383.224.094.48.138.766.133a2.63 2.63 0 0 0 .992-.195l.125.484a1.998 1.998 0 0 1-.492.148 4.381 4.381 0 0 1-.75.07 2.61 2.61 0 0 1-.914-.156 2.207 2.207 0 0 1-.742-.453 1.878 1.878 0 0 1-.485-.742 3.204 3.204 0 0 1-.18-1.023c0-.365.06-.698.18-1 .12-.302.287-.563.5-.782.214-.218.471-.388.774-.507a2.69 2.69 0 0 1 1-.18c.296 0 .536.023.718.07.183.047.315.094.399.14l-.149.493a1.85 1.85 0 0 0-.406-.14 2.386 2.386 0 0 0-.539-.055zM8 8h1v1H8V8zm0 2h1v1H8v-1z"
  }), React.createElement("path", {
    d: "M15.5 1H.5l-.5.5v13l.5.5h15l.5-.5v-13l-.5-.5zM15 14H1V5h14v9zm0-10H1V2h14v2z"
  }));
};

module.exports.VscTerminalDebian = function VscTerminalDebian(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.084.029a1.276 1.276 0 0 0-.355.05L6.622.065a9.46 9.46 0 0 1 .514-.048c.075-.005.15-.01.224-.017a1.67 1.67 0 0 1-.276.029zm4.127 7.646c.094-.238.172-.436.16-.762l-.133.282c.135-.41.123-.847.112-1.262-.005-.187-.01-.37-.002-.543l-.054-.015c-.048-1.411-1.268-2.911-2.354-3.419-.936-.432-2.376-.506-3.042-.18a.657.657 0 0 1 .212-.085c.107-.031.197-.058.135-.093-.6.06-.778.171-.973.294a1.92 1.92 0 0 1-.635.273c-.11.106.051.063.181.029.129-.035.226-.06-.004.076a1.7 1.7 0 0 1-.303.05c-.26.025-.492.048-.96.532.026.041.11-.009.168-.044.072-.043.106-.063-.054.137C3.07 2.871 1.78 4.31 1.507 4.787l.143.025c-.1.25-.213.461-.313.649-.136.254-.249.464-.273.667a16.97 16.97 0 0 1-.062.635C.907 7.619.79 8.679 1.12 9.06l-.04.406.052.11c.036.079.071.157.12.23l-.093.008c.22.692.338.704.473.717.137.013.291.028.585.757-.084-.028-.17-.06-.293-.226-.015.127.18.508.41.806l-.097.112a.89.89 0 0 0 .27.311c.023.019.045.036.066.055-.372-.203.1.428.371.79.078.104.14.186.159.218l.073-.132c-.01.19.136.433.41.772l.229-.009c.094.186.438.522.647.538l-.139.181c.254.08.321.135.397.195.08.064.17.136.502.253l-.13-.23c.108.095.192.186.273.272.162.176.31.335.62.481.352.123.536.152.74.184.168.026.35.055.649.14a33.82 33.82 0 0 0-.217-.005c-.506-.012-1.056-.025-1.443-.163-3.016-.817-5.776-4.356-5.574-8-.02-.311-.01-.655 0-.961.012-.422.022-.776-.049-.882l.032-.105c.166-.54.365-1.191.742-1.957L.861 3.92v-.002.001c.012.012.106.107.275-.18.04-.09.079-.182.117-.276.08-.19.16-.383.264-.56l.08-.02c.054-.315.533-.744.93-1.1.19-.171.362-.326.46-.443l.02.138C3.541.977 4.414.611 5.074.334c.152-.063.291-.122.414-.176-.107.118.067.082.311.032.15-.03.325-.067.478-.076-.04.023-.082.044-.122.065-.085.045-.17.088-.25.145.26-.062.373-.044.499-.024.109.018.227.036.456.006-.174.025-.384.094-.35.12.245.029.398-.002.537-.03.174-.034.327-.065.61.03L7.625.275c.235.085.409.137.564.183.313.094.55.165 1.067.439a.58.58 0 0 0 .23-.037c.112-.035.218-.069.477.037.014.025.022.046.03.066.03.08.054.143.456.383.056-.022-.097-.162-.22-.274l-.003-.004c1.01.54 2.108 1.692 2.443 2.924-.188-.347-.162-.171-.134.015.018.124.037.253-.006.235.14.377.255.766.325 1.168l-.023-.085c-.102-.368-.3-1.081-.626-1.555-.012.137-.092.122-.165.108-.105-.019-.196-.036-.058.393.081.119.096.074.109.034.015-.047.027-.086.147.164.002.133.034.266.07.414.022.094.046.195.065.306-.034-.006-.07-.07-.106-.13-.045-.076-.087-.147-.117-.101.076.358.201.545.25.572-.009.02-.021.02-.034.021-.027.002-.056.003-.059.167.022.428.102.39.166.361.02-.009.037-.017.051-.01a1.724 1.724 0 0 1-.083.245c-.086.221-.188.48-.106.816a2.356 2.356 0 0 0-.106-.295 5.896 5.896 0 0 1-.046-.117c-.018.151-.01.256-.003.355.013.166.023.312-.094.62.135-.442.12-.841-.007-.649.03.343-.12.642-.254.908-.111.222-.211.42-.184.602l-.161-.222c-.238.344-.22.417-.202.489.015.06.03.12-.105.339.051-.09.041-.112.031-.133-.01-.024-.021-.046.053-.158-.05.003-.17.12-.316.265-.123.121-.265.261-.402.368-1.172.94-2.571 1.062-3.926.556.006-.031-.006-.066-.097-.128-1.148-.88-1.827-1.628-1.591-3.36.068-.051.117-.193.175-.362.09-.263.203-.59.448-.745.245-.541.979-1.04 1.764-1.052.8-.044 1.476.427 1.816.872-.618-.576-1.63-.751-2.493-.324-.882.396-1.405 1.368-1.329 2.336.01-.016.021-.023.03-.03.02-.015.037-.027.048-.108-.027 1.88 2.026 3.258 3.504 2.563l.018.039c.397-.109.497-.205.633-.335.07-.067.148-.142.28-.233a.441.441 0 0 1-.075.085c-.068.067-.143.14-.05.142.166-.043.634-.465.947-.746l.133-.119c.062-.134.051-.177.04-.221-.012-.052-.025-.104.076-.3l.229-.114c.03-.088.062-.168.092-.243zM6.612 10.06a.018.018 0 0 0-.005.016.114.114 0 0 0 .005-.016zm-.005.016c.008.069.269.268.465.369.516.19 1.1.198 1.559.181-.993.415-2.889-.422-3.509-1.532.057.012.168.14.303.297.204.234.462.532.678.605-.213-.17-.377-.387-.53-.61.288.33.637.6 1.019.779a.102.102 0 0 1 .01-.077l.005-.012zM6.752.219a6.612 6.612 0 0 1-.075-.013c.472.014.437.045.283.08.018-.029-.09-.047-.208-.067zM9.63 6.732c.032-.477-.094-.326-.136-.144.019.01.036.059.052.107.028.08.054.158.084.037zm-.211.664a1.68 1.68 0 0 1-.314.703c.006-.061-.038-.074-.083-.086-.092-.026-.183-.052.176-.504a1.113 1.113 0 0 1-.126.242c-.112.184-.21.344.126.133l.033-.06a1.43 1.43 0 0 0 .188-.428zm-1.34 1.247c-.347-.053-.662-.186-.397-.19.221.02.44.02.656-.033a3.544 3.544 0 0 1-.26.223zM6.958.285l-.1.02.094-.008.006-.012zM4.79 8.818l-.038.186c.047.064.092.13.136.195.12.175.237.348.4.483a4.73 4.73 0 0 0-.214-.368c-.08-.13-.169-.272-.285-.496zm.226-.319c.052.108.104.213.185.302l.082.24-.038-.063c-.1-.166-.2-.333-.252-.524l.023.045zm7.474-1.282-.039.098a4.717 4.717 0 0 1-.462 1.474c.261-.49.43-1.028.501-1.572zM.436 3.426zm.002.022c.008.037.043.028.075.02.06-.015.114-.03-.004.236-.074.052-.119.087-.144.106l-.027.02a.05.05 0 0 1 .008-.017.597.597 0 0 0 .092-.365zM.118 4.76a2.92 2.92 0 0 1-.106.436.588.588 0 0 0-.005-.154c-.013-.105-.025-.197.135-.402a4.009 4.009 0 0 0-.023.12z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscTerminalLinux = function VscTerminalLinux(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13.281 11.156a.84.84 0 0 1 .375.297c.084.125.143.276.18.453.02.104.044.2.07.29a1.772 1.772 0 0 0 .219.476c.047.073.11.153.188.242.067.073.127.167.18.281a.793.793 0 0 1 .077.328.49.49 0 0 1-.093.305.944.944 0 0 1-.235.219c-.12.083-.245.156-.375.219-.13.062-.26.127-.39.195a3.624 3.624 0 0 0-.555.328c-.156.115-.313.26-.469.438a2.815 2.815 0 0 1-.625.523 1.471 1.471 0 0 1-.383.172c-.13.036-.26.06-.39.07-.302 0-.552-.052-.75-.156-.198-.104-.37-.294-.516-.57-.042-.079-.083-.128-.125-.149a.774.774 0 0 0-.203-.055L8.67 15c-.26-.02-.525-.031-.796-.031a4.28 4.28 0 0 0-.672.054c-.229.037-.456.081-.68.133-.046.01-.093.05-.14.117a1.7 1.7 0 0 1-.196.227 1.106 1.106 0 0 1-.335.219 1.475 1.475 0 0 1-.555.101c-.172 0-.357-.018-.555-.054a1.82 1.82 0 0 1-.531-.18 3.578 3.578 0 0 0-.953-.328c-.313-.057-.643-.11-.992-.156a3.392 3.392 0 0 1-.344-.063.774.774 0 0 1-.29-.133.705.705 0 0 1-.194-.219.78.78 0 0 1-.079-.351c0-.162.021-.318.063-.469.042-.15.065-.31.07-.476 0-.115-.008-.227-.023-.336a3.53 3.53 0 0 1-.032-.352c0-.265.063-.46.188-.586.125-.125.307-.224.547-.297a.99.99 0 0 0 .297-.148 2.27 2.27 0 0 0 .234-.203 1.86 1.86 0 0 0 .203-.242c.063-.089.133-.178.211-.266a.114.114 0 0 0 .024-.07c0-.063-.003-.123-.008-.18l-.016-.188c0-.354.055-.71.164-1.07.11-.36.253-.71.43-1.055a9.08 9.08 0 0 1 .594-.992c.218-.317.435-.612.648-.883a4.35 4.35 0 0 0 .68-1.203c.15-.416.229-.87.234-1.36 0-.207-.01-.413-.031-.616a6.122 6.122 0 0 1-.031-.625c0-.417.047-.792.14-1.125.094-.334.24-.62.438-.86s.456-.419.773-.539C7.474.075 7.854.01 8.296 0c.527 0 .946.104 1.259.313.312.208.552.481.718.82.167.338.274.716.32 1.133.048.416.074.838.079 1.265v.133c0 .214.002.404.008.57a2.527 2.527 0 0 0 .226.977c.073.161.182.336.328.523.25.329.506.66.766.993.26.333.497.677.71 1.03.214.355.389.725.524 1.11.136.386.206.802.211 1.25a3.3 3.3 0 0 1-.164 1.04zm-6.554-8.14c.072 0 .132.018.18.054a.357.357 0 0 1 .109.149.85.85 0 0 1 .054.187c.01.063.016.128.016.196a.282.282 0 0 1-.024.125.27.27 0 0 1-.07.086l-.094.078a.796.796 0 0 0-.093.093.428.428 0 0 1-.149.141 2.129 2.129 0 0 0-.18.117 1.31 1.31 0 0 0-.156.133.264.264 0 0 0-.07.195c0 .047.023.086.07.117a.704.704 0 0 1 .266.305c.052.12.11.237.172.352.062.114.143.21.242.289.099.078.253.117.46.117h.048c.208-.01.406-.065.594-.164.187-.099.375-.203.562-.313a.633.633 0 0 1 .102-.046.37.37 0 0 0 .101-.055l.57-.445a.926.926 0 0 0 .024-.102 2.75 2.75 0 0 0 .016-.11.236.236 0 0 0-.04-.14.4.4 0 0 0-.093-.094.34.34 0 0 0-.133-.054.909.909 0 0 1-.14-.04 1.083 1.083 0 0 1-.352-.14 1.457 1.457 0 0 0-.344-.156c-.02-.006-.036-.021-.047-.047a.983.983 0 0 1-.031-.094.23.23 0 0 1-.008-.102.126.126 0 0 0-.008-.078c0-.062.005-.127.016-.195a.551.551 0 0 1 .07-.195.417.417 0 0 1 .125-.14.411.411 0 0 1 .203-.056c.162 0 .279.06.352.18.073.12.112.25.117.39a.397.397 0 0 1-.039.18.379.379 0 0 0-.04.172c0 .042.014.07.04.086a.26.26 0 0 0 .102.031c.12 0 .197-.028.234-.085a.533.533 0 0 0 .062-.258c0-.12-.01-.253-.03-.399a1.32 1.32 0 0 0-.126-.406.969.969 0 0 0-.242-.313.574.574 0 0 0-.383-.124c-.27 0-.466.067-.586.203-.12.135-.182.338-.187.609 0 .078.005.156.015.234.01.079.016.157.016.235 0 .026-.003.039-.008.039a.218.218 0 0 1-.047-.016 4.263 4.263 0 0 1-.093-.039.774.774 0 0 0-.118-.039.514.514 0 0 0-.203-.008 1.007 1.007 0 0 1-.125.008c-.073 0-.11-.013-.11-.039 0-.078-.004-.177-.015-.297-.01-.12-.036-.24-.078-.36a.995.995 0 0 0-.156-.296c-.063-.078-.156-.12-.281-.125a.323.323 0 0 0-.227.086.905.905 0 0 0-.164.203.64.64 0 0 0-.086.266 5.4 5.4 0 0 1-.031.25 1.459 1.459 0 0 0 .07.406c.026.083.055.156.086.219.031.062.068.093.11.093.025 0 .06-.018.101-.054.042-.037.063-.07.063-.102 0-.016-.008-.026-.024-.031a.147.147 0 0 0-.047-.008c-.036 0-.068-.018-.094-.055a.468.468 0 0 1-.062-.125 5.144 5.144 0 0 1-.047-.148.564.564 0 0 1 .055-.398c.047-.084.133-.128.258-.133zM5.023 15.18c.125 0 .248-.01.368-.032a.97.97 0 0 0 .336-.125.614.614 0 0 0 .234-.242.943.943 0 0 0 .094-.375.816.816 0 0 0-.047-.273.963.963 0 0 0-.133-.25 2.763 2.763 0 0 0-.203-.281 2.763 2.763 0 0 1-.203-.282 62.93 62.93 0 0 1-.29-.43c-.093-.14-.187-.288-.28-.445a8.124 8.124 0 0 1-.235-.406 2.646 2.646 0 0 0-.266-.398 1.203 1.203 0 0 0-.218-.211.469.469 0 0 0-.29-.094.436.436 0 0 0-.296.11 2.26 2.26 0 0 0-.258.265 3.241 3.241 0 0 1-.297.305c-.11.099-.25.177-.422.234a.744.744 0 0 0-.312.172c-.073.073-.11.185-.11.336 0 .104.008.208.024.312.015.104.026.209.031.313 0 .14-.02.273-.063.398a1.157 1.157 0 0 0-.062.367c0 .141.05.24.148.297.1.058.211.097.336.117.157.027.305.047.446.063.14.016.278.04.414.07.135.032.27.065.406.102.135.036.279.094.43.172.03.015.078.034.14.054l.211.07c.078.027.151.048.219.063a.741.741 0 0 0 .148.024zm2.86-.938c.146 0 .302-.015.469-.047a3.54 3.54 0 0 0 .976-.336 2.59 2.59 0 0 0 .406-.257.222.222 0 0 0 .032-.047.305.305 0 0 0 .023-.063v-.008c.031-.114.057-.24.078-.375a8.63 8.63 0 0 0 .055-.414 8.98 8.98 0 0 1 .055-.414c.02-.135.039-.268.054-.398.021-.14.047-.276.078-.406.032-.13.073-.253.125-.368a1.03 1.03 0 0 1 .211-.304 1.54 1.54 0 0 1 .344-.25v-.016l-.008-.023a.29.29 0 0 1 .047-.149 1.4 1.4 0 0 1 .117-.164.582.582 0 0 1 .149-.133.946.946 0 0 1 .164-.078 9.837 9.837 0 0 0-.102-.375 4.938 4.938 0 0 1-.094-.375 7.126 7.126 0 0 0-.093-.476 2.954 2.954 0 0 0-.11-.36 1.317 1.317 0 0 0-.18-.32c-.077-.104-.174-.23-.288-.375a1.189 1.189 0 0 1-.118-.156.555.555 0 0 1-.046-.196 2.206 2.206 0 0 0-.047-.203 9.48 9.48 0 0 0-.242-.75 2.91 2.91 0 0 0-.172-.383 3.87 3.87 0 0 0-.172-.289c-.052-.078-.107-.117-.164-.117-.125 0-.274.05-.446.149-.171.099-.354.208-.546.328-.193.12-.38.232-.563.336-.182.104-.346.153-.492.148a.7.7 0 0 1-.43-.148 2.236 2.236 0 0 1-.36-.344c-.109-.13-.2-.242-.273-.336-.073-.094-.127-.146-.164-.156-.041 0-.065.031-.07.093a2.56 2.56 0 0 0-.008.211v.133c0 .032-.005.052-.016.063-.057.12-.12.237-.187.351-.068.115-.135.232-.203.352a1.611 1.611 0 0 0-.219.758c0 .078.005.156.016.234.01.078.036.154.078.227l-.016.03a1.31 1.31 0 0 1-.133.157 1.072 1.072 0 0 0-.132.164 2.796 2.796 0 0 0-.407.93c-.078.333-.12.672-.125 1.015 0 .089.006.178.016.266.01.089.016.177.016.266a.526.526 0 0 1-.008.086.525.525 0 0 0-.008.086.75.75 0 0 1 .313.109c.12.068.25.154.39.258.14.104.274.224.399.36.125.135.244.267.359.398.115.13.198.26.25.39.052.13.086.237.101.32a.444.444 0 0 1-.125.329.955.955 0 0 1-.312.203c.089.156.198.289.328.398.13.11.271.198.422.266.151.068.315.117.492.148.177.032.35.047.516.047zm3.133 1.11c.109 0 .216-.016.32-.047a1.65 1.65 0 0 0 .445-.203c.136-.089.26-.198.375-.329a3.07 3.07 0 0 1 .977-.75l.258-.117a2.18 2.18 0 0 0 .257-.133.962.962 0 0 0 .165-.132.256.256 0 0 0 .078-.188.295.295 0 0 0-.024-.117.58.58 0 0 0-.07-.117 5.136 5.136 0 0 1-.203-.305 1.978 1.978 0 0 1-.149-.297l-.125-.312a2.558 2.558 0 0 1-.11-.352.28.28 0 0 0-.054-.101.53.53 0 0 0-.46-.235.533.533 0 0 0-.266.07l-.266.149a7.335 7.335 0 0 1-.281.148.656.656 0 0 1-.297.07.411.411 0 0 1-.258-.077.636.636 0 0 1-.172-.211 2.218 2.218 0 0 1-.117-.258l-.094-.258a1.26 1.26 0 0 1-.14.188.666.666 0 0 0-.125.203c-.068.156-.11.33-.125.523-.026.302-.06.596-.102.883a4.7 4.7 0 0 1-.21.86 1.914 1.914 0 0 0-.063.273 2.88 2.88 0 0 0-.032.289c0 .255.079.466.235.633.156.166.367.25.633.25z"
  }));
};

module.exports.VscTerminalPowershell = function VscTerminalPowershell(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m2.5 1.007 12.999.17.43.501-1.82 12.872-.57.489-13-.17-.43-.502L1.93 1.495l.57-.488zM1.18 13.885l11.998.157 1.68-11.882L2.86 2.003 1.18 13.885zm5.791-3.49-.14.991 5 .066.14-.99-5-.066zm1.71-2.457-3.663-2.93-.692.796 2.636 2.112L3.739 9.95l.465.812L8.68 7.938z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscTerminalTmux = function VscTerminalTmux(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M13.5 1h-12l-.5.5v13l.5.5h12l.5-.5v-13l-.5-.5zM7 7.5V13H2V2h5v5.5zm6 5.5H8V8h5v5zm0-6H8V2h5v5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscTerminalUbuntu = function VscTerminalUbuntu(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M3.26 8A1.37 1.37 0 1 1 .52 8a1.37 1.37 0 0 1 2.74 0zm7.79 6.66a1.37 1.37 0 1 0 2.374-1.37 1.37 1.37 0 0 0-2.374 1.37zm2.37-11.95a1.37 1.37 0 1 0-2.37-1.373 1.37 1.37 0 0 0 2.37 1.373zM8.79 4.1a3.9 3.9 0 0 1 3.89 3.55h2a5.93 5.93 0 0 0-1.73-3.8 1.91 1.91 0 0 1-1.66-.12 2.001 2.001 0 0 1-.94-1.38 6 6 0 0 0-1.54-.2 5.83 5.83 0 0 0-2.61.61l1 1.73a3.94 3.94 0 0 1 1.59-.39zM4.88 8a3.93 3.93 0 0 1 1.66-3.2l-1-1.7A5.93 5.93 0 0 0 3.1 6.5a1.92 1.92 0 0 1 0 3 5.93 5.93 0 0 0 2.42 3.4l1-1.7A3.93 3.93 0 0 1 4.88 8zm3.91 3.91a4 4 0 0 1-1.65-.37l-1 1.73c.81.403 1.704.612 2.61.61.52 0 1.038-.067 1.54-.2a2 2 0 0 1 .94-1.38 1.911 1.911 0 0 1 1.66-.12 5.93 5.93 0 0 0 1.73-3.8h-2a3.91 3.91 0 0 1-3.83 3.53z"
  }));
};

module.exports.VscTerminal = function VscTerminal(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 24 24"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3 1.5 1.5 3v18L3 22.5h18l1.5-1.5V3L21 1.5H3zM3 21V3h18v18H3zm5.656-4.01 1.038 1.061 5.26-5.243v-.912l-5.26-5.26-1.035 1.06 4.59 4.702-4.593 4.592z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscTextSize = function VscTextSize(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M3.36 7 1 13h1.34l.51-1.47h2.26L5.64 13H7L4.65 7H3.36zm-.15 3.53.78-2.14.78 2.14H3.21zM11.82 4h-1.6L7 13h1.56l.75-2.29h3.36l.77 2.29H15l-3.18-9zM9.67 9.5l1.18-3.59c.059-.185.1-.376.12-.57.027.192.064.382.11.57l1.25 3.59H9.67z"
  }));
};

module.exports.VscThreeBars = function VscThreeBars(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14 5H2V3h12v2zm0 4H2V7h12v2zM2 13h12v-2H2v2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscThumbsdown = function VscThumbsdown(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M5.46 14.11a1.46 1.46 0 0 1-.81-.25 1.38 1.38 0 0 1-.45-1.69L5.17 10H2.38a1.36 1.36 0 0 1-1.16-.61 1.35 1.35 0 0 1-.09-1.32C1.8 6.62 3 4 3.4 2.9A1.38 1.38 0 0 1 4.69 2h8.93A1.4 1.4 0 0 1 15 3.4v3.51a1.38 1.38 0 0 1-1.38 1.38h-1.38L6.4 13.75a1.41 1.41 0 0 1-.94.36zM4.69 3a.39.39 0 0 0-.36.25C3.93 4.34 2.86 6.7 2 8.49a.39.39 0 0 0 0 .36.37.37 0 0 0 .38.15h3.3l.52.68v.46l-1.09 2.44a.37.37 0 0 0 .13.46.38.38 0 0 0 .48 0l6.06-5.59.47-.13h1.37a.38.38 0 0 0 .38-.41V3.4a.4.4 0 0 0-.38-.4H4.69z"
  }));
};

module.exports.VscThumbsup = function VscThumbsup(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M10.54 2c.289.001.57.088.81.25a1.38 1.38 0 0 1 .45 1.69l-.97 2.17h2.79a1.36 1.36 0 0 1 1.16.61 1.35 1.35 0 0 1 .09 1.32c-.67 1.45-1.87 4.07-2.27 5.17a1.38 1.38 0 0 1-1.29.9H2.38A1.4 1.4 0 0 1 1 12.71V9.2a1.38 1.38 0 0 1 1.38-1.38h1.38L9.6 2.36a1.41 1.41 0 0 1 .94-.36zm.77 11.11a.39.39 0 0 0 .36-.25c.4-1.09 1.47-3.45 2.33-5.24a.39.39 0 0 0 0-.36.37.37 0 0 0-.38-.15h-3.3l-.52-.68v-.46l1.09-2.44a.37.37 0 0 0-.13-.46.38.38 0 0 0-.48 0L4.22 8.66l-.47.13H2.38A.38.38 0 0 0 2 9.2v3.51a.4.4 0 0 0 .38.4h8.93z"
  }));
};

module.exports.VscTools = function VscTools(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m14.773 3.485-.78-.184-2.108 2.096-1.194-1.216 2.056-2.157-.18-.792a4.42 4.42 0 0 0-1.347-.228 3.64 3.64 0 0 0-1.457.28 3.824 3.824 0 0 0-1.186.84 3.736 3.736 0 0 0-.875 1.265 3.938 3.938 0 0 0 0 2.966 335.341 335.341 0 0 0-6.173 6.234c-.21.275-.31.618-.284.963a1.403 1.403 0 0 0 .464.967c.124.135.272.247.437.328.17.075.353.118.538.127.316-.006.619-.126.854-.337 1.548-1.457 4.514-4.45 6.199-6.204.457.194.948.294 1.444.293a3.736 3.736 0 0 0 2.677-1.133 3.885 3.885 0 0 0 1.111-2.73 4.211 4.211 0 0 0-.196-1.378zM2.933 13.928a.31.31 0 0 1-.135.07.437.437 0 0 1-.149 0 .346.346 0 0 1-.144-.057.336.336 0 0 1-.114-.11c-.14-.143-.271-.415-.14-.568 1.37-1.457 4.191-4.305 5.955-6.046.1.132.21.258.328.376.118.123.245.237.38.341-1.706 1.75-4.488 4.564-5.98 5.994zm11.118-9.065c.002.765-.296 1.5-.832 2.048a2.861 2.861 0 0 1-4.007 0 2.992 2.992 0 0 1-.635-3.137A2.748 2.748 0 0 1 10.14 2.18a2.76 2.76 0 0 1 1.072-.214h.254L9.649 3.839v.696l1.895 1.886h.66l1.847-1.816v.258zM3.24 6.688h1.531l.705.717.678-.674-.665-.678V6.01l.057-1.649-.22-.437-2.86-1.882-.591.066-.831.849-.066.599 1.838 2.918.424.215zm-.945-3.632L4.609 4.58 4.57 5.703H3.494L2.002 3.341l.293-.285zm7.105 6.96.674-.673 3.106 3.185a1.479 1.479 0 0 1 0 2.039 1.404 1.404 0 0 1-1.549.315 1.31 1.31 0 0 1-.437-.315l-3.142-3.203.679-.678 3.132 3.194a.402.402 0 0 0 .153.105.477.477 0 0 0 .359 0 .403.403 0 0 0 .153-.105.436.436 0 0 0 .1-.153.525.525 0 0 0 .036-.184.547.547 0 0 0-.035-.184.436.436 0 0 0-.1-.153L9.4 10.016z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscTrash = function VscTrash(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M10 3h3v1h-1v9l-1 1H4l-1-1V4H2V3h3V2a1 1 0 0 1 1-1h3a1 1 0 0 1 1 1v1zM9 2H6v1h3V2zM4 13h7V4H4v9zm2-8H5v7h1V5zm1 0h1v7H7V5zm2 0h1v7H9V5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscTriangleDown = function VscTriangleDown(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M2 5.56 2.413 5h11.194l.393.54L8.373 11h-.827L2 5.56z"
  }));
};

module.exports.VscTriangleLeft = function VscTriangleLeft(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m10.44 2 .56.413v11.194l-.54.393L5 8.373v-.827L10.44 2z"
  }));
};

module.exports.VscTriangleRight = function VscTriangleRight(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M5.56 14 5 13.587V2.393L5.54 2 11 7.627v.827L5.56 14z"
  }));
};

module.exports.VscTriangleUp = function VscTriangleUp(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "m14 10.44-.413.56H2.393L2 10.46 7.627 5h.827L14 10.44z"
  }));
};

module.exports.VscTwitter = function VscTwitter(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M15 3.784a5.63 5.63 0 0 1-.65.803 6.058 6.058 0 0 1-.786.68 5.442 5.442 0 0 1 .014.377c0 .574-.061 1.141-.184 1.702a8.467 8.467 0 0 1-.534 1.627 8.444 8.444 0 0 1-1.264 2.04 7.768 7.768 0 0 1-1.72 1.521 7.835 7.835 0 0 1-2.095.95 8.524 8.524 0 0 1-2.379.329 8.178 8.178 0 0 1-2.293-.325A7.921 7.921 0 0 1 1 12.52a5.762 5.762 0 0 0 4.252-1.19 2.842 2.842 0 0 1-2.273-1.19 2.878 2.878 0 0 1-.407-.8c.091.014.181.026.27.035a2.797 2.797 0 0 0 1.022-.089 2.808 2.808 0 0 1-.926-.362 2.942 2.942 0 0 1-.728-.633 2.839 2.839 0 0 1-.65-1.822v-.033c.402.227.837.348 1.306.362a2.943 2.943 0 0 1-.936-1.04 2.955 2.955 0 0 1-.253-.649 2.945 2.945 0 0 1 .007-1.453c.063-.243.161-.474.294-.693.364.451.77.856 1.216 1.213a8.215 8.215 0 0 0 3.008 1.525 7.965 7.965 0 0 0 1.695.263 2.15 2.15 0 0 1-.058-.325 3.265 3.265 0 0 1-.017-.331c0-.397.075-.77.226-1.118a2.892 2.892 0 0 1 1.528-1.528 2.79 2.79 0 0 1 1.117-.225 2.846 2.846 0 0 1 2.099.909 5.7 5.7 0 0 0 1.818-.698 2.815 2.815 0 0 1-1.258 1.586A5.704 5.704 0 0 0 15 3.785z"
  }));
};

module.exports.VscTypeHierarchySub = function VscTypeHierarchySub(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13.5 11h-1.729L8.438 6H9.5l.5-.5v-4L9.5 1h-4l-.5.5v4l.5.5h1.062l-3.333 5H1.5l-.5.5v3l.5.5h3l.5-.5v-3l-.5-.5h-.068L7.5 6.4l3.068 4.6H10.5l-.5.5v3l.5.5h3l.5-.5v-3l-.5-.5zM6 5V2h3v3H6zm-2 7v2H2v-2h2zm9 2h-2v-2h2v2z"
  }));
};

module.exports.VscTypeHierarchySuper = function VscTypeHierarchySuper(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13.5 1h-3l-.5.5v3l.5.5h.068L7.5 9.6 4.432 5H4.5l.5-.5v-3L4.5 1h-3l-.5.5v3l.5.5h1.729l3.333 5H5.5l-.5.5v4l.5.5h4l.5-.5v-4l-.5-.5H8.438l3.333-5H13.5l.5-.5v-3l-.5-.5zM2 4V2h2v2H2zm7 7v3H6v-3h3zm4-7h-2V2h2v2z"
  }));
};

module.exports.VscTypeHierarchy = function VscTypeHierarchy(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M13.5 12h-1.793L10 10.293V6.5L9.5 6H8V4h.5l.5-.5v-2L8.5 1h-2l-.5.5v2l.5.5H7v2H5.5l-.5.5v3.793L3.293 12H1.5l-.5.5v2l.5.5h2l.5-.5v-1.793L5.707 11h3.586L11 12.707V14.5l.5.5h2l.5-.5v-2l-.5-.5zM7 2h1v1H7V2zM6 7h3v3H6V7zm-3 7H2v-1h1v1zm10 0h-1v-1h1v1z"
  }));
};

module.exports.VscUnfold = function VscUnfold(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.53 6.51v-4l-1 1-.71-.71L7.65 1h.71l1.84 1.83-.71.7-1-1v3.98h-.96zm0 2.98v4l-1-1-.71.71L7.65 15h.71l1.84-1.83-.71-.7-1 1V9.49h-.96zM13.73 4 14 5.02l-3.68 2.93L14 10.98 13.73 12h-4.2v-1h3L9.55 8.57H6.54L3.45 11h3.08v1H2.27L2 10.98l3.68-2.92L2 5.02 2.27 4h4.26v1H3.45l3 2.42h3.01L12.53 5h-3V4h4.2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscUngroupByRefType = function VscUngroupByRefType(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M2.9 1 5 3.1l-.8.7L3 2.6V7H2V2.5L.8 3.8l-.7-.7L2.2 1h.7zM3 13.4V9H2v4.4L.8 12.2l-.7.7L2.2 15h.7L5 12.9l-.7-.7L3 13.4zM8.5 7h-2L6 6.5v-2l.5-.5h2l.5.5v2l-.5.5zM7 6h1V5H7v1zm7.5 1h-3l-.5-.5v-3l.5-.5h3l.5.5v3l-.5.5zM12 6h2V4h-2v2zm-3.5 6h-2l-.5-.5v-2l.5-.5h2l.5.5v2l-.5.5zM7 11h1v-1H7v1zm7.5 2h-3l-.5-.5v-3l.5-.5h3l.5.5v3l-.5.5zM12 12h2v-2h-2v2zm-1-2H9v1h2v-1zm0-5H9v1h2V5z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscUnlock = function VscUnlock(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5 7V5a3 3 0 0 1 5.83-1h1.044A4.002 4.002 0 0 0 4 5v2H3L2 8v6l1 1h10l1-1V8l-1-1H5zm6 1h2v6H3V8h8z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscUnmute = function VscUnmute(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 4.83h2.79L8.15 1l.85.35v13l-.85.33-3.86-3.85H1.5l-.5-.5v-5l.5-.5zM4.85 10 8 13.14V2.56L4.85 5.68l-.35.15H2v4h2.5l.35.17zM15 7.83a6.97 6.97 0 0 1-1.578 4.428l-.712-.71A5.975 5.975 0 0 0 14 7.83c0-1.4-.48-2.689-1.284-3.71l.712-.71A6.971 6.971 0 0 1 15 7.83zm-2 0a4.978 4.978 0 0 1-1.002 3.004l-.716-.716A3.982 3.982 0 0 0 12 7.83a3.98 3.98 0 0 0-.713-2.28l.716-.716c.626.835.997 1.872.997 2.996zm-2 0c0 .574-.16 1.11-.44 1.566l-.739-.738a1.993 1.993 0 0 0 .005-1.647l.739-.739c.276.454.435.988.435 1.558z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscUnverified = function VscUnverified(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.67 14.72h.71L10.1 13h2.4l.5-.5v-2.42l1.74-1.72v-.71l-1.71-1.72V3.49l-.5-.49H10.1L8.38 1.29h-.71L6 3H3.53L3 3.5v2.43L1.31 7.65v.71L3 10.08v2.42l.53.5H6l1.67 1.72zM6.16 12H4V9.87l-.12-.35L2.37 8l1.48-1.51.15-.35V4h2.16l.36-.14L8 2.35l1.54 1.51.35.14H12v2.14l.17.35L13.69 8l-1.55 1.52-.14.35V12H9.89l-.38.15L8 13.66l-1.48-1.52-.36-.14zm1.443-5.859a.962.962 0 0 0-.128.291c-.03.109-.05.215-.062.317l-.005.043h-.895l.003-.051c.018-.326.089-.615.212-.864.052-.108.117-.214.193-.318.081-.106.18-.2.294-.28.119-.084.255-.15.409-.2A1.71 1.71 0 0 1 8.165 5c.28 0 .523.046.726.14.2.089.366.21.494.363.127.152.22.326.279.52.058.194.087.394.087.599 0 .191-.032.371-.098.54-.064.164-.143.32-.238.466-.094.143-.197.28-.31.41-.11.129-.211.252-.304.372a2.47 2.47 0 0 0-.23.34.653.653 0 0 0-.088.318v.48h-.888v-.539c0-.168.031-.323.094-.464a2.15 2.15 0 0 1 .24-.401c.096-.127.2-.25.308-.368a4.74 4.74 0 0 0 .299-.356c.093-.12.17-.246.228-.377a.984.984 0 0 0 .09-.421 1.04 1.04 0 0 0-.047-.318v-.001a.638.638 0 0 0-.13-.243.558.558 0 0 0-.216-.158H8.46a.689.689 0 0 0-.294-.059.643.643 0 0 0-.339.083.742.742 0 0 0-.223.215zM8.5 11h-.888v-.888H8.5V11z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscVariableGroup = function VscVariableGroup(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M5.387 11.523a.402.402 0 0 1 .593-.367c.058.031.11.065.157.102.047.036.088.07.125.101a.177.177 0 0 0 .117.047c.052 0 .12-.04.203-.117.083-.078.175-.182.273-.313.1-.13.201-.268.305-.414.104-.146.2-.294.29-.445l.226-.39c.062-.11.107-.199.133-.266a15.33 15.33 0 0 0-.133-.524 15.384 15.384 0 0 1-.133-.523 3.72 3.72 0 0 0-.133-.422 1.04 1.04 0 0 0-.187-.313.656.656 0 0 0-.266-.187 1.374 1.374 0 0 0-.375-.07 1.628 1.628 0 0 0-.328.031v-.195L7.69 7a2.345 2.345 0 0 1 .461.734c.052.13.097.263.133.399.037.135.076.283.117.445.078-.115.175-.26.29-.438a4.49 4.49 0 0 1 .398-.523c.15-.172.31-.315.476-.43A1.02 1.02 0 0 1 10.089 7c.13 0 .247.034.351.101.105.068.157.175.157.32 0 .282-.141.423-.422.423a.608.608 0 0 1-.29-.07.608.608 0 0 0-.288-.071c-.1 0-.203.05-.313.148a2.3 2.3 0 0 0-.312.352 9.5 9.5 0 0 0-.485.734l.446 1.852a1.56 1.56 0 0 0 .093.344.669.669 0 0 0 .094.171.184.184 0 0 0 .125.079.37.37 0 0 0 .211-.086 2.14 2.14 0 0 0 .43-.47c.052-.077.093-.15.125-.218l.187.094a2.025 2.025 0 0 1-.219.367 3.775 3.775 0 0 1-.351.422 3.38 3.38 0 0 1-.406.36c-.141.104-.269.153-.383.148a.397.397 0 0 1-.281-.102 1.491 1.491 0 0 1-.204-.234 1.599 1.599 0 0 1-.132-.36 8.263 8.263 0 0 1-.118-.507 34.16 34.16 0 0 1-.101-.532 2.212 2.212 0 0 0-.11-.414l-.203.375a4.489 4.489 0 0 1-.28.453c-.11.157-.222.316-.337.477a2.46 2.46 0 0 1-.375.422c-.135.12-.265.221-.39.305A.66.66 0 0 1 5.91 12a.539.539 0 0 1-.36-.133.454.454 0 0 1-.163-.344zm6.11.477c.28-.36.496-.748.648-1.164a3.87 3.87 0 0 0 .226-1.32c0-.47-.075-.912-.226-1.329A4.57 4.57 0 0 0 11.495 7h.734a3.77 3.77 0 0 1 .922 2.515c0 .474-.073.917-.218 1.329-.146.411-.38.796-.704 1.156h-.734zM3.77 12a3.373 3.373 0 0 1-.704-1.149 3.97 3.97 0 0 1-.218-1.336c0-.953.307-1.791.922-2.515h.726a4.132 4.132 0 0 0-.64 1.18 4.205 4.205 0 0 0-.227 1.335A3.929 3.929 0 0 0 4.496 12H3.77z"
  }), React.createElement("path", {
    d: "M15.5 1H.5l-.5.5v13l.5.5h15l.5-.5v-13l-.5-.5zM15 14H1V5h14v9zm0-10H1V2h14v2z"
  }));
};

module.exports.VscVerifiedFilled = function VscVerifiedFilled(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8.38 14.72h-.71L6 13H3.53L3 12.5v-2.42L1.31 8.36v-.71L3 5.93V3.5l.53-.5H6l1.67-1.71h.71L10.1 3h2.43l.5.49v2.44l1.71 1.72v.71L13 10.08v2.42l-.5.5h-2.4l-1.72 1.72Zm-1.65-4.24h.71l3.77-3.77L10.5 6 7.09 9.42 5.71 8.04 5 8.75l1.73 1.73Z"
  }));
};

module.exports.VscVerified = function VscVerified(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.67 14.72h.71L10.1 13h2.4l.5-.5v-2.42l1.74-1.72v-.71l-1.71-1.72V3.49l-.5-.49H10.1L8.38 1.29h-.71L6 3H3.53L3 3.5v2.43L1.31 7.65v.71L3 10.08v2.42l.53.5H6l1.67 1.72zM6.16 12H4V9.87l-.12-.35L2.37 8l1.48-1.51.15-.35V4h2.16l.36-.14L8 2.35l1.54 1.51.35.14H12v2.14l.17.35L13.69 8l-1.55 1.52-.14.35V12H9.89l-.38.15L8 13.66l-1.48-1.52-.36-.14zm.57-1.52h.71l3.77-3.77L10.5 6 7.09 9.42 5.71 8.04 5 8.75l1.73 1.73z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscVersions = function VscVersions(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M8 3 7 4v8l1 1h6l1-1V4l-1-1H8zm6 9H8V4h6v8zM5 9V5h1V4H4.5l-.5.5v7l.5.5H6v-1H5V9zM2 8V6h1V5H1.5l-.5.5v5l.5.5H3v-1H2V8z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscVmActive = function VscVmActive(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 2h13l.5.5v5.503a5.006 5.006 0 0 0-1-.583V3H2v9h5a5 5 0 0 0 1 3H4v-1h3v-1H1.5l-.5-.5v-10l.5-.5z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M9.778 8.674a4 4 0 1 1 4.444 6.652 4 4 0 0 1-4.444-6.652zm2.13 4.99 2.387-3.182-.8-.6-2.077 2.769-1.301-1.041-.625.78 1.704 1.364.713-.09z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscVmConnect = function VscVmConnect(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 2h13l.5.5v5.503a5.006 5.006 0 0 0-1-.583V3H2v9h5a5 5 0 0 0 1 3H4v-1h3v-1H1.5l-.5-.5v-10l.5-.5z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    d: "M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8zm0 7a3 3 0 1 1 0-6.001A3 3 0 0 1 12 15z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m12.133 11.435 1.436 1.436.431-.431-1.004-1.005L14 10.431l-.431-.43-1.436 1.434zm-1.129 1.067L10 11.498l.431-.431 1.436 1.435-1.436 1.436-.431-.431 1.004-1.005z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscVmOutline = function VscVmOutline(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 2h13l.5.5v5.503a5.006 5.006 0 0 0-1-.583V3H2v9h5a5 5 0 0 0 1 3H4v-1h3v-1H1.5l-.5-.5v-10l.5-.5z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    d: "M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8zm0 7a3 3 0 1 1 0-6.001A3 3 0 0 1 12 15z"
  }));
};

module.exports.VscVmRunning = function VscVmRunning(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M1.5 2h13l.5.5v5.503a5.006 5.006 0 0 0-1-.583V3H2v9h5a5 5 0 0 0 1 3H4v-1h3v-1H1.5l-.5-.5v-10l.5-.5z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    d: "M12 8c.367 0 .721.047 1.063.14.34.094.658.23.953.407.294.177.563.385.808.625.245.24.455.509.63.808a4.03 4.03 0 0 1 .405 3.082c-.093.342-.229.66-.406.954a4.382 4.382 0 0 1-.625.808c-.24.245-.509.455-.808.63a4.029 4.029 0 0 1-3.082.405 3.784 3.784 0 0 1-.954-.406 4.382 4.382 0 0 1-.808-.625 3.808 3.808 0 0 1-.63-.808 4.027 4.027 0 0 1-.405-3.082c.093-.342.229-.66.406-.954.177-.294.385-.563.625-.808.24-.245.509-.455.808-.63A4.028 4.028 0 0 1 12 8zm2 3.988L11 10v4l3-2.012z"
  }));
};

module.exports.VscVm = function VscVm(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M14.5 2h-13l-.5.5v10l.5.5H7v1H4v1h8v-1H9v-1h5.5l.5-.5v-10l-.5-.5zM14 12H2V3h12v9z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscWand = function VscWand(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M4.38 5h1V4h1V3h-1V2h-1v1h-1v1h1v1zm8 4h-1v1h-1v1h1v1h1v-1h1v-1h-1V9zM14 2V1h-1v1h-1v1h1v1h1V3h1V2h-1zm-2.947 2.442a1.49 1.49 0 0 0-2.12 0l-7.49 7.49a1.49 1.49 0 0 0 0 2.12c.59.59 1.54.59 2.12 0l7.49-7.49c.58-.58.58-1.53 0-2.12zm-8.2 8.9c-.2.2-.51.2-.71 0-.2-.2-.2-.51 0-.71l6.46-6.46.71.71-6.46 6.46zm7.49-7.49-.32.32-.71-.71.32-.32c.2-.2.51-.2.71 0 .19.2.19.52 0 .71z"
  }));
};

module.exports.VscWarning = function VscWarning(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M7.56 1h.88l6.54 12.26-.44.74H1.44L1 13.26 7.56 1zM8 2.28 2.28 13H13.7L8 2.28zM8.625 12v-1h-1.25v1h1.25zm-1.25-2V6h1.25v4h-1.25z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscWatch = function VscWatch(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M7.5 9h2V8H8V5.5H7v3l.5.5z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M5.5 3.669A4.998 4.998 0 0 0 3 8a4.998 4.998 0 0 0 2.5 4.331V14.5l.5.5h4l.5-.5v-2.169A4.998 4.998 0 0 0 13 8a4.998 4.998 0 0 0-2.5-4.331V1.5L10 1H6l-.5.5v2.169zM12 8a4 4 0 1 1-8 0 4 4 0 0 1 8 0z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscWhitespace = function VscWhitespace(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M12 2V1H6.5a3.5 3.5 0 0 0 0 7H8v5H7v1h5v-1h-1V2h1zM8 7H6.5a2.5 2.5 0 1 1 0-5H8v5zm2 6H9V2h1v11z"
  }));
};

module.exports.VscWholeWord = function VscWholeWord(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M0 11h1v2h14v-2h1v3H0v-3Z",
    "clip-rule": "evenodd"
  }), React.createElement("path", {
    d: "M6.84 11h-.88v-.86h-.022c-.383.66-.947.989-1.692.989-.548 0-.977-.145-1.289-.435-.308-.29-.462-.675-.462-1.155 0-1.028.605-1.626 1.816-1.794l1.649-.23c0-.935-.378-1.403-1.134-1.403-.662 0-1.26.226-1.794.677v-.902c.541-.344 1.164-.516 1.87-.516 1.292 0 1.938.684 1.938 2.052V11Zm-.88-2.782L4.633 8.4c-.408.058-.716.16-.924.307-.208.143-.311.399-.311.768 0 .268.095.488.284.66.194.168.45.253.768.253a1.41 1.41 0 0 0 1.08-.457c.286-.308.43-.696.43-1.165v-.548ZM9.348 10.205h-.022V11h-.88V2.857h.88v3.61h.021c.434-.73 1.068-1.096 1.902-1.096.705 0 1.257.247 1.654.741.401.49.602 1.15.602 1.977 0 .92-.224 1.658-.672 2.213-.447.551-1.06.827-1.837.827-.726 0-1.276-.308-1.649-.924Zm-.022-2.218v.768c0 .455.147.841.44 1.16.298.315.674.473 1.128.473.534 0 .951-.204 1.252-.613.304-.408.456-.975.456-1.702 0-.613-.141-1.092-.424-1.44-.283-.347-.666-.52-1.15-.52-.511 0-.923.178-1.235.536-.311.355-.467.8-.467 1.338Z"
  }));
};

module.exports.VscWindow = function VscWindow(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M14.5 2h-13l-.5.5v11l.5.5h13l.5-.5v-11l-.5-.5zM14 13H2V6h12v7zm0-8H2V3h12v2z"
  }));
};

module.exports.VscWordWrap = function VscWordWrap(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M3.868 3.449a1.21 1.21 0 0 0-.473-.329c-.274-.111-.623-.15-1.055-.076a3.5 3.5 0 0 0-.71.208c-.082.035-.16.077-.235.125l-.043.03v1.056l.168-.139c.15-.124.326-.225.527-.303.196-.074.4-.113.604-.113.188 0 .33.051.431.157.087.095.137.248.147.456l-.962.144c-.219.03-.41.086-.57.166a1.245 1.245 0 0 0-.398.311c-.103.125-.181.27-.229.426-.097.33-.093.68.011 1.008a1.096 1.096 0 0 0 .638.67c.155.063.328.093.528.093a1.25 1.25 0 0 0 .978-.441v.345h1.007V4.65c0-.255-.03-.484-.089-.681a1.423 1.423 0 0 0-.275-.52zm-.636 1.896v.236c0 .119-.018.231-.055.341a.745.745 0 0 1-.377.447.694.694 0 0 1-.512.027.454.454 0 0 1-.156-.094.389.389 0 0 1-.094-.139.474.474 0 0 1-.035-.186c0-.077.01-.147.024-.212a.33.33 0 0 1 .078-.141.436.436 0 0 1 .161-.109 1.3 1.3 0 0 1 .305-.073l.661-.097zm5.051-1.067a2.253 2.253 0 0 0-.244-.656 1.354 1.354 0 0 0-.436-.459 1.165 1.165 0 0 0-.642-.173 1.136 1.136 0 0 0-.69.223 1.33 1.33 0 0 0-.264.266V1H5.09v6.224h.918v-.281c.123.152.287.266.472.328.098.032.208.047.33.047.255 0 .483-.06.677-.177.192-.115.355-.278.486-.486a2.29 2.29 0 0 0 .293-.718 3.87 3.87 0 0 0 .096-.886 3.714 3.714 0 0 0-.078-.773zm-.86.758c0 .232-.02.439-.06.613-.036.172-.09.315-.159.424a.639.639 0 0 1-.233.237.582.582 0 0 1-.565.014.683.683 0 0 1-.21-.183.925.925 0 0 1-.142-.283A1.187 1.187 0 0 1 6 5.5v-.517c0-.164.02-.314.06-.447.036-.132.087-.242.156-.336a.668.668 0 0 1 .228-.208.584.584 0 0 1 .29-.071.554.554 0 0 1 .496.279c.063.099.108.214.143.354.031.143.05.306.05.482zM2.407 9.9a.913.913 0 0 1 .316-.239c.218-.1.547-.105.766-.018.104.042.204.1.32.184l.33.26V8.945l-.097-.062a1.932 1.932 0 0 0-.905-.215c-.308 0-.593.057-.846.168-.25.11-.467.27-.647.475-.18.21-.318.453-.403.717-.09.272-.137.57-.137.895 0 .289.043.561.13.808.086.249.211.471.373.652.161.185.361.333.597.441.232.104.493.155.778.155.233 0 .434-.028.613-.084.165-.05.322-.123.466-.217l.078-.061v-.889l-.2.095a.4.4 0 0 1-.076.026c-.05.017-.099.035-.128.049-.036.023-.227.09-.227.09-.06.024-.14.043-.218.059a.977.977 0 0 1-.599-.057.827.827 0 0 1-.306-.225 1.088 1.088 0 0 1-.205-.376 1.728 1.728 0 0 1-.076-.529c0-.21.028-.399.083-.56.054-.158.13-.294.22-.4zM14 6h-4V5h4.5l.5.5v6l-.5.5H7.879l2.07 2.071-.706.707-2.89-2.889v-.707l2.89-2.89L9.95 9l-2 2H14V6z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscWorkspaceTrusted = function VscWorkspaceTrusted(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8.069 0c.262 0 .52.017.76.057a4.1 4.1 0 0 1 .697.154c.228.069.451.155.674.263.217.103.44.229.663.366.377.24.748.434 1.126.589a7.537 7.537 0 0 0 2.331.525c.406.029.823.046 1.257.046v4c0 .76-.097 1.48-.291 2.166a8.996 8.996 0 0 1-.789 1.943 10.312 10.312 0 0 1-1.188 1.725 15.091 15.091 0 0 1-1.492 1.532 17.57 17.57 0 0 1-1.703 1.325c-.594.412-1.194.795-1.794 1.143l-.24.143-.24-.143a27.093 27.093 0 0 1-1.806-1.143 15.58 15.58 0 0 1-1.703-1.325 15.082 15.082 0 0 1-1.491-1.532 10.947 10.947 0 0 1-1.194-1.725 9.753 9.753 0 0 1-.789-1.943A7.897 7.897 0 0 1 .571 6V2c.435 0 .852-.017 1.258-.046a8.16 8.16 0 0 0 1.188-.171c.383-.086.766-.2 1.143-.354A6.563 6.563 0 0 0 5.28.846C5.72.56 6.166.349 6.606.21A4.79 4.79 0 0 1 8.069 0zm6.502 2.983a9.566 9.566 0 0 1-2.234-.377 7.96 7.96 0 0 1-2.046-.943A4.263 4.263 0 0 0 9.23 1.16 3.885 3.885 0 0 0 8.074.994a3.99 3.99 0 0 0-1.165.166 3.946 3.946 0 0 0-1.058.503A7.926 7.926 0 0 1 3.8 2.61c-.709.206-1.451.332-2.229.378v3.017c0 .663.086 1.297.258 1.908a8.58 8.58 0 0 0 .72 1.743 9.604 9.604 0 0 0 1.08 1.572c.417.491.862.948 1.342 1.382.48.435.983.835 1.509 1.206.531.372 1.063.709 1.594 1.017a22.397 22.397 0 0 0 1.589-1.017 15.389 15.389 0 0 0 1.514-1.206c.48-.434.926-.891 1.343-1.382a9.596 9.596 0 0 0 1.08-1.572 8.258 8.258 0 0 0 .709-1.743 6.814 6.814 0 0 0 .262-1.908V2.983z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "m11.797 4.709-.44-.378-.406.035-4.36 5.148-1.485-2.12-.4-.068-.463.331-.069.4 1.909 2.726.217.12.457.028.234-.102 4.835-5.715-.029-.405z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscWorkspaceUnknown = function VscWorkspaceUnknown(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8.067 0c.263 0 .52.017.76.057a4.1 4.1 0 0 1 .697.154c.229.069.452.155.675.263.217.103.44.229.662.366a7.2 7.2 0 0 0 1.126.589 7.534 7.534 0 0 0 2.332.525c.405.029.822.046 1.257.046v4c0 .76-.097 1.48-.292 2.166a8.996 8.996 0 0 1-.788 1.943 10.306 10.306 0 0 1-1.189 1.725 15.082 15.082 0 0 1-1.491 1.532 17.57 17.57 0 0 1-1.703 1.325c-.594.412-1.194.795-1.794 1.143l-.24.143-.24-.143a27.088 27.088 0 0 1-1.806-1.143 15.579 15.579 0 0 1-1.703-1.325 15.08 15.08 0 0 1-1.491-1.532 10.948 10.948 0 0 1-1.195-1.725 9.753 9.753 0 0 1-.788-1.943A7.897 7.897 0 0 1 .57 6V2c.434 0 .851-.017 1.257-.046a8.16 8.16 0 0 0 1.189-.171c.383-.086.765-.2 1.143-.354a6.563 6.563 0 0 0 1.12-.583C5.719.56 6.164.349 6.604.21A4.79 4.79 0 0 1 8.067 0zm6.503 2.983a9.567 9.567 0 0 1-2.234-.377 7.96 7.96 0 0 1-2.046-.943 4.264 4.264 0 0 0-1.063-.503A3.885 3.885 0 0 0 8.073.994a3.99 3.99 0 0 0-1.166.166 3.946 3.946 0 0 0-1.057.503 7.927 7.927 0 0 1-2.051.948c-.709.206-1.452.332-2.229.378v3.017c0 .663.086 1.297.257 1.908a8.58 8.58 0 0 0 .72 1.743 9.604 9.604 0 0 0 1.08 1.572c.417.491.863.948 1.343 1.382.48.435.983.835 1.509 1.206.531.372 1.062.709 1.594 1.017a22.4 22.4 0 0 0 1.588-1.017 15.384 15.384 0 0 0 1.515-1.206c.48-.434.925-.891 1.343-1.382a9.609 9.609 0 0 0 1.08-1.572 8.269 8.269 0 0 0 .708-1.743 6.814 6.814 0 0 0 .263-1.908V2.983z"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M9.433 4.72c.171.171.314.377.411.606.103.228.155.48.149.754a1.6 1.6 0 0 1-.114.64 2.24 2.24 0 0 1-.292.48 2.787 2.787 0 0 1-.354.383 4.52 4.52 0 0 0-.337.32 1.421 1.421 0 0 0-.24.32.7.7 0 0 0-.086.348v.36l-.131.138h-.715l-.143-.143V8.57c0-.24.04-.45.12-.634.075-.177.166-.343.28-.486a3.42 3.42 0 0 1 .366-.382c.12-.109.229-.212.332-.32.097-.103.182-.212.245-.326a.707.707 0 0 0 .086-.354.966.966 0 0 0-.074-.36.972.972 0 0 0-.2-.298.94.94 0 0 0-1.32 0 .88.88 0 0 0-.2.298.829.829 0 0 0-.075.36L7 6.21h-.715l-.131-.137c0-.263.046-.514.148-.748.103-.229.24-.435.412-.606.177-.177.383-.32.611-.417a1.883 1.883 0 0 1 1.503 0c.229.103.434.24.606.417zM7.57 9.646l.143-.143h.714l.143.143v.714l-.143.143h-.714l-.143-.143v-.714z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscWorkspaceUntrusted = function VscWorkspaceUntrusted(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    d: "M8.067 0c.263 0 .52.017.76.057a4.1 4.1 0 0 1 .697.154c.229.069.452.155.675.263.217.103.44.229.662.366a7.2 7.2 0 0 0 1.126.589 7.534 7.534 0 0 0 2.332.525c.405.029.822.046 1.257.046v4c0 .76-.097 1.48-.292 2.166a8.996 8.996 0 0 1-.788 1.943 10.306 10.306 0 0 1-1.189 1.725 15.082 15.082 0 0 1-1.491 1.532 17.57 17.57 0 0 1-1.703 1.325c-.594.412-1.194.795-1.794 1.143l-.24.143-.24-.143a27.088 27.088 0 0 1-1.806-1.143 15.579 15.579 0 0 1-1.703-1.325 15.08 15.08 0 0 1-1.491-1.532 10.948 10.948 0 0 1-1.195-1.725 9.753 9.753 0 0 1-.788-1.943A7.897 7.897 0 0 1 .57 6V2c.434 0 .851-.017 1.257-.046a8.16 8.16 0 0 0 1.189-.171c.383-.086.765-.2 1.143-.354a6.563 6.563 0 0 0 1.12-.583C5.719.56 6.164.349 6.604.21A4.79 4.79 0 0 1 8.067 0zm6.503 2.983a9.567 9.567 0 0 1-2.234-.377 7.96 7.96 0 0 1-2.046-.943 4.264 4.264 0 0 0-1.063-.503A3.885 3.885 0 0 0 8.073.994a3.99 3.99 0 0 0-1.166.166 3.946 3.946 0 0 0-1.057.503 7.927 7.927 0 0 1-2.051.948c-.709.206-1.452.332-2.229.378v3.017c0 .663.086 1.297.257 1.908a8.58 8.58 0 0 0 .72 1.743 9.604 9.604 0 0 0 1.08 1.572c.417.491.863.948 1.343 1.382.48.435.983.835 1.509 1.206.531.372 1.062.709 1.594 1.017a22.4 22.4 0 0 0 1.588-1.017 15.384 15.384 0 0 0 1.515-1.206c.48-.434.925-.891 1.343-1.382a9.609 9.609 0 0 0 1.08-1.572 8.269 8.269 0 0 0 .708-1.743 6.814 6.814 0 0 0 .263-1.908V2.983z"
  }), React.createElement("path", {
    d: "m10.787 5.446-.4-.406h-.206L8.2 7.023 6.216 5.04h-.2l-.406.406v.2l1.983 1.983L5.61 9.61v.206l.406.4h.2l1.983-1.983 1.982 1.983h.206l.4-.4V9.61L8.804 7.63l1.983-1.983v-.2z"
  }));
};

module.exports.VscZoomIn = function VscZoomIn(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M12.027 6.149a5.52 5.52 0 0 1-1.27 3.908l4.26 4.26-.7.71-4.26-4.27a5.52 5.52 0 1 1 1.97-4.608zm-5.45 4.888a4.51 4.51 0 0 0 3.18-1.32l-.04.02a4.51 4.51 0 0 0 1.36-3.2 4.5 4.5 0 1 0-4.5 4.5zm2.44-4v-1h-2v-2h-1v2h-2v1h2v2h1v-2h2z",
    "clip-rule": "evenodd"
  }));
};

module.exports.VscZoomOut = function VscZoomOut(props) {
  return React.createElement("svg", _extends({}, props, {
    fill: "currentColor",
    viewBox: "0 0 16 16"
  }), React.createElement("path", {
    "fill-rule": "evenodd",
    d: "M12.027 6.149a5.52 5.52 0 0 1-1.27 3.908l4.26 4.26-.7.71-4.26-4.27a5.52 5.52 0 1 1 1.97-4.608zm-5.45 4.888a4.51 4.51 0 0 0 3.18-1.32l-.04.02a4.51 4.51 0 0 0 1.36-3.2 4.5 4.5 0 1 0-4.5 4.5zm-2.54-4.98h5v1h-5v-1z",
    "clip-rule": "evenodd"
  }));
};